...
(() => {
    ...
    const gForm = document.getElementById("payslip-generate-form"), gBtn = document.getElementById("payslip-generate-btn");
    if (gBtn && gForm)
    ...
    guardSubmit(gForm, "Generate Payslip route is unavailable. Please contact technical support or your domain administrator.");
    const eForm = document.getElementById("payslip-export-form"), monthSel = document.querySelector(".month_date"), yearSel = document.querySelector(".year_date");
    if (eForm) {
        const fm = eForm.querySelector("input.filter_month"), fy = eForm.querySelector("input.filter_year");
        const syncHidden = () => {
            if (fm && monthSel)
                fm.value = monthSel.value ?? "";
            if (fy && yearSel)
                fy.value = yearSel.value ?? "";
        };
        syncHidden();
    ...
    guardSubmit(eForm, "Export Payslip route is unavailable. Please contact technical support or your domain administrator.");
    ...
    if (bc) {
        if (!bc.getAttribute("data-listener-bound-click")) {
            ...
            bc.addEventListener("click", (e) => {
                const href = bc.getAttribute("href") ?? bc.getAttribute("data-url") ?? "#";
                if (!href || href === "#") {
                    e.preventDefault();
                    const msg = bc.getAttribute("data-guard-msg") ??
