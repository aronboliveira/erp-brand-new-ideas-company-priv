 ...
 * @module change
 */
/* eslint-disable @typescript-eslint/explicit-function-return-type, @typescript-eslint/no-unsafe-argument, @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-return, @typescript-eslint/no-unused-vars */
  ...
  const showToastOrAlert = msg => {
    try {
      ...
      if (hasToast) {
        ...
        t.show();
        setTimeout((): void => { el.remove(); }, 3400);
      } else {
        alert(msg);
      }
    } catch {
