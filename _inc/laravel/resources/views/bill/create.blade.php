...
@section('content')
    <div class="row">
        {{ Collective\Html\FormFacade::open(array('url' => ViewsConstants::BIL,'class'=>'w-100')) }}
        <div class="col-12">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group" id="vendor-box">
                                ...
                                {{ Collective\Html\FormFacade::select('vendor_id', $vendors,$vendorId, array('class' => 'form-control select','id'=>'vendor','data-url'=>route(ViewsConstants.BIL.'.vendor'),'required'=>'required')) }}
                            </div>
                        ...
                        @php
                            ...
                            $fields = [
                                ['name'=>'bill_date',     'type'=>'date',     'label'=>__('Bill Date'),   'required'=>true],
                                ['name'=>'due_date',      'type'=>'date',     'label'=>__('Due Date'),    'required'=>true],
                                ['name'=>'bill_number',   'type'=>'readonly', 'label'=>__('Bill Number'), 'value'=>$bill_number],
                                ['name'=>'category_id',   'type'=>'select',   'label'=>__('Category'),    'options'=>$category],
                                ['name'=>'order_number',  'type'=>'number',   'label'=>__('Order Number'),'required'=>false],
                            ];
                        @endphp
                        <div class="col-md-6">
                            <div class="row">
                                @foreach($fields as $f)
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            @switch($f['type'])
                                                @case('date')
                                                    {{ Form::date($f['name'], null, ['class'=>'form-control','required'=>($f['required']??false)]) }}
                                                    @break
                                                @case('number')
                                                    {{ Form::number($f['name'], null, ['class'=>'form-control'] + ($f['required']?['required'=>'required']:[])) }}
                                                    @break
                                                @case('select')
                                                    {{ Form::select($f['name'], $f['options'], null, ['class'=>'form-control select']) }}
                                                    @break
                                                @case('readonly')
                                                    <input type="text" class="form-control" value="{{ $f['value'] }}" readonly>
                                                    @break
                                            @endswitch
                                        </div>
                                    </div>
                        ...
                        <table class="table mb-0" data-repeater-list="items" id="sortable-table">
                            ...
                            <tr>
                                <td width="25%" class="form-group pt-0">
                                     {{ Collective\Html\FormFacade::select('item', $product_services,'', array('class' => 'form-control select2 item ','data-url'=>route(ViewsConstants.BIL.'.product'))) }}
                                </td>
                                <td>
        ...
        <div class="modal-footer">
            <input type="button" value="{{__('Cancel')}}" onclick="location.href = '{{route(ViewsConstants::BIL.'.index')}}';" class="btn btn-light">
