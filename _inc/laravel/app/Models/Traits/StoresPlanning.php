...
trait StoresPlanning
{
	protected function addPlanningColumns(Blueprint $table, bool $nullableTitle = false, bool $nullableFixedDate = true, bool $nullableFixedTime = true, bool $nullableDept = true, string $onDeleteDept = 'set null'): void
	{
	    try {
    		...
    		$nullableDept ? $table->uuid(CPC::COL_DEP_ID)->nullable()->index() : $table->uuid(CPC::COL_DEP_ID)->index();
    		$table->unsignedTinyInteger(PJC::COL_MIN_DR)->default(15)->nullable();
    		$table->unsignedTinyInteger(PJC::COL_EXP_DR)->default(30)->nullable();
    		$table->unsignedTinyInteger(PJC::COL_MAX_DR)->default(60)->nullable();
    		$table->string('url', 256)->nullable();
    		$table->string('location', 256)->nullable();
    		$table->text('note')->nullable();
    		$table->boolean(CPC::COL_IS_INT)->default(true)->nullable();
	        ...
	        Log::error(static::class . '::addPlanningColumns — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
	    }
	}
	...
	protected function dropPlanningColumnForeigns(Blueprint $table, string $tableName): void
	{
	    try {
	        ...
	        Log::error(static::class . '::dropPlanningColumnForeigns — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
	    }
	}
}
