        ...
        if(Auth::check())
        {
            ...
            if(\Auth::user()->can('show hrm dashboard'))
            {
                ...
                if($user->type != 'client' && $user->type != 'company')
                {
                    ...
                    $announcements = Announcement::orderBy('announcements.id', 'desc')->take(5)->leftjoin('employee_announcements', 'announcements.id', '=', 'employee_announcements.announcement_id')->where('employee_announcements.employee_id', '=', $emp->id)->orWhere(function ($q){
                    ...
                    $date               = date("Y-m-d");
                    $time               = date("H:i:s");
                    $employeeAttendance = EmployeeAttendance::orderBy('id', 'desc')->where('employee_id', '=', !empty(\Auth::user()->employee) ? \Auth::user()->employee->id : 0)->where('date', '=', $date)->first();
                ...
                else
                {
                    ...
                    $notClockIn  = EmployeeAttendance::where('date', '=', $currentDate)->get()->pluck('employee_id');
    ...
    public function clientView()
    {
        ...
        if(Auth::check())
        {
            ...
            elseif(Auth::user()->type == 'client')
            {
                ...
                return view('dashboard.clientView', compact('calendarTasks', 'arrErr', 'arrCount', 'chartData', 'project', 'invoice', 'top_tasks', 'top_due_invoice', 'users', 'project_status', 'projectData', 'taskData', 'transdate', 'currentYear'));
            }
        }
    }
