...
use App\Exceptions\Reliability\{QuarantineRollbackRequiredException, WarehousePostWriteValidationFailedException};
...
class WarehouseOperationService
{
     ...
     */
    public function run(string $operationType, callable $callback, array $options = []): WarehouseOperationResult
    {
        $operationKey = (string) ($options['operation_key'] ?? 'warehouse-op-' . (string) Str::uuid());
        $eventType = (string) ($options['event_type'] ?? $operationType . '.completed');
        $stream = (string) ($options['stream'] ?? 'warehouse.operations');
        $policy = $this->warehouseReliabilityPolicy($options);
        ...
        $operationCallback = function (?OperationLedger $ledger, CriticalOperationService $operations) use ($callback, $options, $eventType): mixed {
            ...
            if ($this->postWriteValidationRequested($options)) {
                ...
                if (!$assessment->postWriteValidationRequired) {
                    $operations->recordStep($ledger, 'warehouse.post_write_validation', 'Assess warehouse post-write validation threshold', [
                        ...
                        'result' => [
                            'validated' => false,
                            'reason' => 'below_warehouse_post_write_threshold',
                ...
                $validator = $options['post_write_validator'] ?? new WarehousePostWriteValidator();
                $validation = $validator instanceof WarehousePostWriteValidator
                    ? $validator->validate($eventType, $payload, $ledger)
                    : (new WarehousePostWriteValidator())->validate($eventType, $payload, $ledger);

                $operations->recordStep($ledger, 'warehouse.post_write_validation', 'Validate warehouse write after persistence', [
                    'step_type' => 'validation',
                    'sequence' => 90,
                ...
                if (!$validation->passed) {
                    ...
                    throw new WarehousePostWriteValidationFailedException($validation, $assessment, $ledger);
                }
            }
        ...
        try {
            $value = $this->operations->run($operationType, $operationCallback, array_merge($options, [
                ...
                'domain' => $options['domain'] ?? 'warehouse',
                ...
                'outbox' => function (mixed $result, OperationLedger $ledger) use ($options, $eventType, $stream): array {
                    $payload = $this->resolveArray($options['payload'] ?? null, $result, $ledger, true);
                    $assessment = $this->warehouseReliabilityPolicy($options)->assess($eventType, $payload, $ledger, $options);
                    $messageKey = $this->resolveValue($options['message_key'] ?? null, $result, $ledger);
                    $metadata = array_merge(
                        [
                            'operation_key' => $ledger->operation_key,
                            'warehouse_reliability' => $assessment->toArray(),
                            'retry' => [
        ...
        return new WarehouseOperationResult(
            $value,
            $ledger,
    ...
    private function warehouseReliabilityPolicy(array $options): WarehouseReliabilityPolicy
    {
        $policy = $options['warehouse_reliability_policy'] ?? null;

        return $policy instanceof WarehouseReliabilityPolicy ? $policy : new WarehouseReliabilityPolicy();
    }
