...
def main() -> None:
	init(autoreset=True)
	reference_input = input("Enter reference path (structure to mirror): ").strip()
	reference: Path
	if os.path.isabs(reference_input):
		reference = Path(reference_input)
	else:
		reference = Path.cwd() / reference_input
	target_input = input("Enter target path (directory to reorganize): ").strip()
	target: Path
	if os.path.isabs(target_input):
		target = Path(target_input)
	else:
		target = (Path.cwd() / target_input) if not target_input == '.' else Path.cwd()
	...
	for filename, file_path in target_files:
		if filename in reference_files:
			...
			if len(current_rel.parts) > len(expected_rel.parts):
				print(Fore.YELLOW + f"⚠ Skipping nested file: {file_path} "
