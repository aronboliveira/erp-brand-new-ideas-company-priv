...
$user = Auth::user();
        ...
        \Log::error('holidays/calendar — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
    }
@endphp
...
@section(YW::ADM_CTT)
    @if(!$hasUser)
        <div class="{{ VC::ALT_WRN_MB0 }}" role="alert">{{ __('Failed to load Holidays for the current user.') }}</div>
    @else
        <div class="row">
            <div class="{{ VC::CS12 }}">
                                                    ...
                                                    <div class="{{ VC::PX3 }}">
                                                        <div class="row {{ VC::ALC }}">
                                                            <div class="col ml-n2">
                                                                ...
                                                                <p class="card-text small {{ VC::TX_PM }}">{{ $titleTxt }}</p>
                                                                <p class="card-text small {{ VC::TX_DK }}">
...
@endphp
                                            ...
                                            @endforeach
                                        @else
                                            <div class="{{ VC::TXCT }}">{{ __('No holidays found!') }}</div>
                                        @endif
                                    </div>
                                </div>
                            </li>
