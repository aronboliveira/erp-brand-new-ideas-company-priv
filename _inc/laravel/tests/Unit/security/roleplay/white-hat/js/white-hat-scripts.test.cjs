...
describe("White Hat — Script Validation", () => {
  ...
  describe("dom_xss_probe.cjs", () => {
    const { generateDomXssPayloads, auditHtmlForSinks } = require(
      path.join(SCRIPTS, "js/scripts/dom_xss_probe.cjs")
    );
  ...
  describe("owasp_sqli_scan.sh", () => {
    test("script produz output estruturado", () => {
      const out = run(
        `bash "${path.join(SCRIPTS, "bash/scripts/owasp_sqli_scan.sh")}"`,
        60000
      );
  ...
  describe("session_fixation_test.sh", () => {
    test("script produz output sobre fixação de sessão", () => {
      const out = run(
        `bash "${path.join(SCRIPTS, "bash/scripts/session_fixation_test.sh")}"`,
        30000
      );
  ...
  describe("owasp_header_audit.py", () => {
    test("script analisa headers e produz relatório", () => {
      const out = run(
        `python3 "${path.join(SCRIPTS, "py/scripts/owasp_header_audit.py")}" ${SERVER}`,
        20000
      );
      expect(out).toContain("[WHITE-HAT]");
  ...
  describe("auth_bypass_probe.py", () => {
    test("script testa bypass de autenticação", () => {
      const out = run(
        `python3 "${path.join(SCRIPTS, "py/scripts/auth_bypass_probe.py")}" ${SERVER}`,
        30000
      );
  ...
  describe("param_tamper.php", () => {
    test("script PHP executa e testa type juggling", () => {
      const out = run(
        `php "${path.join(SCRIPTS, "php/scripts/param_tamper.php")}"`,
        30000
      );
      expect(out).toContain("[WHITE-HAT]");
