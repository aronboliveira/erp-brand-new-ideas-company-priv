...
(function () {
    ...
    const errFb = "# ERROR", dataClientLocalized = "data-client-localized", dataGuardMsg = "data-guard-msg", dataSvLocalized = "data-sv-localized", dataErrGuard = "data-error-guard", dataListenerGuard = "data-listener-guard";
    ...
    const initDataTable = () => {
        ...
        if (dtFn.isDataTable($table))
            return;
        const title = ($("#filename").val() ?? "").toString().trim(), hasButtons = $.fn.dataTable?.Buttons;
    ...
    const initTypeRadios = () => {
        ...
        radios.forEach((el, i) => {
            bindWithObserver(el, "change", onTypeChange, dataListenerGuard + "-type-" + i);
        });
