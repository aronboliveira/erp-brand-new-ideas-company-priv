...
(function (): void {
  ...
  function autoOpenModal(): void {
    const params = new URLSearchParams(window.location.search);
    const action = params.get("modal") as ModalAction | null;
    if (!action) return;
  ...
  function findTrigger(
    ...
    for (const trigger of triggers) {
      ...
      if (action === "create" && /\/create\/?$/.test(url)) return trigger;
      if (action === "edit" && id) {
        const re = new RegExp("/" + escRe(String(id)) + "/edit/?$");
        if (re.test(url)) return trigger;
      }
    }
    return null;
  }

  function openDirectly(action: ModalAction, id: string | null): void {
    if (typeof jQuery === "undefined" && typeof $ === "undefined") return;
    const jq: JQueryStatic =
      typeof $ !== "undefined" ? ($ as JQueryStatic) : jQuery;
    ...
    jq.ajax({
      ...
      success(html: string) {
        ...
        if (typeof window.taskCheckbox === "function") window.taskCheckbox();
        if (typeof window.common_bind === "function")
          window.common_bind("#commonModal");
        if (typeof window.commonLoader === "function") window.commonLoader();
      },
      error(xhr: { status: number; statusText: string; responseJSON?: Record<string, string> }) {
        devWarn(
          "modal-autoopen",
          `Failed to load modal content from ${url}: ${xhr.status} ${xhr.statusText}`,
        );
