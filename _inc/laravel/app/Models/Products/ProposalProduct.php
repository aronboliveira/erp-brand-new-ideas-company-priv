...
final class ProposalProduct extends Model
{
    ...
    protected $table = DC::TABLE_PPS_PRD;
    ...
    /** Proposal owning this line item. */
    public function proposal(): BelongsTo
    {
        return $this->belongsTo(Proposal::class, BC::COL_PPS_ID, 'id');
    }
    ...
    /** Cacheable aggregation: sum(total) for a proposal. */
    public static function cachedTotalForProposal(string $proposalId): float
    {
        static $cache = [];
        if (isset($cache[$proposalId])) return (float) $cache[$proposalId];

        try {
            $tbl = (new static)->getTable();
            $row = DB::selectOne(
                "select coalesce(sum(quantity * price), 0) as s from {$tbl} where " . BC::COL_PPS_ID . " = ?",
                [$proposalId]
            );
            $cache[$proposalId] = (float) ($row?->s ?? 0.0);
            return (float) $cache[$proposalId];
        } catch (\Throwable $e) {
            Log::warning(static::class . '::cachedTotalForProposal failed', [
