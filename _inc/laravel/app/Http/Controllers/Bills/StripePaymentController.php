...
class StripePaymentController extends Controller
{
    ...
    {
        $objUser = Auth::user();
        if ($objUser->type == PermissionsConstants::SA) {
            $orders = Order::select([
                'orders.*',
                'users.name as user_name',
            ])->join('users', 'orders.user_id', '=', 'users.id')->orderBy('orders.created_at', 'DESC')->with('totalCouponUsed.couponDetail')->with(['totalCouponUsed.couponDetail'])->get();
        } else {
            $orders = Order::select([
                'orders.*',
                'users.name as user_name',
            ...
            try {
                ...
                if ($data['amount_refunded'] == 0 && empty($data['failure_code']) && $data['paid'] == 1 && $data['captured'] == 1) {
                    ...
                    Order::create([
                        ...
                        'tax_id' => isset($data['balance_transaction']) ? $data['balance_transaction'] : '',
                ...
                try {
                    ...
                    if ($data['amount_refunded'] == 0 && empty($data['failure_code']) && $data['paid'] == 1 && $data['captured'] == 1) {
                        $payments = InvoicePayment::create([
                            ...
                            'tax_id' => $data['balance_transaction'],
                            'payment_type' => __('STRIPE'),
