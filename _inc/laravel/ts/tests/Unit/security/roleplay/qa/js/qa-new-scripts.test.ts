...
describe("QA — WASM Input Validator", () => {
  ...
  const { validateInput, generateBoundaryInputs } = require(path.join(SCRIPTS, "wasm/scripts/input_validator.ts"));
  ...
  test("script executa via node", () => {
    const output = execSync(`node --experimental-strip-types "${path.join(SCRIPTS, "wasm/scripts/input_validator.ts")}"`, {
      encoding: "utf-8",
      timeout: 10000,
    });
...
describe("QA — Accessibility + Security Audit", () => {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const { A11Y_SECURITY_CHECKS, runChecks } = require(path.join(SCRIPTS, "js/scripts/accessibility_security_audit.ts"));
