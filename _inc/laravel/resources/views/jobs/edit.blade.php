@php
    use App\Config\Constants\{
        PlansConstants as PL, ExtendingLayoutsConstants as EL, StacksConstants as ST,
        ViewsConstants as VW, ViewClassNamesConstants as VC, YieldingConstants as YW
    };
    ...
    $applicantArr     = is_array($job->applicant ?? null) ? $job->applicant : [];
    $visibilityArr    = is_array($job->visibility ?? null) ? $job->visibility : [];
...
@section(YW::ADM_ACT_BTN)
    @if($plan?->{PL::COL_GPT} == 1)
        <div class="{{ VC::FEND }}">
            <a href="{{ $genUrl }}" data-size="lg" class="btn btn-primary btn-icon btn-sm ai-btn"
                        ...
                        <div class="card-body job-create">
                            <div class="{{ VC::RW }}">
                                ...
                                <div class="{{ VC::CM12 }} {{ VC::DFL_JCB }}">
                                    <div></div>
                                    @if($plan?->{PL::COL_GPT} == 1)
                                        <a href="{{ $gramUrl }}" data-size="md" class="btn btn-primary btn-icon btn-sm grammar-btn"
                                        data-ajax-popup-over="true"
                                        id="grammarCheck"
                                        data-url="{{ $gramUrl }}"
                                        data-guard-msg="{{ $gramGuardMsg }}"
                                        data-bs-placement="top"
