                    ...
                    <table class="table datatable">
                            ...
                            @foreach ($jobOnBoards as $job)
                                <tr>
                                    ...
                                    <td>{{$user?->dateFormat(!empty($job->applications)?$job->applications->created_at:'-' )}}</td>
                                    <td>{{$user?->dateFormat($job->joining_date)}}</td>
                                    <td>
                                    ...
                                    <td>
                                        ...
                                        <div class="action-btn bg-warning ms-2">
                                            {{--                                            <a href="{{route('job.on.board.convert', $job->id)}}" class="{{ ViewClassNamesConstants::BT_SM_CT_PR }}"--}}
                                            ...
                                            {{--                                            </a>--}}
                                            {!! Collective\Html\FormFacade::open(['method' => 'get', 'route' => ['job.on.board.convert', $job->id],'id'=>'job-form-'.$job->id]) !!}
                                            ...
                                            <div class="action-btn bg-primary ms-2">
                                                <a href="#" data-url="{{route('job.on.board.edit', $job->id)}}" data-ajax-popup="true" class="mx-3 btn btn-sm align-items-center" data-bs-toggle="tooltip" title="{{__('Edit')}}" data-original-title="{{__('Edit')}}"><i class="{{ ViewClassNamesConstants::TI_PC_WT }}"></i></a>
                                            </div>
                                            ...
                                            @if ($job->status == 'confirm' )
                                                <div class="action-btn bg-secondary ms-2">
                                                    <a href="{{route('offer_letter.download.pdf',$job->id)}}" class="mx-3 btn btn-sm  align-items-center " data-bs-toggle="tooltip" data-bs-placement="top" title="{{__('OfferLetter PDF')}}" target="_blanks"><i class="ti ti-download text-white"></i></a>
                                                </div>
                                                <div class="action-btn bg-secondary ms-2">
                                                    <a href="{{route('offer_letter.download.doc',$job->id)}}" class="mx-3 btn btn-sm  align-items-center " data-bs-toggle="tooltip" data-bs-placement="top" title="{{__('OfferLetter DOC')}}" target="_blanks"><i class="ti ti-download text-white"></i></a>
                                                </div>
                                            @endif
