        ...
        if (!form.getAttribute("data-listener-bound-submit")) {
            ...
            form.addEventListener("submit", function (e) {
                try {
                    ...
                    const sd = form.querySelector('input[name="start_date"]'), ed = form.querySelector('input[name="end_date"]');
                    if (sd && ed && sd.value && ed.value) {
                        const s = new Date(sd.value), en = new Date(ed.value);
                        if (en < s) {
                            e.preventDefault();
                            ...
                            const bs2 = typeof window.bootstrap !== "undefined"
                                ? window.bootstrap
                                : null;
                            if (bs2?.Toast) {
