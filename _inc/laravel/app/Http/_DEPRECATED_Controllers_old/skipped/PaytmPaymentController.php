...
class PaytmPaymentController extends Controller
{
        ...
        if ($plan) {
            ...
            if (isset($request->coupon) && !empty($request->coupon)) {
                ...
                if (!empty($coupons)) {
                    ...
                    if ($usedCoupon >= $coupons->limit) {
                        return Utility::errorRes(__('This coupon code has expired.'));
                    }
                    $price = $price - $discount_value;
                } else {
                    return Utility::errorRes(__('This coupon code is invalid or has expired.'));
                }
            }
            ...
            if ($price <= 0) {
                ...
                if ($assignPlan['is_success'] == true && !empty($plan)) {
                    ...
                    return Utility::errorRes(__('Plan fail to upgrade.'));
                }
            }
            ...
            return Utility::errorRes(__('Plan is deleted.'));
        }
    }
    ...
    public function customerPayWithPaytm(Request $request)
    {
        ...
        if ($invoice) {
            ...
            return Utility::errorRes(__('Invoice is deleted.'));
        }
    }
        ...
        if ($invoice) {
            try {
                ...
                if ($transaction->isSuccessful()) {
                    ...
                    if ($invoice->getDue() <= 0) {
                        Invoice::changeStatus($invoice->id, 4);
                    } else {
                        Invoice::changeStatus($invoice->id, 3);
                    }
                    ...
                    if (isset($setting['payment_notification']) && $setting['payment_notification'] == 1) {
                        Utility::sendSlackMsg('new_invoice_payment', $notificationArr, $invoice->created_by);
                    }
                    //Telegram Notification
                    if (isset($setting['telegram_payment_notification']) && $setting['telegram_payment_notification'] == 1) {
                        Utility::sendTelegramMsg('new_invoice_payment', $notificationArr, $invoice->created_by);
                    }
                    //Twilio Notification
                    if (isset($setting['twilio_payment_notification']) && $setting['twilio_payment_notification'] == 1) {
                        Utility::sendTwilioMsg($customer->contact, 'new_invoice_payment', $notificationArr, $invoice->created_by);
                    }
                    //webhook
