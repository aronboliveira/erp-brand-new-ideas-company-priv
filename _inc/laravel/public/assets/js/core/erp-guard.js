...
(function (global) {
  ...
  class ERPGuard {
    ...
    #autoGuardElement(element) {
      ...
      switch (guardType) {
        case "submit": {
          // Support submit guards declared on child nodes by resolving to a form.
          const targetForm = element instanceof HTMLFormElement ? element : element.closest?.("form") || element.querySelector?.("form");

          if (targetForm instanceof HTMLFormElement) {
            this.bindSubmitGuard(targetForm);
          }
          break;
        }
        case "click":
