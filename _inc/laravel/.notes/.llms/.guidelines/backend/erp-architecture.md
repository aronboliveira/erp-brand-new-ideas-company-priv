...
- Reliability foundation added 2026-05-10:
  `operation_ledgers`, `operation_steps`, `outbox_messages`,
  `inbox_messages`, and `operational_events`. See
...
- **Medium-High:** 31 FIND_IN_SET with raw SQL interpolation
- **Medium:** 109 ConsoleOutput in HTTP paths; upload getClientOriginalName() + 0777; ProjectController N+1 queries

## Frontend Security Fixes (2026-03-02)

- Fixed `javascript:` URI XSS in `public/assets/js/routes/timeTrackers/confirm.js`
- Added missing `safeFormAction()` in permissions/index.js, pipelines/index.js, performanceTypes/index.js
- These were regressions from the previous session's security fixes
