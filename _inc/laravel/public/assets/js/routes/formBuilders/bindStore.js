...
(() => {
    try {
        ...
        const radios = document.querySelectorAll(".lead_radio"), section = document.getElementById("lead_activated");
        const applyLeadToggle = () => {
            try {
                const on = Array.from(radios).find(r => r.checked)
                    ?.value === "1";
                if (!section)
                    return;
                if (on) {
        ...
        if (empLink && empLink.getAttribute("data-listener-active") !== "true") {
            ...
            if (!empLink.getAttribute("data-listener-bound-click")) {
                ...
                empLink.addEventListener("click", (e) => {
                    try {
                        const href = (empLink.getAttribute("href") ?? "#").trim(), url = (empLink.getAttribute("data-url") ?? "#").trim();
                        if (url !== "#" && href !== "#")
                            return;
                        e.preventDefault();
                        const msg = empLink.getAttribute("data-guard-msg") ??
