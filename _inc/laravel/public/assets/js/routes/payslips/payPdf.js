...
(() => {
    ...
    const localizeGuard = (el) => {
        ...
        const lang = getLang(), key = "savepdf_unavailable";
    ...
    const toast = (message) => {
        ...
        if (hasBs()) {
            ...
            const id = "toast-savepdf-error", node = document.createElement("div");
            node.id = id;
    ...
    const doSavePdf = (btn) => {
        try {
            const area = Q("#printableArea"), h2p = window.html2pdf;
            if (!area || !h2p?.().set)
                throw new Error("missing");
            const filename = (Q(".invoice .invoice-title h4")?.textContent ?? "document").trim(), opt = {
                margin: 0.3,
                filename,
    ...
    const bind = (btn) => {
        if (!btn || btn.getAttribute(ONCE) === "true")
            return;
