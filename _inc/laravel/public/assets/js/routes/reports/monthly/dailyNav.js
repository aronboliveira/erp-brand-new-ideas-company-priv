...
(() => {
    try {
        const homeTab = document.getElementById("pills-home-tab"), listenerAttr = "data-daily-purchase-nav-listener-added";
        if (homeTab && homeTab.getAttribute(listenerAttr) !== "true") {
            ...
            if (!homeTab.getAttribute("data-listener-bound-click")) {
                ...
                homeTab.addEventListener("click", (e) => {
                    e.preventDefault();
                    const url = homeTab.getAttribute("data-url"), href = homeTab.getAttribute("href");
                    if ((!url || url === "#") && (!href || href === "#"))
                        return;
                    const msg = "{{ $dailyPurchaseNavMsg }}", toastEl = document.querySelector(".toast");
                    if (toastEl &&
                        window.bootstrap &&
                        typeof bootstrap.Toast === "function") {
                        const toast = new bootstrap.Toast(toastEl), body = toastEl.querySelector(".toast-body");
                        if (body)
