...
describe("White Hat — Crypto Verifier (WASM)", () => {
  const { verifyHashFormat, verifyJwtIntegrity, verifyCsrfTokens } = require(path.join(SCRIPTS, "wasm/scripts/crypto_verifier.cjs"));
  ...
  describe("verifyCsrfTokens", () => {
    test("tokens aleatórios não são previsíveis", () => {
      const tokens = Array.from({ length: 10 }, () => require("crypto").randomBytes(32).toString("hex"));
...
describe("White Hat — CSP Bypass Tester", () => {
  const { CSP_BYPASS_VECTORS, parseCSP, analyzeBypasses } = require(path.join(SCRIPTS, "js/scripts/csp_bypass_test.cjs"));
