...
$usr ??= null;
    ...
    $qrHtml = '';
    try {
        $canQR = class_exists(\Milon\Barcode\DNS2D::class);
        ...
        if ($canQR && $routeUrl !== '') {
            ...
            $qrHtml = $dns2d->getBarcodeHTML($routeUrl, "QRCODE", 2, 2);
        }
    } catch (\Throwable $e) {
                        ...
                        @foreach($invoice->itemData as $key => $item)
                            @php
                                try {
                                    ...
                                    \Log::error('invoices/templates/template1 — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
                                }
@endphp
                            <tr>
                                <td>{{ $item->name ?? '-' }}</td>
