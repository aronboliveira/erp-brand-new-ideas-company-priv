...
const SPECIAL_NAMES = ["O'Brien", "名前テスト", "اسم", "José María Ñoño", "Teste 🎉👍", "<b>negrito</b>", "<script>alert(1)</script>", "test\\path\\file"];

const XSS_VECTORS = ["<script>alert(1)</script>", '<img src=x onerror="alert(1)">', "<svg/onload=alert(1)>", "javascript:alert(1)", '"><script>alert(1)</script>'];
...
describe("QA — Edge Cases & Input Validation (Unit)", () => {
  describe("Caracteres especiais em nomes", () => {
    test.each(SPECIAL_NAMES)('nome "%s" é uma string válida', name => {
      expect(typeof name).toBe("string");
      expect(name.length).toBeGreaterThan(0);
    });

    test.each(SPECIAL_NAMES)('nome "%s" pode ser URL-encoded', name => {
  ...
  describe("XSS — sanitização HTML", () => {
    test.each(XSS_VECTORS)('XSS "%s" é neutralizado por escapeHtml', vector => {
