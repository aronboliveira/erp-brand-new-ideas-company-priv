...
describe("CISO — Policy Hash Verifier (WASM)", () => {
  ...
  const { SECURITY_POLICIES, hashPolicy, auditPolicy } = require(path.join(SCRIPTS, "wasm/scripts/policy_hash.ts"));
  ...
  test("script executa via node", () => {
    const output = execSync(`node --experimental-strip-types "${path.join(SCRIPTS, "wasm/scripts/policy_hash.ts")}"`, {
      encoding: "utf-8",
      timeout: 10000,
    });
...
describe("CISO — TLS Certificate Audit", () => {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const { generateTlsReport, WEAK_CIPHERS, WEAK_PROTOCOLS } = require(path.join(SCRIPTS, "js/scripts/tls_certificate_audit.ts"));
