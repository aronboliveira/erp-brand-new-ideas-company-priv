...
$user = Auth::user();
        ...
        \Log::error('contracts/show — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
    }
@endphp
    ...
    @push(StacksConstants::ADM_SCR_PG)
        ...
        <script async>
            (function () {
                ...
                const errFb = H.ERR_FALLBACK || "# ERROR";
                ...
                const showErrorNow = H.showError || (msg => alert(msg ?? errFb));
                const scheduleClickError = H.scheduleErrorOnEvent
                    ? (msg => H.scheduleErrorOnEvent(msg, "click"))
                    : (msg => alert(msg ?? errFb));
                const localize = H.getLocalizedMessage || ((el, key) => errFb);
                const verifyRouteFromEl = H.verifyRouteFromElement || (el => "");
                const token = H.getToken || (() =>
                    document.querySelector('meta[name="csrf-token"]')?.getAttribute("content") ?? "");
                const ensureJq = () => {
                    if (H.ensureJQuery) {
                        return H.ensureJQuery(() => scheduleClickError(localize(document.body, "plugin_unavailable")));
                    }
                    if (!$ || !$.fn) {
                ...
                const ensureDropzone = () => {
                    if (H.ensureDropzone) {
                        return H.ensureDropzone(() => scheduleClickError(localize(document.body, "plugin_unavailable")));
                    }
                    if (!window.Dropzone) {
                        scheduleClickError(localize(document.body, "plugin_unavailable"));
                        return false;
                ...
                const bindComments = () => {
                    ...
                    $.ajax({
                        ...
                        try {
                            ...
                            const html =
                            "<div class='{{ VC::LG_IT }} px-0'><div class='{{ VC::R_ALC }}'><div class='{{ VC::C_AT }}'><a href='#' class='avatar avatar-sm rounded-circle {{ VC::MS2 }}'><img src=" +
                            (d.default_img ?? "") +
                            " alt='' class='avatar-sm rounded-circle'></a></div><div class='col ml-n2'><p class='{{ VC::DBL }} h6 {{ VC::TXSM }} font-weight-light {{ VC::MB0 }} text-break'>" +
                            (d.comment ?? "") +
                            "</p><small class='{{ VC::DBL }}'>" +
                            (d.current_time ?? "") +
                            "</small></div><div class='{{ VC::ACT_BTN_DNG }} me-4'><div class='{{ VC::C_AT }}'><a href='#' class='{{ VC::BT_SM_CT }} delete-comment' data-url='" +
                            (d.deleteUrl ?? "#") +
                            "'><i class='{{ VC::TI_TRS_WT }}'></i></a></div></div></div></div>";
        ...
        @can('manage contract')
            ...
            <script defer src="{{ asset('assets/js/routes/contracts/note.js') }}">
                ...
                const qs = H.qs || ((s, r = document) => r.querySelector(s));
                const showErrorNow = H.showError || (msg => alert(msg ?? errFb));
                const schedulePointerupError = H.scheduleErrorOnEvent
                    ? (msg => H.scheduleErrorOnEvent(msg, "pointerup"))
                    : (msg => alert(msg ?? errFb));
                const getMsg = H.getLocalizedMessage || ((el, key) => errFb);
                const verifyRoute = H.verifyRoute || (candidate => {
                    const a = document.createElement("a");
                ...
                const ensureJq = () => {
                    if (H.ensureJQuery) {
                        return H.ensureJQuery(() => schedulePointerupError(getMsg(document.body, "plugin_unavailable")));
                    }
                    if (!$ || !$.fn) {
...
@endphp
        ...
        <li class="{{ VC::BCI_ACT }}" aria-current="page">{{ method_exists($user, 'contractNumberFormat') ? $user->contractNumberFormat($cid) : __('Failed to format contract number') }}</li>
    @endsection
...
@endphp
            ...
            target="_blank"
            data-url="{{ $dlUrl }}"
            data-guard-msg="{{ base64_encode($msgDownload) }}"
...
@endphp
            ...
            class="{{ VC::BT_SM_PM }} btn-icon m-1"
            data-sv-localized="true"
            data-guard-msg="{{ base64_encode($msgPreview) }}">
...
@endphp
                ...
                class="{{ VC::BT_SM_PM }} btn-icon m-1"
                data-sv-localized="true"
                data-guard-msg="{{ base64_encode($msgSendMail) }}"
                data-bs-toggle="tooltip"
                ...
                data-sv-localized="true"
                data-guard-msg="{{ base64_encode($msgCopy) }}"
                data-bs-toggle="tooltip"
                title="{{ __('Duplicate') }}">
                ...
                data-ajax-popup="true"
                data-sv-localized="true"
                data-guard-msg="{{ base64_encode($msgSign) }}"
                data-bs-toggle="tooltip"
                    ...
                    <li class="{{ VC::STT_DD_IT }}">
                        ...
                        <div class="{{ VC::DRP_DSH_MN }}">
                            @forelse ($statusList as $k => $label)
                                <a class="{{ VC::DRP_IT }} status"
                                data-id="{{ $k }}"
                                data-url="{{ $statusUrl }}"
                                data-sv-localized="true"
                                data-guard-msg="{{ base64_encode($msgStatus) }}"
                                href="#">{{ ucfirst($label) }}</a>
                            @empty
                                <a class="{{ VC::DRP_IT }} disabled" href="#">{{ __('No status available') }}</a>
                            @endforelse
                        </div>
                    </li>
                </ul>
        ...
        @push(StacksConstants::ADM_SCR_PG)
            <script defer>
                (function () {
                    try {
                        const H = window.ContractHelpers || {};
                        const svToastOrAlert = H.svToastOrAlert || window.svToastOrAlert || function (msg) { alert(msg); };
                        const guardAnchor = H.guardAnchor || function(a, isDataUrl) {
                            if (!a) return;
                            var msg = a.getAttribute('data-guard-msg') || 'This action is unavailable.';
                            var attr = isDataUrl ? 'data-url' : 'href';
                            if ((a.getAttribute(attr) || '#') === '#') {
                                a.addEventListener('click', function (e) { e.preventDefault(); svToastOrAlert(msg); });
                            }
                        };
...
@endphp
            <div id="useradd-1">
                <div class="row">
                    <div class="col-xl-7">
                        <div class="row">
                            <div class="{{ VC::CL4 }} {{ VC::C6 }}">
                                <div class="{{ VC::CD }}">
                                    <div class="{{ VC::CD_BD }}" style="min-height: 205px;">
                                        <div class="theme-avatar {{ VC::BG_P }}"><i class="{{ VC::TI_USR_PLS }}"></i></div>
                            ...
                            <div class="{{ VC::CL4 }} {{ VC::C6 }}">
                                <div class="{{ VC::CD }}">
                                    <div class="{{ VC::CD_BD }}" style="min-height: 205px;">
                                        <div class="theme-avatar bg-warning"><i class="{{ VC::TI_FL }}"></i></div>
                    ...
                    <div class="col-xxl-5">
                        <div class="card report_card total_amount_card">
                            <div class="{{ VC::CD_BD }} pt-0" style="margin-bottom: -30px; margin-top: -10px;">
                                <address class="{{ VC::MB0 }} {{ VC::TXSM }}">
                                    <dl class="row {{ VC::MT4 }} {{ VC::ALC }}">
                                        <h5>{{ __('Contract Detail') }}</h5><br>
                                        <dt class="{{ VC::CS4 }} h6 {{ VC::TXSM }}">{{ __('Subject') }}</dt>
                                        <dd class="col-sm-8 {{ VC::TXSM }}">{{ !empty($contract->subject) ? $contract->subject : __('No subject available') }}</dd>
                                        <dt class="{{ VC::CS4 }} h6 {{ VC::TXSM }}">{{ __('Project') }}</dt>
                                        <dd class="col-sm-8 {{ VC::TXSM }}">{{ data_get($contract,'projects.project_name') ?: __('No project available') }}</dd>
                                        <dt class="{{ VC::CS4 }} h6 {{ VC::TXSM }}">{{ __('Value') }}</dt>
                                        <dd class="col-sm-8 {{ VC::TXSM }}">{{ isset($contract->value) && is_numeric($contract->value) ? ($hasPriceFormat ? ($authUser?->priceFormat($contract->value) ?? __('Failed to format value')) : __('Failed to format value')) : __('No value available') }}</dd>
                                        <dt class="{{ VC::CS4 }} h6 {{ VC::TXSM }}">{{ __('Type') }}</dt>
                                        <dd class="col-sm-8 {{ VC::TXSM }}">{{ data_get($contract,'types.name') ?: __('No type available') }}</dd>
                                        <dt class="{{ VC::CS4 }} h6 {{ VC::TXSM }}">{{ __('Status') }}</dt>
                                        <dd class="col-sm-8 {{ VC::TXSM }}">{{ !empty($contract->status) ? $contract->status : __('No status available') }}</dd>
                                        <dt class="{{ VC::CS4 }} h6 {{ VC::TXSM }}">{{ __('Start Date') }}</dt>
                                        <dd class="col-sm-8 {{ VC::TXSM }}">{{ !empty($contract->start_date) ? ($hasDateFormat ? ($authUser?->dateFormat($contract->start_date) ?? __('Failed to format start date')) : __('Failed to format start date')) : __('No start date available') }}</dd>
                                        <dt class="{{ VC::CS4 }} h6 {{ VC::TXSM }}">{{ __('End Date') }}</dt>
                                        <dd class="col-sm-8 {{ VC::TXSM }}">{{ !empty($contract->end_date) ? ($hasDateFormat ? ($authUser?->dateFormat($contract->end_date) ?? __('Failed to format end date')) : __('Failed to format end date')) : __('No end date available') }}</dd>
                                    </dl>
                                </address>
                            </div>
                        </div>
                ...
                <div class="{{ VC::CD }}">
                    <div class="{{ VC::CD_HD }}"><h5 class="{{ VC::MB0 }}">{{ __('Contract Description ') }}</h5></div>
                    <div class="{{ VC::CD_BD }}">
                        <div class="{{ VC::CM12 }}">
            ...
            <div id="useradd-2">
                <div class="{{ VC::CD }}">
                    <div class="{{ VC::CD_HD }}"><h5 class="{{ VC::MB0 }}">{{ __('Contract Attachments') }}</h5></div>
                    <div class="{{ VC::CD_BD }}">
                        <div class="{{ VC::FM_G }}">
...
@endphp
                                    <div class="card {{ VC::MB3 }} border {{ VC::SNN }}">
                                                    ...
                                                    <div class="{{ VC::C_AT }} actions">
                                                        <div class="{{ VC::ACT_BTN_DNG }}">
                                                            {!! Form::open(['method' => 'DELETE', $delUrl === '#' ? 'url' : 'route' => $delUrl === '#' ? '#' : [$delRouteName, $contract->id ?? 0, $file->id ?? 0], 'id' => 'file-del-form-'.$file->id]) !!}
                                                                <a href="#!" class="{{ VC::BT_SM_CT_PR }}" data-sv-localized="true" data-guard-msg="{{ base64_encode($delMsg) }}"><i class="{{ VC::TI_TRS_WT }}" data-bs-toggle="tooltip" data-bs-original-title="{{ __('Delete') }}"></i></a>
                                                            {!! Form::close() !!}
                                                        </div>
                                                    </div>
                                                @endif
                                ...
                                @empty
                                    <div class="{{ VC::TXCT_MT }} py-3">{{ __('No attachments available') }}</div>
                                @endforelse
                            </div>
                        </div>
                    </div>
            ...
            <div id="useradd-3">
                <div class="{{ VC::CD }}">
                    <div class="{{ VC::CD_HD }}"><h5 class="{{ VC::MB0 }}">{{ __('Comments') }}</h5></div>
                    <div class="{{ VC::CD_BD }}">
                        @php
                            try {
...
@endphp
                        @if($authUser && $authUser->{UsersConstants::COL_TP} === PermissionsConstants::CPN)
                            <div class="{{ VC::C12 }} {{ VC::DFL }}">
                                <div class="{{ VC::FM_G }} {{ VC::MB0 }} form-send {{ VC::W100 }}">
                                    <form method="post" class="card-comment-box" id="form-comment" data-action="{{ $cmtStoreUrl }}" data-sv-localized="true" data-guard-msg="{{ base64_encode($cmtStoreMsg) }}">
                                        <textarea rows="1" class="{{ VC::FM_CT }}" name="comment" data-toggle="autosize" placeholder="{{ __('Add a comment...') }}"></textarea>
                                    </form>
                                </div>
                                <button id="comment_submit" class="btn btn-send {{ VC::MT2 }}"><i class="f-16 {{ VC::TX_PM }} ti ti-brand-telegram"></i></button>
                            </div>
                        @elseif($authUser && $authUser->{UsersConstants::COL_TP} === PermissionsConstants::CL && ($contract->status ?? '') === 'accept')
                            <div class="{{ VC::C12 }} {{ VC::DFL }}">
                                <div class="{{ VC::FM_G }} {{ VC::MB0 }} form-send {{ VC::W100 }}">
...
@endphp
                                <div class="{{ VC::LGI }}">
                                    <div class="{{ VC::R_ALC }}">
                                        ...
                                        <div class="col ml-n2">
                                            ...
                                            <small class="{{ VC::DBL }}">{{ isset($comment->created_at) ? $comment->created_at->diffForHumans() : __('No timestamp available') }}</small>
                                        </div>
                                        @if($mayDelete)
                                            <div class="{{ VC::C_AT }} actions">
                                                <div class="{{ VC::ACT_BTN_DNG_2 }}">
                                                    {!! Form::open(['method' => 'DELETE', $cDelUrl === '#' ? 'url' : 'route' => $cDelUrl === '#' ? '#' : [$cDelName, $comment->id ?? 0], 'id' => 'comment-del-form-'.$comment->id]) !!}
                                                        <a href="#!" class="{{ VC::BT_SM_CT_PR }}" data-sv-localized="true" data-guard-msg="{{ base64_encode($cDelMsg) }}"><i class="{{ VC::TI_TRS_WT }}"></i></a>
                                                    {!! Form::close() !!}
                                                </div>
                                            </div>
                                        @endif
                            ...
                            @empty
                                <div class="{{ VC::TXCT_MT }} py-3">{{ __('No comments available') }}</div>
                            @endforelse
                        </div>
                    </div>
                </div>
...
@endphp
                            <div class="{{ VC::DFL_JCB }}">
                                <div class="{{ VC::MB0 }}">
                                    <a href="#" data-size="md" class="{{ VC::BT_PRM }} btn-icon {{ VC::BT_SM }}" data-ajax-popup-over="true" id="grammarCheck" data-url="{{ $grammarUrl }}" data-sv-localized="true" data-guard-msg="{{ base64_encode($grammarMsg) }}" data-bs-placement="top" data-title="{{ __('Grammar check with AI') }}">
...
@endphp
                                <div class="{{ VC::LGI }}">
                                    <div class="{{ VC::R_ALC }}">
                                        ...
                                        <div class="col ml-n2">
                                            ...
                                            <small class="{{ VC::DBL }}">{{ isset($note->created_at) ? $note->created_at->diffForHumans() : __('No timestamp available') }}</small>
                                        </div>
                                        @if($mayDelete)
                                            <div class="{{ VC::C_AT }} actions">
                                                <div class="{{ VC::ACT_BTN_DNG_2 }}">
                                                    {!! Form::open(['method' => 'DELETE', $nDelUrl === '#' ? 'url' : 'route' => $nDelUrl === '#' ? '#' : [$nDelName, $note->id ?? 0], 'id' => 'note-del-form-'.$note->id]) !!}
                                                        <a href="#!" class="{{ VC::BT_SM_CT_PR }}" data-sv-localized="true" data-guard-msg="{{ base64_encode($nDelMsg) }}"><i class="{{ VC::TI_TRS_WT }}"></i></a>
                                                    {!! Form::close() !!}
                                                </div>
                                            </div>
                                        @endif
                            ...
                            @empty
                                <div class="{{ VC::TXCT_MT }} py-3">{{ __('No notes available') }}</div>
                            @endforelse
                        </div>
                    </div>
                </div>
...
@else
    <div class="{{ VC::ALT_WRN }}">{{ __('No contract found.') }}</div>
@endif
