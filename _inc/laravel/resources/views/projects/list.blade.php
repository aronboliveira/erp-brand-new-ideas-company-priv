                    ...
                    @if(isset($projects) && !empty($projects) && count($projects) > 0)
                        @foreach ($projects as $key => $project)
                            <tr>
                                ...
                                <td class="text-end">
                                    <h5 class="mb-0 text-success">{{ $project->projectProgress($project , $last_task->id)['percentage'] }}</h5>
                                    <div class="progress mb-0">
                                        <div class="progress-bar bg-{{ $project->projectProgress($project , $last_task->id)['color'] }}" style="width: {{ $project->projectProgress($project , $last_task->id)['percentage'] }};"></div>
                                    </div>
                                </td>
                                <td class="text-end">
                                    <span>
                                        ...
                                        @can('delete project')
                                            <div class="action-btn bg-danger ms-2">
                                                    {!! Collective\Html\FormFacade::open(['method' => 'DELETE', 'route' => ['projects.user.destroy', [$project->id,$user?->id]]]) !!}
