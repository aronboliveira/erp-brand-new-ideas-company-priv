...
(function () {
        ...
        if (!el.getAttribute("data-listener-bound-click")) {
            ...
            el.addEventListener("click", function (e) {
                ...
                if (!txt)
                    return;
                e.preventDefault();
                const parts = String(txt).split("|"), title = parts[0] || "", body = parts[1] || "", yes = el.getAttribute("data-confirm-yes"), hasBs = !!(document.querySelector('link[rel="stylesheet"][href*="bootstrap"]') && window.bootstrap);
                if (hasBs) {
    ...
    function init() {
        ...
        document
            ...
            .forEach((el) => guardForm(el));
        document
            .querySelectorAll(".bs-pass-para")
            .forEach((el) => hookConfirm(el));
        try {
