    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="mt-2" id="multiCollapseExample1">
                <div class="card">
                    <div class="card-body">
                        {{ Collective\Html\FormFacade::open(['route' => [ViewsConstants::RPT . '.ledger'], 'method' => 'GET', 'id' => 'report_ledger']) }}
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-xl-3 col-lg-3 col-md-6 col-sm-12 col-12">
                                        <div class="btn-box">
                                            ...
                                            {{ Collective\Html\FormFacade::select('account', $accounts, isset($_GET['account']) ? $_GET['account'] : '', ['class' => 'form-control select']) }}
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-auto">
                                <div class="row">
                                    <div class="col-auto mt-4">
                                        ...
                                        <a href="{{ route(ViewsConstants::RPT . '.ledger') }}" class="btn btn-sm btn-danger"
                                    ...
                                    @foreach ($accountArrays as $account)
                                        ...
                                        @foreach ($account[0] as $a)
                                            ...
                                            @foreach ($account['bill'] as $billProduct)
                                                <tr>
                                                    ...
                                                    @php                                                        
                                                        ...
                                                        $vendor = \App\Models\Vendor::find(!empty($bill) ? $bill->vendor_id : '');
                                                    @endphp
                                            ...
                                            @php
                                                    ...
                                                    $vendor = \App\Models\Vendor::find($billId->vendor_id);
                                                @endphp
                                                <tr>
                                                    <td>{{ $accountName->name }}</td>
                                                    <td>{{ !empty($vendor) ? $vendor->name : '-' }}</td>
                                                    @if($billId->type == 'Expense')
                                                    <td>{{ \Auth::user()->expenseNumberFormat($billId->bill_id) }}{{ __(' Manually Payment') }}
                                                    </td>
                                                    @else
                                            ...
                                            @foreach ($account['payment'] as $paymentData)
                                                @php
                                                    $vendor = \App\Models\Vendor::find($paymentData->vendor_id);
                                                @endphp
                                                <tr>
                                                    <td>{{ $accountName->name }}</td>
                                                    <td>{{ !empty($vendor) ? $vendor->name : '-' }}</td>
