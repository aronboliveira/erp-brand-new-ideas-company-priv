...
$lang = Utility::fetchUserLang();
        ...
        \Log::error('customers/show — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
    }
@endphp
...
@endphp
    ...
    @endpush
    <li class="{{ VC::BCI }}">{{!empty($customer['name']) ? $customer['name'] : 'Undefined Customer' }}</li>
@endsection
@if(!empty($customer) && isset($customer->id))
@else
    <div class="{{ VC::TXT_MT }}">{{ __('Customer not found') }}</div>
@endif
@push(StacksConstants::ADM_SCR_PG)
...
@endphp
            <a
                ...
                data-guard-msg="{{ base64_encode($invoiceGuardMsg) }}"
                class="{{ VC::BT_SM_PM }}"
            >
                {{ __('Create Invoice') }}
            </a>
...
@endphp
            <a
                ...
                data-guard-msg="{{ base64_encode($createProposalGuardMsg) }}"
                class="{{ VC::BT_SM_PM }}"
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if(isset($pstatus) && is_numeric($pstatus) && $pstatus >= 0 && $pstatus <= 4 && isset(Proposal::$statuses[$pstatus]))
                                                <span class="badge {{ $statusBadgeClasses[$pstatus] ?? 'bg-secondary' }} p-2 {{ VC::PX3 }} rounded">{{ __(Proposal::$statuses[$pstatus]) }}</span>
                                            @else
...
@endphp
                                    <tr>
                                        <td>
                                            <a id="invoice-show-btn-{{ $iid ?? 'x' }}" href="{{ $showUrl }}" data-url="{{ $showUrl }}" data-guard-msg="{{ base64_encode($guardMsg) }}" class="{{ VC::BT_OUTPM }}">{{ $inum ? ($hasInvoiceNumber ? $user?->invoiceNumberFormat($inum) : __('Failed to format invoice number')) : __('No invoice number available') }}</a>
                                            @push(StacksConstants::ADM_SCR_PG)
                                        ...
                                        <td>
                                            @if($due)
                                                @php
 $dueStr = $hasDateFormat ? $user?->dateFormat($due) : __('Failed to format date');
@endphp
                                                @if($due < date('Y-m-d')) <span class="{{ VC::TX_DNG }}">{{ $dueStr }}</span> @else {{ $dueStr }} @endif
                                            @else
                                        ...
                                        <td><span class="badge {{ (isset($istatus) && isset(Invoice::$statuses[$istatus])) ? ($statusBadgeClasses[$istatus] ?? 'bg-secondary') : 'bg-secondary' }} p-2 {{ VC::PX3 }} rounded">{{ (isset($istatus) && isset(Invoice::$statuses[$istatus])) ? __(Invoice::$statuses[$istatus]) : __('Unknown status') }}</span></td>
                                        @if(Gate::check('edit invoice') || Gate::check('delete invoice') || Gate::check('show invoice'))
