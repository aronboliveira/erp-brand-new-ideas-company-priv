...
@if(isset($lead) && !empty($lead))
    @if(isset($call))
        {{ Form::model($call, array('route' => array(ViewsConstants::LD.'.calls.update', $lead->id, $call->id), 'method' => 'PUT')) }}
    @else
        {{ Form::open(array('route' => [ViewsConstants::LD.'.calls.store',$lead->id])) }}
    @endif
