...
$meta_title     ??= '';
$meta_desc      ??= '';
$themeCSS       ??= (function () use (&$color) {$color ??= '#4b4b4b';
...
try {$docLang = str_replace('_', '-', is_string($lang) ? $lang : (is_string(app()->getLocale()) ? app()->getLocale() : DC::DEFAULT_LANG));} catch (\InvalidArgumentException $e) {Log::warning('DocLang InvalidArgument: ' . get_class($e) . ' | "' . $e->getMessage() . '" | file=' . __FILE__ . ' | line=' . __LINE__);} catch (\Exception $e) {Log::error('DocLang Exception: ' . get_class($e) . ' | "' . $e->getMessage() . '" | file=' . __FILE__ . ' | line=' . __LINE__);} catch (\Throwable $e) {Log::critical('DocLang Throwable: ' . get_class($e) . ' | "' . $e->getMessage() . '" | file=' . __FILE__ . ' | line=' . __LINE__);}

if (!isset($purchase) || empty($purchase)) {echo TemplateHelper::getNoDataHtml('purchase', $docLang);
    return;}

try {$settings_data = Utility::settingsById(data_get($purchase, DC::COL_TABLE_CREATOR));} catch (\InvalidArgumentException $e) {Log::warning('settingsById InvalidArgument: ' . get_class($e) . ' | "' . $e->getMessage() . '" | file=' . __FILE__ . ' | line=' . __LINE__);
...
$dir = '';
try {$dir = (data_get($settings_data, SC::RTL) === 'on') ? 'rtl' : '';} catch (\InvalidArgumentException $e) {Log::warning('RTL InvalidArgument: ' . get_class($e) . ' | "' . $e->getMessage() . '" | file=' . __FILE__ . ' | line=' . __LINE__);} catch (\Exception $e) {Log::error('RTL Exception: ' . get_class($e) . ' | "' . $e->getMessage() . '" | file=' . __FILE__ . ' | line=' . __LINE__);} catch (\Throwable $e) {Log::critical('RTL Throwable: ' . get_class($e) . ' | "' . $e->getMessage() . '" | file=' . __FILE__ . ' | line=' . __LINE__);}
...
try {$base = 'purchase.link.copy';
    ...
    if ($qrValue === '#') Log::warning('QR route unavailable or param missing | route=' . ($resolved ?? 'null'));} catch (\InvalidArgumentException $e) {Log::warning('QR Route InvalidArgument: ' . get_class($e) . ' | "' . $e->getMessage() . '" | file=' . __FILE__ . ' | line=' . __LINE__);
...
<head>
    ...
    <style type="text/css">
        ...
        * {margin: 0;
        ...
        .purchase-footer {padding: 15px 0;}
            ...
            <table class="vertical-align-top">
                <tbody>
                    <tr>
                        <td>
                            <p>
                                <?= !empty($settings['company_name'])      ? e($settings['company_name']) . '<br>'        : '' ?>
                                <?= !empty($settings['mail_from_address']) ? e($settings['mail_from_address']) . '<br><br>' : '' ?>
                                <?= !empty($settings['company_address'])   ? e($settings['company_address']) : '' ?>
                                    ...
                                    <tr>
                                        <td colspan="2">
                                            <div class="view-qrcode">
                                                <?php
                                                try {echo (new \Milon\Barcode\DNS2D)->getBarcodeHTML($qrValue, 'QRCODE', 2, 2);} catch (\InvalidArgumentException $e) {Log::warning('QR HTML InvalidArgument: ' . get_class($e) . ' | "' . $e->getMessage() . '"');
                                                    echo '<div></div>';} catch (\Exception $e) {Log::error('QR HTML Exception: ' . get_class($e) . ' | "' . $e->getMessage() . '"');
                                                    echo '<div></div>';} catch (\Throwable $e) {Log::critical('QR HTML Throwable: ' . get_class($e) . ' | "' . $e->getMessage() . '"');
                                                    echo '<div></div>';}
                                                ?>
                                            </div>
                                        </td>
                        ...
                        <td colspan="2" class="sub-total">
                            <table class="total-table">
                                ...
                                <tr>
                                    <td><?= e(__('Discount')) ?>:</td>
                                    <td><?php
                                        try {echo e(Utility::priceFormat($settings, method_exists($purchase, 'getTotalDiscount') ? $purchase->getTotalDiscount() : 0));} catch (\InvalidArgumentException $e) {Log::warning('tot disc fmt: ' . $e->getMessage());
