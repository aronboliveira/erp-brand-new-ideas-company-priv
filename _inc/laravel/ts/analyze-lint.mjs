...
for (const f of report) {
  ...
  for (const m of f.messages) {
    if (m.severity === 2) errCnt++;
    else warnCnt++;
    const r = m.ruleId || "(parse-error)";
    if (!byRule[r])
      byRule[r] = { total: 0, fixable: 0, errors: 0, warnings: 0 };
    byRule[r].total++;
    if (m.severity === 2) byRule[r].errors++;
    else byRule[r].warnings++;
...
for (const [rule, s] of sorted) {
  console.log(
    rule.padEnd(55) +
      ...
      String(s.warnings).padStart(7) +
      String(s.fixable).padStart(6),
  );
}
