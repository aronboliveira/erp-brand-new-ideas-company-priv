...
class FinanceCompensationService
{
    ...
    public function markRequired(OutboxMessage $message, string $reason, array $context = []): ?OperationLedger
    {
        ...
        $this->operations->recordStep($ledger, 'compensation.required:' . $message->id, 'Finance compensation required', [
            'step_type' => 'cleanup',
            'sequence' => 900,
        ...
        $this->events->record('finance.compensation.required', 'Finance operation needs compensation', [
            'reason' => $reason,
            'message_key' => $message->message_key,
            'event_type' => $message->event_type,
    ...
    public function markCompensated(OutboxMessage $message, string $summary, array $result = []): ?OperationLedger
    {
        $ledger = $message->operationLedger;
        if (!$ledger) {
            return null;
        ...
        $this->operations->recordStep($ledger, 'compensation.completed:' . $message->id, 'Finance compensation completed', [
            'step_type' => 'cleanup',
            'sequence' => 910,
        ...
        $this->events->record('finance.compensation.completed', $summary, $result, $this->eventOptions($ledger, $message));
