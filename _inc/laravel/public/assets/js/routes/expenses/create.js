  ...
  try {
    ...
    const fetchDetail = async (selectEl, detailSel) => {
      try {
        ...
        detail.classList.toggle("d-none", !(data?.html && String(data.html).trim().length));
      } catch (err) {
        console.error(`[create] Error:`, err);
      }
    };
      ...
      qsa("tbody[data-repeater-item]").forEach(tbody => {
        ...
        if (row1) {
          const qty = toNum(qs(".quantity", row1)?.value),
            price = toNum(qs(".price", row1)?.value),
            disc = toNum(qs(".discount", row1)?.value),
            taxRate = toNum(qs(".itemTaxRate", row1)?.value),
            line = qty * price,
        ...
        itemSel.addEventListener("change", async () => {
          ...
          try {
            ...
            const unit = qs(".unit", tbody),
              price = qs(".price", tbody),
              taxesBox = qs(".taxes", tbody),
              taxRate = qs(".itemTaxRate", tbody),
              taxPrice = qs(".itemTaxPrice", tbody);
