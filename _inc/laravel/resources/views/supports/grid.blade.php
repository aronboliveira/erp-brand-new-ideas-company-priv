...
$user = Auth::user();
        ...
        \Log::error('supports/grid — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
    }
@endphp
...
@section(YieldingConstants::ADM_BDC)
    ...
    <li class="{{ VC::BCI_ACT }}" aria-current="page">{{__('Support')}}</li>
@endsection
...
@endphp
		...
		@empty
			<div class="{{ VC::CM12 }}"><p class="{{ VC::TXCT_MT }}">{{ __('No supports available') }}</p></div>
		@endforelse
	</div>
@endsection
