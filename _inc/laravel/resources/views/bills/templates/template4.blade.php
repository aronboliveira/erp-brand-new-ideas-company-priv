...
<head>
    <?php try {echo view('fragments.std', ['meta_title' => $meta_title, 'meta_desc' => $meta_desc])->render();} catch (\InvalidArgumentException $e) {Log::warning('Meta view: ' . $e->getMessage());} catch (\Exception $e) {Log::error('Meta view: ' . $e->getMessage());} catch (\Throwable $e) {Log::critical('Meta view: ' . $e->getMessage());} ?>
...
<body>
                                    ...
                                    <tr>
                                        <td colspan="2">
                                            <div class="view-qrcode">
                                                <?php
                                                try {$enc = data_get($bill, 'bill_id') ? Crypt::encrypt($bill->bill_id) : null;
                                                    $route = $enc ? route(VW::BIL . '.link.copy', $enc) : '#';
                                                    echo (new \Milon\Barcode\DNS2D)->getBarcodeHTML($route, 'QRCODE', 2, 2);} catch (\InvalidArgumentException $e) {Log::warning('QR: ' . $e->getMessage());
                                                    echo '<div></div>';} catch (\Exception $e) {Log::error('QR: ' . $e->getMessage());
                                                    echo '<div></div>';} catch (\Throwable $e) {Log::critical('QR: ' . $e->getMessage());
                                                    echo '<div></div>';}
                                                ?>
                                            </div>
                                        </td>
                ...
                <tbody style="border-bottom:1px solid <?= e($color) ?>">
                    <?php if (isset($bill->itemData) && is_iterable($bill->itemData) && count($bill->itemData) > 0): ?>
                        <?php foreach ($bill->itemData as $item): ?>
                            ...
                            $unitName = '';
                            try {$u = ProductServiceUnit::find(data_get($item, 'unit'));
                                $unitName = data_get($u, 'name', '');} catch (\InvalidArgumentException $e) {Log::warning('Unit: ' . $e->getMessage());} catch (\Exception $e) {Log::error('Unit: ' . $e->getMessage());} catch (\Throwable $e) {Log::critical('Unit: ' . $e->getMessage());}
                            $itemtax = 0.0;
                            ?>
                            <tr>
                                ...
                                <td><?php try {echo e(Utility::priceFormat($settings, ($price * $qty) - $disc + $itemtax));} catch (\InvalidArgumentException $e) {Log::warning('Line total: ' . $e->getMessage());
                ...
                <tfoot>
                    <tr style="border-bottom:1px solid <?= e($color) ?>">
                        ...
                        <td><?php try {echo e(Utility::priceFormat($settings, data_get($bill, 'totalTaxPrice', 0)));} catch (\InvalidArgumentException $e) {Log::warning('totalTaxPrice: ' . $e->getMessage());
                                ...
                                echo '0';} ?></td>
                        <td><?php try {echo e(Utility::priceFormat($settings, method_exists($bill, 'getSubTotal') ? $bill->getSubTotal() : 0));} catch (\InvalidArgumentException $e) {Log::warning('subtotal row: ' . $e->getMessage());
                                echo '0';} catch (\Exception $e) {Log::error('subtotal row: ' . $e->getMessage());
                                echo '0';} catch (\Throwable $e) {Log::critical('subtotal row: ' . $e->getMessage());
                                echo '0';} ?></td>
                    </tr>
                    <tr style="border-bottom:1px solid <?= e($color) ?>">
                        <td colspan="4"></td>
                        <td colspan="2" class="sub-total">
                            <table class="total-table">
                                ...
                                <?php if (method_exists($bill, 'getTotalDiscount') && $bill->getTotalDiscount()): ?>
                                    <tr style="border-bottom:1px solid <?= e($color) ?>">
                                        <td><?= e(__('Discount')) ?>:</td>
                                        <td><?php try {echo e(Utility::priceFormat($settings, $bill->getTotalDiscount()));} catch (\InvalidArgumentException $e) {Log::warning('TotDisc: ' . $e->getMessage());
                                                echo '0';} catch (\Exception $e) {Log::error('TotDisc: ' . $e->getMessage());
                                                echo '0';} catch (\Throwable $e) {Log::critical('TotDisc: ' . $e->getMessage());
                                                echo '0';} ?></td>
                                    </tr>
                                <?php endif; ?>
                                ...
                                <tr style="border-bottom:1px solid <?= e($color) ?>">
                                    <td><?= e(__('Total')) ?>:</td>
                                    <td><?php try {echo e(Utility::priceFormat($settings, (method_exists($bill, 'getSubTotal') ? $bill->getSubTotal() : 0) - (method_exists($bill, 'getTotalDiscount') ? $bill->getTotalDiscount() : 0) + (method_exists($bill, 'getTotalTax') ? $bill->getTotalTax() : 0)));} catch (\InvalidArgumentException $e) {Log::warning('Grand total: ' . $e->getMessage());
                                ...
                                <tr style="border-bottom:1px solid <?= e($color) ?>">
                                    <td><?= e(__('Paid')) ?>:</td>
                                    <td><?php try {echo e(Utility::priceFormat($settings, (method_exists($bill, 'getTotal') ? $bill->getTotal() : 0) - (method_exists($bill, 'getDue') ? $bill->getDue() : 0) - (method_exists($bill, 'billTotalDebitNote') ? $bill->billTotalDebitNote() : 0)));} catch (\InvalidArgumentException $e) {Log::warning('Paid: ' . $e->getMessage());
                                ...
                                <tr style="border-bottom:1px solid <?= e($color) ?>">
                                    ...
                                    <td><?php try {echo e(Utility::priceFormat($settings, method_exists($bill, 'billTotalDebitNote') ? $bill->billTotalDebitNote() : 0));} catch (\InvalidArgumentException $e) {Log::warning('Debit note: ' . $e->getMessage());
                                            echo '0';} catch (\Exception $e) {Log::error('Debit note: ' . $e->getMessage());
                                            echo '0';} catch (\Throwable $e) {Log::critical('Debit note: ' . $e->getMessage());
                                            echo '0';} ?></td>
                                </tr>
                                <tr>
                                    <td><?= e(__('Due Amount')) ?>:</td>
                                    <td><?php try {echo e(Utility::priceFormat($settings, method_exists($bill, 'getDue') ? $bill->getDue() : 0));} catch (\InvalidArgumentException $e) {Log::warning('Due: ' . $e->getMessage());
    ...
    <?php if (!isset($preview)): ?>
        <?php try {echo view(VW::BIL . '.script')->render();} catch (\InvalidArgumentException $e) {Log::warning('Script include: ' . $e->getMessage());} catch (\Exception $e) {Log::error('Script include: ' . $e->getMessage());} catch (\Throwable $e) {Log::critical('Script include: ' . $e->getMessage());} ?>
    <?php endif; ?>
</body>
