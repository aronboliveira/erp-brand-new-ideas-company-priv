...
$user = Auth::user();
        ...
        \Log::error('reports/pos_vs_purchase — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
    }
    $lang ??= 'en';
    $currentYear ??= date('Y');
    $monthList ??= [];
...
@section(YieldingConstants::ADM_BDC)
    ...
    <li class="{{ VC::BCI }}">{{__('POS Vs Purchase')}}</li>
@endsection
...
@endphp
        ...
        data-guard-msg="{{ base64_encode($downloadGuardMsgPvp) }}"
...
@endphp
                                    ...
                                    <tr>
                                        <td colspan="{{ 1 + count($months) }}" class="{{ VC::TX_DK }}">
