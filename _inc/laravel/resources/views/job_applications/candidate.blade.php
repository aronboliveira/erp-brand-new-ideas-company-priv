...
$user = Auth::user();
        ...
        \Log::error('job_applications/candidate — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
    }
@endphp
...
@section(YieldingConstants::ADM_BDC)
    ...
    <li class="{{ VC::BCI }}">{{ __('Archive Application') }}</li>
@endsection
...
@endphp
                                                    <div class="{{ VC::DFL }}">
                                                        ...
                                                        <div class="{{ VC::ACT_BTN_INF }}">
                                                            <a
                                                                ...
                                                                title="{{ __('Preview') }}"
                                                                data-url="{{ $resumeUrl }}"
                                                                data-guard-msg="{{ base64_encode($previewGuardMsg) }}"
                                                                data-sv-localized="true"
                                                                {{ $resumeUrl === '#' ? 'aria-disabled=true' : '' }}
                                                            >
                                ...
                                @else
                                    <tr>
                                        <td colspan="6" class="{{ VC::TXCT }}">{{ __('No archived applications found.') }}</td>
                                    </tr>
                                @endif
                            </tbody>
                        </table>
