...
trait IsCardNote
{
	...
	protected function addCardNoteColumns(Blueprint $table, bool $nullableCustomer = false, $onDeleteCustomer = 'restrict', bool $nullableReconcile = true, bool $nullableInvoice = true, bool $nullableAcc = true, bool $nullableCat = true, $onDeleteAcc = 'set null', $onDeleteCat = 'set null', $billIdentifier = BC::COL_BL_ID): void
	{
	    try {
    		...
    		$this->addPaymentConclusionColumns($table, nullableAcc: $nullableAcc, nullableCat: $nullableCat, onDeleteAcc: $onDeleteAcc, onDeleteCat: $onDeleteCat);
    		$table->uuid($billIdentifier)->nullable()->index();
	        ...
	        Log::error(static::class . '::addCardNoteColumns — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
	    }
	}
	protected function dropCardNoteColumnForeigns(Blueprint $table, string $tableName): void
	{
	    try {
	        ...
	        Log::error(static::class . '::dropCardNoteColumnForeigns — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
	    }
	}
}
