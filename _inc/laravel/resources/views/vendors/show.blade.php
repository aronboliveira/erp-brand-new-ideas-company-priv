...
$user = Auth::user();
        ...
        \Log::error('vendors/show — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
    }
@endphp
...
@section(YW::ADM_BDC)
    ...
    <li class="{{ VC::BCI }}">
        <a id="{{ $vndIndexId }}"
           ...
           data-guard-msg="{{ base64_encode($vndIndexGuard) }}"
           data-sv-localized="true"
           {{ $vndIndexUrl === '#' ? 'aria-disabled=true' : '' }}>
            {{ __('Vendor') }}
        </a>
    </li>
    <li class="{{ VC::BCI }}">{{ $vName }}</li>
@endsection
...
@endphp
            <a id="{{ $crId }}"
               href="{{ $crUrl }}"
               data-url="{{ $crUrl }}"
               data-guard-msg="{{ base64_encode($crMsg) }}"
               data-sv-localized="true"
               class="{{ VC::BT_SM_PM }}">
                {{ __('Create Bill') }}
            </a>
...
@endphp
            <a id="{{ $edId }}"
               ...
               data-guard-msg="{{ base64_encode($edMsg) }}"
               data-sv-localized="true"
               title="{{ __('Edit') }}"
               class="{{ VC::BT_SM_PM }}">
...
@endphp
                                    <tr>
                                        ...
                                        <td>
                                            @php
 $isOver = ($dueDate !== '' && $dueDate < now()->toDateString());
@endphp
                                            @if($isOver)
                                                <span class="{{ VC::TX_DNG }}">{{ $dueDateFmt }}</span>
                                            @else
                                                {{ $dueDateFmt }}
                                            @endif
                                        </td>
...
@endphp
                                                    ...
                                                    <a id="{{ $dupBtnId }}"
                                                       ...
                                                       data-confirm-yes="document.getElementById('{{ $dupFormId }}').submit();">
                                                        <i class="{{ VC::TI_COPY }} text-success"></i>
                                                    </a>
                                                    @push(ST::ADM_SCR_PG)
                                                        <script defer>
                                                            (() => {
...
@endphp
                                                    ...
                                                    @endpush
                                                    <a id="{{ $showId }}" href="{{ $showUrl }}" data-url="{{ $showUrl }}" data-guard-msg="{{ base64_encode($showMsg) }}" data-sv-localized="true" class="me-2">
...
@endphp
                                                    ...
                                                    @endpush
                                                    <a id="{{ $editId }}" href="{{ $editUrl }}" data-url="{{ $editUrl }}" data-guard-msg="{{ base64_encode($editMsg) }}" data-sv-localized="true" class="me-2">
...
@endphp
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="{{ VC::TXCT_MT }}">{{ __('No bills available') }}</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
