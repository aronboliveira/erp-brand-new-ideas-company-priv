...
onDelegate("click", '[data-ajax-popup="true"]', function (e) {
    ...
    if (vcHidden)
        data.vc_name = vcHidden.value;
    const whHidden = byId("warehouse_name_hidden");
    if (whHidden)
        data.warehouse_name = whHidden.value;
    const discHidden = byId("discount_hidden");
    if (discHidden)
        data.discount = discHidden.value;
...
onDelegate("click", ".plus, .minus", function () {
    ...
    if (this.classList.contains("plus")) {
        if (!isNaN(max) && val >= max)
            ...
            input.value = (val + stepVal).toFixed(step.getDecimals());
    }
    else {
...
function JsSearchBox() {
    if (typeof $ === "undefined" || !qs(".js-searchBox"))
        return;
