...
case "${1:-basic}" in
    ...
    results|summary)
        ...
        if [ -f "$RESULTS_DIR/smoke_test_summary.txt" ]; then
            less "$RESULTS_DIR/smoke_test_summary.txt"
        else
    ...
    results-auth)
        ...
        if [ -f "$RESULTS_DIR/smoke_test_authenticated_summary.txt" ]; then
            less "$RESULTS_DIR/smoke_test_authenticated_summary.txt"
        else
    ...
    analysis)
        ...
        if [ -f "$RESULTS_DIR/routes_smoke_test_analysis.md" ]; then
            less "$RESULTS_DIR/routes_smoke_test_analysis.md"
        else
    ...
    clean)
        ...
        rm -f "$RESULTS_DIR"/smoke_test*.txt
        rm -f "$RESULTS_DIR"/smoke_test*.json
...
RESULTS LOCATION:
    Persistent: utils/.llms/smoke_test_*.{json,txt,md}
