...
class ScreenshotsControllerTest extends TestCase
{
	...
	public function index_displays_screenshots_for_super_admin()
	{
		...
		$response->assertStatus(200)
			...
			->assertViewHas('screenshots', function ($screenshots) {
				return is_array($screenshots) && $screenshots[0]['screenshots_heading'] === 'First';
			})
	...
	public function screenshots_store_adds_new_entry_with_heading()
	{
		...
		$response->assertRedirect()
			->assertSessionHas('success', 'Screenshots add successfully');

		$setting = LandingPageSetting::where('name', 'screenshots')->first();
		$items = json_decode($setting->value, true);
		$this->assertCount(1, $items);
		$this->assertEquals('Screenshot One', $items[0]['screenshots_heading']);
		$this->assertArrayNotHasKey('screenshots', $items[0]);
	}
	...
	public function screenshots_edit_displays_edit_form()
	{
		...
		$response->assertStatus(200)
			->assertViewIs('landingpage::landingpage.screenshots.edit')
			->assertViewHas('screenshot', $initial[0])
			->assertViewHas('key', 0);
	}
	...
	public function screenshots_update_modifies_entry_heading()
	{
		...
		$response = $this->actingAs($user)
			->put(action([ScreenshotsController::class, 'screenshotsUpdate'], ['key' => 0]), $payload);
	...
	public function screenshots_delete_removes_entry()
	{
		...
		$response = $this->actingAs($user)
			->delete(action([ScreenshotsController::class, 'screenshotsDelete'], ['key' => 0]));
	...
	public function update_modifies_single_setting_value()
	{
		...
		$response = $this->actingAs($user)
			->put(action([ScreenshotsController::class, 'update'], ['id' => $setting->id]), $payload);
	...
	public function destroy_deletes_single_setting()
	{
		...
		$response = $this->actingAs($user)
			->delete(action([ScreenshotsController::class, 'destroy'], ['id' => $setting->id]));
