...
(() => {
    try {
        ...
        f.addEventListener("submit", (e) => {
            try {
                const action = (f.getAttribute("action") ?? "").trim(), url = (f.getAttribute("data-url") ?? "").trim() || action;
                if (action !== "#" || url !== "#")
                    return;
                e.preventDefault();
                ...
                const hasBootstrapCss = !!document.querySelector('link[href*="bootstrap"]'), hasBootstrapJs = typeof window.bootstrap !== "undefined";
