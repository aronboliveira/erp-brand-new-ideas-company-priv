@php
    try {
        ...
        if (empty($lang)) $lang = app()->getLocale() ?: DatabaseConstants::DEFAULT_LANG;
                $langName = cache()->remember('full_language_data_' . $lang . '_' . ($user?->id ?? 'guest'), now()->addMinutes(30), function () use ($lang) {
            return \App\Models\Language::languageData($lang);
        });
        if (!$langName || empty($langName->full_name))
        ...
        \Log::error('partials/admin/header — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
    }
@endphp
                    ...
                    <li class="{{ ViewClassNamesConstants::DRP_DSH }} drp-company">
                        @if ($user instanceof User)
                            <a class="{{ ViewClassNamesConstants::DRP_NO_ARROW }}" data-bs-toggle="dropdown" href="#" role="button" aria-haspopup="false" aria-expanded="false">
                                ...
                                <span class="hide-mob {{ VC::MS2 }}">{{__('Hi, ')}}{{$user?->name }}!</span>
                                <i class="ti ti-chevron-down drp-arrow nocolor hide-mob"></i>
                            </a>
                        @endif
                        @php
                            try {
                                $profileRoute = Route::has('user.profile')
                                    ? route('user.profile')
...
@endphp
                        <div class="{{ ViewClassNamesConstants::DRP_DSH_MN }}">
                            <a
                                ...
                                data-guard-msg="{{ base64_encode($profileMsg) }}"
                            >
                                <i class="{{ VC::TI_USR }} {{ VC::TX_DK }}"></i>
                            ...
                            >
                                <i class="ti ti-power {{ VC::TX_DK }}"></i>
                                <span>{{ __('Logout') }}</span>
                            </a>
                        ...
                        <div class="{{ ViewClassNamesConstants::DRP_MN_DSH_END }}">
                            @foreach ($changeLinks as $link)
                                <a
                                    id="{{ $link['id'] }}"
                                    href="{{ $link['route'] }}"
                                    class="{{ VC::DRP_IT }} {{ $lang === $link['code'] ? 'text-primary' : '' }}"
                                    data-url="{{ $link['route'] }}"
                                    data-sv-localized="true"
                                    data-guard-msg="{{ base64_encode($link['message']) }}"
                                >
                            ...
                            @if (!empty($createLink))
                                <a
                                    ...
                                    class="{{ VC::DRP_IT }} {{ VC::TX_PM }}"
                                    data-ajax-popup="true"
                                    data-title="{{ __('Create New Language') }}"
                                    data-url="{{ $createLink['createRoute'] }}"
                                    data-sv-localized="true"
                                    data-guard-msg="{{ base64_encode($createLink['createMsg']) }}"
                                >
                                    {{ __('Create Language') }}
                                </a>
                            @endif
                            ...
                            @if (!empty($manageLink))
                                <a
                                    ...
                                    data-sv-localized="true"
                                    data-guard-msg="{{ base64_encode($manageLink['manageMsg']) }}"
                                >
                                    {{ __('Manage Language') }}
                                </a>
                            @endif
