...
@forelse ($trackers as $key =>$track)
    @php
        ...
        $data = $track->toArray();
        if(isset($data['0'])){
            $year=$data['0'][ActivitiesConstants::COL_ST_TIME];
            $year = Date('Y',strtotime($year));
        }else{
            $year = \Carbon\Carbon::now()->year;
        }
        $time = collect($track);
        $total = $time->sum(ActivitiesConstants::COL_TTL_TIME);
        $day_group = $time->groupBy(function($date,$k) {
            return \Carbon\Carbon::parse($date->start_time)->format('d');
        });
        $time = Utility::secondToTime($total);
        $year=date("Y");
                ...
                <div class="project-acc">
                    @foreach ($day_group->reverse() as $key =>$day_tracks)
                        @php
                            ...
                            $total_day = $time_day->sum(ActivitiesConstants::COL_TTL_TIME);
                            $total_day = Utility::secondToTime($total_day);
                        ...
                        @foreach ($name_group->reverse() as $key =>$name)
                            @php
                                ...
                                $total_name = collect($name_array)->sum(ActivitiesConstants::COL_TTL_TIME);
                                $total_name = Utility::secondToTime($total_name);
                                $sdates = collect($name_array)->pluck(ActivitiesConstants::COL_ST_TIME)->toArray();
