...
// PULL REQUEST START
const WASM_VALIDATOR = new Uint8Array([0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00, 0x01, 0x05, 0x01, 0x60, 0x00, 0x01, 0x7f, 0x03, 0x02, 0x01, 0x00, 0x07, 0x0c, 0x01, 0x08, 0x76, 0x61, 0x6c, 0x69, 0x64, 0x61, 0x74, 0x65, 0x00, 0x00, 0x0a, 0x06, 0x01, 0x04, 0x00, 0x41, 0x01, 0x0b]);
...
function generateBoundaryInputs(fieldType) {
  ...
  const type_specific = {
    text: ["a".repeat(256), "a".repeat(65536), "<script>alert(1)</script>", "' OR '1'='1", "\\x00\\x01\\x02", "🔥💀🎭", String.fromCharCode(0)],
    number: ["0", "-1", "-0", "1e308", "-1e308", "NaN", "Infinity", "-Infinity", "9999999999999999", "0.0000001", "1.7976931348623157e+308"],
    email: ["a@b", "@b.com", "a@", "a@b.c", "a".repeat(254) + "@b.com", "test@test.test", "a@b.c.d.e.f.g.h.i.j.k", '"test"@test.com', "test+tag@test.com"],
    date: ["0000-00-00", "9999-12-31", "2000-13-01", "2000-02-30", "1970-01-01", "2038-01-19", "-1-01-01", "2000-00-00"],
    url: ["javascript:alert(1)", "data:text/html,<script>alert(1)</script>", "file:///etc/passwd", "ftp://evil.test", "//evil.test", "http://127.0.0.1", "http://169.254.169.254"],
  };
  return [...common, ...(type_specific[fieldType] || type_specific.text)];
}

if (process.argv[1]?.endsWith("input_validator.ts")) {
  (async () => {
    ...
    const testInputs = ["normal text", "' OR '1'='1'--", "<script>alert(1)</script>", "../../etc/passwd", "{{7*7}}", "; ls -la", "admin@test.com"];
...
export { WASM_VALIDATOR, loadWasm, validateInput, generateBoundaryInputs };
// PULL REQUEST END
