...
if (isset($proposal) && !empty($proposal)) {if (!function_exists('e')) {
    ...
    try {$proposalTotalQuantity = (string)(data_get($proposal, 'totalQuantity') ?? '0');
        ...
        $proposalGrandTotal = Utility::priceFormat($settings, (data_get($proposal, 'getSubTotal') ? $proposal->getSubTotal() : 0) - (data_get($proposal, 'getTotalDiscount') ? $proposal->getTotalDiscount() : 0) + (data_get($proposal, 'getTotalTax') ? $proposal->getTotalTax() : 0)) ?: '0';} catch (\InvalidArgumentException $e) {Log::warning('Totals InvalidArgument: ' . get_class($e) . ' | "' . $e->getMessage() . '" | file=' . __FILE__ . ' | line=' . __LINE__);
        $proposalTotalQuantity = '0';
        $proposalTotalRate = '0';
        $proposalTotalDiscount = '0';
        $proposalTotalTaxPrice = '0';
...
?>
    ...
    <body class="">
        <div class="proposal-preview-main" id="boxes">
            <div class="proposal-header" style="background: <?= e($color) ?>;color:<?= e($font_color) ?>">
                <table>
                    <tbody>
                        <tr>
                ...
                <table class="vertical-align-top">
                    <tbody>
                        <tr>
                            <td>
                                <p>
                                    <?php if (!empty($settings['company_name'])) {echo e($settings['company_name']);} else {echo e(__('No company name available'));} ?><br>
                                    <?php if (!empty($settings['mail_from_address'])) {echo e($settings['mail_from_address']);} else {echo e(__('No email available'));} ?><br><br><br>
                                    <?php if (!empty($settings['company_address'])) {echo e($settings['company_address']);} else {echo e(__('No address available'));} ?>
                                    <?php if (!empty($settings['company_city'])) {echo '<br>' . e($settings['company_city']) . ', ';} ?>
                                    <?php if (!empty($settings['company_state'])) {echo e($settings['company_state']);} ?>
                                    <?php if (!empty($settings['company_zipcode'])) {echo ' - ' . e($settings['company_zipcode']);} ?>
                                    <?php if (!empty($settings['company_country'])) {echo '<br>' . e($settings['company_country']);} ?>
                                    <?php if (!empty($settings['company_telephone'])) {echo e($settings['company_telephone']);}
                                    echo '<br>'; ?>
                                    <?php if (!empty($settings['registration_number'])) {echo e(__('Registration Number')) . ' : ' . e($settings['registration_number']);}
                                    echo '<br>'; ?>
