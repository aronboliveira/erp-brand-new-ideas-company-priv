...
$lang = Utility::fetchUserLang();
        ...
        \Log::error('product_services/create — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
    }
@endphp
...
@endphp
        @if($plan?->{\App\Config\Constants\PlansConstants::COL_GPT} == 1)
            <div class="{{ VC::TX_END }}">
                <a href="#"
                   class="{{ VC::BT_PRM }} btn-icon btn-sm ai-btn"
                   data-size="md"
        ...
        <div class="{{ VC::RW }}">
            <div class="{{ VC::FM_GCB6 }}">
                ...
                @error('name')<span id="name-error" class="{{ VC::INV_FB }} {{ VC::DBL }}" role="alert"><strong class="{{ VC::TX_DNG }}">{{ $message }}</strong></span>@enderror
            </div>
            <div class="{{ VC::FM_GCB6 }}">
                {{ Form::label('sku', __('SKU'), ['class' => VC::FM_LB]) }}<span class="{{ VC::TX_DNG }}">*</span>
                {{ Form::text('sku', null, $skuAttrs) }}
                @error('sku')<span id="sku-error" class="{{ VC::INV_FB }} {{ VC::DBL }}" role="alert"><strong class="{{ VC::TX_DNG }}">{{ $message }}</strong></span>@enderror
            </div>
            <div class="{{ VC::FM_GCB6 }}">
                {{ Form::label('sale_price', __('Sale Price'), ['class' => VC::FM_LB]) }}<span class="{{ VC::TX_DNG }}">*</span>
            ...
            <div class="{{ VC::FM_GCB6 }}">
                {{ Form::label('purchase_price', __('Purchase Price'), ['class' => VC::FM_LB]) }}<span class="{{ VC::TX_DNG }}">*</span>
            ...
            <div class="{{ VC::FM_GCB6 }}">
                {{ Form::label('category_id', __('Category'), ['class' => VC::FM_LB]) }}<span class="{{ VC::TX_DNG }}">*</span>
            ...
            <div class="{{ VC::FM_GCB6 }}">
                {{ Form::label('unit_id', __('Unit'), ['class' => VC::FM_LB]) }}<span class="{{ VC::TX_DNG }}">*</span>
            ...
            <div class="{{ VC::FM_GCB6 }}">
                ...
                <div class="choose-file">
                    ...
                    <img id="pro_image_preview" class="{{ VC::MT3 }}" style="width:25%;"/>
                </div>
            </div>
            <div class="{{ VC::FM_GCB6 }}">
                ...
                <div class="{{ VC::RW }}">
                    <div class="{{ VC::CM6 }}">
                        <div class="{{ VC::FM_CHK_IL }}">
                            ...
                            <label class="{{ VC::CST_LB }} {{ VC::FM_LB }}" for="type-product">{{ __('Product') }}</label>
                        </div>
                    </div>
                    <div class="{{ VC::CM6 }}">
                        <div class="{{ VC::FM_CHK_IL }}">
                            <input type="radio" class="form-check-input type" id="type-service" name="type" value="service">
                            <label class="{{ VC::CST_LB }} {{ VC::FM_LB }}" for="type-service">{{ __('Service') }}</label>
                        </div>
                    </div>
                </div>
            </div>
            ...
            @php
 $customFieldsIsList = ($customFields ?? null) instanceof Collection && $customFields->isNotEmpty();
@endphp
