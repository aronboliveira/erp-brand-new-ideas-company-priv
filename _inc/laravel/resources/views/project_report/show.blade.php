                                        ...
                                        <tbody>
                                            @foreach($project->users as $user)
                                                @php
                                                    ...
                                                    $total_user_task = App\Models\ProjectTask::where(ProjectsConstants::COL_PJ_ID,$project->id)
                                                        ->whereRaw("FIND_IN_SET(?,  ".ProjectsConstants::COL_ASGN.") > 0", [$user?->id])->get()->count();

                                                    $all_task = App\Models\ProjectTask::where(ProjectsConstants::COL_PJ_ID,$project->id)
                                                        ->whereRaw("FIND_IN_SET(?,  ".ProjectsConstants::COL_ASGN.") > 0", [$user?->id])->get();

                                                    $total_complete_task = App\Models\ProjectTask::where(ProjectsConstants::COL_PJ_ID,'=',$project->id)->where('stage_id',$lastStage->id)
                                                    ->where(ProjectsConstants::COL_ASGN,'=',$user?->id)->count();

                                                    $logged_hours = 0;
                                                    $timesheets = App\Models\Timesheet::where(ProjectsConstants::COL_PJ_ID,$project->id)
                                                    ->where(DatabaseConstants::TABLE_CREATOR ,$user?->id)->get();
                                                @endphp
                                    ...
                                    <table class="table" >
                                        ...
                                        <tbody>
                                            @foreach($project->milestones as $milestone)
                                                ...
                                                <td> @if($milestone[ProjectsConstants::COL_STAT] == ProjectsConstants::STT_CPT_K)
                                        ...
                                        <table class="table datatable">
                                            ...
                                            @foreach($tasks as $task)
                                                @php
                                                    ...
                                                    $total_user_task = App\Models\ProjectTask::where(ProjectsConstants::COL_PJ_ID,$project->id)->whereRaw("FIND_IN_SET(?,  ".ProjectsConstants::COL_ASGN.") > 0", [$user?->id])->get()->count();

                                                    $all_task = App\Models\ProjectTask::where(ProjectsConstants::COL_PJ_ID,$project->id)->whereRaw("FIND_IN_SET(?,  ".ProjectsConstants::COL_ASGN.") > 0", [$user?->id])->get();

                                                    $total_complete_task = App\Models\ProjectTask::join('task_stages','task_stages.id','=','project_tasks.stage_id')
                                                    ->where('task_stages.project_id','=',$project->id)->where('stage_id',4)->where(ProjectsConstants::COL_ASGN,'=',$user?->id)->get()->count();

                                                    $logged_hours = 0;
                                                    $timesheets = App\Models\Timesheet::where(ProjectsConstants::COL_PJ_ID,$project->id)->where('task_id' ,$task->id)->get();
                                                @endphp
                                                @foreach($timesheets as $timesheet)
