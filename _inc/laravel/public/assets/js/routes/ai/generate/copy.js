...
(function () {
    ...
    const setCheckedFirstRadio = () => {
        ...
        if (!modal)
            return;
        const first = qs("#commonModalOver input[type='radio']");
        if (first) {
            if (!first.checked) {
                first.checked = true;
    ...
    const copyText = () => {
        try {
            const selected = $('input[name="template_name"]:checked').attr("data-name") ?? "", copied = $("#ai-description").val() ?? "";
            if (selected === "") {
            ...
            if (window.show_toastr)
                window.show_toastr("success", "Result text has been copied successfully", "success");
    ...
    const copySelectedText = () => {
        try {
            const selected = $('input[name="template_name"]:checked').attr("data-name") ?? "", selText = (window.getSelection && window.getSelection()?.toString()) || "";
            if (selected === "") {
                scheduleInteractiveErrorClick(getMsg(document.body, "copy_unavailable"));
                return;
            }
    ...
    const bindGenerate = () => {
        ...
        $(document.body).on("click.aiGenerate", "#generate", function () {
            const el = this;
            const form = $("#myForm"), explicit = '{{ route("generate.response") }}', endpoint = resolveRoute(form.get(0), explicit);
            if (!endpoint) {
