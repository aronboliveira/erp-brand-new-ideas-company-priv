...
$user = Auth::user();
        ...
        \Log::error('customers/index — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
    }
@endphp
...
@section(YieldingConstants::ADM_BDC)
    ...
    <li class="{{ VC::BCI }}">{{__('Customer')}}</li>
@endsection
...
@endphp
    <div class="{{ VC::FEND }}">
        <a
            ...
            data-guard-msg="{{ base64_encode($importGuardMsg) }}"
            data-size="md"
...
@endphp
                                        <tr class="cust_tr" data-url="{{ $showHref }}" data-id="{{ $cid }}">
                                            <td>
                                                @can('show customer')
                                                    <a id="customer-show-btn-{{ $cid }}" href="{{ $showHref }}" data-url="{{ $showHref }}" data-guard-msg="{{ base64_encode($showGuardMsg) }}" class="{{ VC::BT_OUTPM }}">
                                                ...
                                                @else
                                                    <a id="customer-show-btn-{{ $cid }}" href="#" data-url="#" data-guard-msg="{{ base64_encode($showGuardMsg) }}" class="{{ VC::BT_OUTPM }}">
                                                        {{ $num !== null ? ($isCustomerNumberFormatAvailable ? $user?->customerNumberFormat($num) : __('Failed to format customer number')) : __('No customer number available') }}
                                                    </a>
                                                @endcan
                                            </td>
                                                ...
                                                @if($isActive)
                                                    @can('show customer')
                                                        <div class="{{ VC::ACT_BTN_INF }}">
                                                            <a id="customer-view-btn-{{ $cid }}" href="{{ $showHref }}" data-url="{{ $showHref }}" data-guard-msg="{{ base64_encode($showGuardMsg) }}" class="{{ VC::BT_SM_FL_CT }}" data-bs-toggle="tooltip" title="{{ __('View') }}">
...
@endphp
                                                        <div class="{{ VC::ACT_BTN_PRIM }}">
                                                            <a id="customer-edit-btn-{{ $cid }}" href="{{ $editHref }}" data-url="{{ $editHref }}" data-guard-msg="{{ base64_encode($editGuardMsg) }}" data-ajax-popup="true" data-size="lg" class="{{ VC::BT_SM_FL_CT }}" data-bs-toggle="tooltip" title="{{ __('Edit') }}" data-title="{{ __('Edit Customer') }}">
...
@endphp
                                                        <div class="{{ VC::ACT_BTN_DNG_2 }}">
                                                            {!! Form::open(['method' => 'DELETE', 'url' => $destroyHref, 'id' => $delFormId]) !!}
                                                                <a id="customer-delete-btn-{{ $cid }}" href="#" data-url="{{ $destroyHref }}" data-guard-msg="{{ base64_encode($destroyGuardMsg) }}" class="{{ VC::BT_SM_CT_PR }}" data-bs-toggle="tooltip" title="{{ __('Delete') }}">
