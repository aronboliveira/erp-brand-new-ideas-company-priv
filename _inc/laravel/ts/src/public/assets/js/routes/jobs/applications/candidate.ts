...
((): void => {
    ...
    if (!el.getAttribute("data-listener-bound-click")) {
      ...
      el.addEventListener("click", (e: Event) => {
        const href = el.getAttribute("href") ?? "#",
          url = el.getAttribute("data-url") ?? href ?? "#";
  ...
  const bindFormGuard = (fm: Element): void => {
    if (!fm || (fm as HTMLElement).dataset?.submitGuarded === "true") return;
    (fm as HTMLElement).dataset.submitGuarded = "true";
    fm.addEventListener("submit", (e: Event) => {
      const action = fm.getAttribute("action") ?? "#",
        url = fm.getAttribute("data-url") ?? action ?? "#";
      if (!bad(action) && !bad(url)) return;
      e.preventDefault();
      toast(fm.getAttribute("data-guard-msg") || DEFAULT_ROUTE_MSG);
      (fm as HTMLElement).dataset.failedRoute = "true";
    });
  };
  ...
  const initDataTables = (): void => {
    ...
    if (window.jQuery?.fn.DataTable) tables.forEach(t => jQuery(t).DataTable());
  };
