...
((): void => {
  ...
  try {
    const cal = document.getElementById("zoom-calendar-link"),
      crt = document.getElementById("zoom-create-link"),
      svLang = window.svLang as unknown as NestedLang | undefined,
      dCal = svLang?.zoomMeetings?.index?.calendarGuardDefault || "",
      dCrt = svLang?.zoomMeetings?.index?.createGuardDefault || "";
    attachGuard(
      cal,
      dCal ??
