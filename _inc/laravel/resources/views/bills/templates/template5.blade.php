...
try {$rid  = data_get($bill, 'bill_id');
    ...
    $href = $enc ? route(VW::BIL . '.link.copy', $enc) : '#';
    if ($href !== '#') {
        $qrHtml = class_exists(\Milon\Barcode\DNS2D::class) && is_callable([\Milon\Barcode\DNS2D, 'getBarcodeHTML']) ? (new \Milon\Barcode\DNS2D)->getBarcodeHTML($href, 'QRCODE', 2, 2 ) : __('Failed to generate QRCode');}
} catch (\InvalidArgumentException $e) {Log::warning('QR: ' . $e->getMessage());
    $qrHtml = '<div></div>';} catch (\Exception $e) {Log::error('QR: ' . $e->getMessage());
    $qrHtml = '<div></div>';} catch (\Throwable $e) {Log::critical('QR: ' . $e->getMessage());
    $qrHtml = '<div></div>';}
?>
<!DOCTYPE html>
        ...
        <div class="bill-header">
            <table class="vertical-align-top">
                <tbody>
                    <tr>
                        <td>
                            <h3 style="display:block;text-transform:uppercase;font-size:30px;font-weight:bold;padding:15px;background:<?= e($color) ?>;color:<?= e($font_color) ?>"><?= e(__('BILL')) ?></h3>
            ...
            <table class="bill-summary" style="margin-top:30px;">
                ...
                <tbody style="border-bottom:1px solid <?= e($color) ?>">
                    <?php if (isset($bill->itemData) && is_iterable($bill->itemData) && count($bill->itemData) > 0): ?>
                        <?php foreach ($bill->itemData as $key => $item): ?>
                            ...
                            $unitLbl = '';
                            try {$u = \App\Models\ProductServiceUnit::find(data_get($item, 'unit'));
                                $unitLbl = data_get($u, 'name', '');} catch (\InvalidArgumentException $e) {Log::warning('Unit: ' . $e->getMessage());} catch (\Exception $e) {Log::error('Unit: ' . $e->getMessage());} catch (\Throwable $e) {Log::critical('Unit: ' . $e->getMessage());}
                            $itemtax = 0.0;
                            ?>
                            <tr>
                                <td><?= e(data_get($item, 'name', '')) ?></td>
                                <td><?= e((string)$qty) . ($unitLbl ? ' (' . e($unitLbl) . ')' : '') ?></td>
                                <td><?php try {echo e(Utility::priceFormat($settings, $price));} catch (\InvalidArgumentException $e) {Log::warning('Rate fmt: ' . $e->getMessage());
                                        ...
                                        echo '0';} ?></td>
                                <td><?= $disc != 0.0 ? e(Utility::priceFormat($settings, $disc)) : '-' ?></td>
                                <td>
                ...
                <tfoot>
                    <tr style="border-bottom:1px solid <?= e($color) ?>">
                        ...
                        <td><?php try {echo e(Utility::priceFormat($settings, data_get($bill, 'totalRate', 0)));} catch (\InvalidArgumentException $e) {Log::warning('totalRate fmt: ' . $e->getMessage());
                                ...
                                echo '0';} ?></td>
                        <td><?php try {echo e(Utility::priceFormat($settings, data_get($bill, 'totalDiscount', 0)));} catch (\InvalidArgumentException $e) {Log::warning('totalDiscount fmt: ' . $e->getMessage());
                                ...
                                echo '0';} ?></td>
                        <td><?php try {echo e(Utility::priceFormat($settings, data_get($bill, 'totalTaxPrice', 0)));} catch (\InvalidArgumentException $e) {Log::warning('totalTaxPrice fmt: ' . $e->getMessage());
                        ...
                        <td colspan="2" class="sub-total">
                            <table class="total-table">
                                ...
                                <?php if (method_exists($bill, 'getTotalDiscount') && $bill->getTotalDiscount()): ?>
                                    <tr style="border-bottom:1px solid <?= e($color) ?>">
                                        <td><?= e(__('Discount')) ?>:</td>
                                        <td><?php try {echo e(Utility::priceFormat($settings, $bill->getTotalDiscount()));} catch (\InvalidArgumentException $e) {Log::warning('Discount fmt: ' . $e->getMessage());
                                                echo '0';} catch (\Exception $e) {Log::error('Discount fmt: ' . $e->getMessage());
                                                echo '0';} catch (\Throwable $e) {Log::critical('Discount fmt: ' . $e->getMessage());
                                                echo '0';} ?></td>
                                    </tr>
                                <?php endif; ?>
                                ...
                                <tr style="border-bottom:1px solid <?= e($color) ?>">
                                    <td><?= e(__('Total')) ?>:</td>
                                    <td><?php
                                        try {$subtotal = method_exists($bill, 'getSubTotal') ? $bill->getSubTotal() : 0;
                                            $tdisc    = method_exists($bill, 'getTotalDiscount') ? $bill->getTotalDiscount() : 0;
                                            $ttax     = method_exists($bill, 'getTotalTax') ? $bill->getTotalTax() : 0;
                                            echo e(Utility::priceFormat($settings, $subtotal - $tdisc + $ttax));} catch (\InvalidArgumentException $e) {Log::warning('Grand total: ' . $e->getMessage());
                                ...
                                <tr style="border-bottom:1px solid <?= e($color) ?>">
                                    <td><?= e(__('Paid')) ?>:</td>
                                    <td><?php
                                        try {$paid = (method_exists($bill, 'getTotal') ? $bill->getTotal() : 0) - (method_exists($bill, 'getDue') ? $bill->getDue() : 0) - (method_exists($bill, 'billTotalDebitNote') ? $bill->billTotalDebitNote() : 0);
                                            echo e(Utility::priceFormat($settings, $paid));} catch (\InvalidArgumentException $e) {Log::warning('Paid fmt: ' . $e->getMessage());
                                ...
                                <tr style="border-bottom:1px solid <?= e($color) ?>">
                                    ...
                                    <td><?php try {echo e(Utility::priceFormat($settings, method_exists($bill, 'billTotalDebitNote') ? $bill->billTotalDebitNote() : 0));} catch (\InvalidArgumentException $e) {Log::warning('Debit note fmt: ' . $e->getMessage());
                                ...
                                <tr style="border-bottom:1px solid <?= e($color) ?>">
                                    ...
                                    <td><?php try {echo e(Utility::priceFormat($settings, method_exists($bill, 'getDue') ? $bill->getDue() : 0));} catch (\InvalidArgumentException $e) {Log::warning('Due fmt: ' . $e->getMessage());
