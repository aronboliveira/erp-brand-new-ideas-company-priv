	...
	public function run(): void
	{
		...
		DB::transaction(function () use ($faker) {
			...
			foreach ($samples as $i => [$name, $ext]) {
				...
				$d->id                   = $docId;
				$d->name                 = $name;
				$d->is_required          = $faker->boolean(35) ? 'true' : 'false';
				$d->is_private           = $faker->boolean(55);
				$d->file_path            = $filePath;
				$d->extension            = strtolower($ext);
				if ($mime) $d->mime_type = $mime;
				if ($kind) $d->type      = $kind;
				$d->size                 = $sizeBytes;
				$d->description          = $faker->boolean(60) ? $faker->sentence(12) : null;
				$d->notes                = $faker->boolean(40) ? $faker->sentence(10) : null;
				$d->expiration_date      = $faker->boolean(30) ? $faker->dateTimeBetween('+10 days', '+18 months') : null;
				$d->last_accessed        = $faker->boolean(70) ? $faker->dateTimeBetween('-6 months', 'now') : null;
				$d->permission_rules     = $rules;
				$d->viewers              = $viewers;
				$d->editors              = $editors;
