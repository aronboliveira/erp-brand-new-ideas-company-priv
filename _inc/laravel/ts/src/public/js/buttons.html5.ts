...
const _saveAs = (function(view) {
	...
	const
		...
		, throw_outside = function(ex) {
			(view.setImmediate || view.setTimeout)(function (): void {
				throw ex;
			}, 0);
		}
...
const _exportData = function ( dt, config )
{
	...
	const footer = config.footer && data.footer ? newLine+join( data.footer ) : '';
	const body = [];
 ...
 */
const _isDuffSafari = function (): void {
	...
	if ( version?.length > 1 && version[1]*1 < 603.1 ) {
		return true;
	}
