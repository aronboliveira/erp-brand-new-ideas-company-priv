...
trait HasFinancialIssuingColumns
{
	protected static function bootHasFinancialIssuingColumns(): void
	{
		try {
			...
			Log::error(static::class . '::bootHasFinancialIssuingColumns — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
		}
	}
	protected function addBasicFinancialIssuingColumns(Blueprint $table): void
	{
		try {
			...
			$table->string(BC::COL_CUR_ID, 3)->default(SC::DEF_SITE_CURRENCY_ID)->nullable();
			...
			Log::error(static::class . '::addBasicFinancialIssuingColumns — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
		}
	}
	protected function addFinancialIssuingColumns(Blueprint $table, bool $nullableReconcile = true): void
	{
		try {
			...
			$table->unsignedDecimal(BC::COL_SVC_FEE, 16, 2)->default(0.00)->nullable();
			$table->unsignedDecimal(BC::COL_TXS_FEE, 16, 2)->default(0.00)->nullable();
			...
			Log::error(static::class . '::addFinancialIssuingColumns — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
		}
	}
	protected function dropBasicFinancialIssuingColumnForeigns(Blueprint $table, ?string $tableName = null): void
	{
		try {
			...
			Log::error(static::class . '::dropBasicFinancialIssuingColumnForeigns — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
		}
	}
	protected function dropFinancialIssuingColumnForeigns(Blueprint $table, ?string $tableName = null): void
	{
		try {
			...
			Log::error(static::class . '::dropFinancialIssuingColumnForeigns — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
		}
	}
}
