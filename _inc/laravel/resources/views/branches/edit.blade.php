@php
    ...
    $branchUpdateRoute    = Route::has(ViewsConstants::BRC . '.update')
        ? route(ViewsConstants::BRC . '.update', $branch->id)
        : '#';
    $branchUpdateFormId   = 'branch-update-form-' . $branch->id;
...
@endphp
@if(!empty($branch) && isset($branch->id))
    {!! Form::model($branch, [
        'route'            => [$branchUpdateRoute],
        'method'           => 'PUT',
        ...
        <script defer>
            (() => {
                const form = document.getElementById('{{ $branchUpdateFormId }}');
    ...
    {!! Form::close() !!}
@else
    <div class="alert alert-danger">{{ __('No branch data found') }}</div>
@endif