...
describe("Backend Developer — Script Validation", () => {
  ...
  describe("frontend_security_audit.cjs", () => {
    const { scanDir, PATTERNS } = require(path.join(SCRIPTS, "js/scripts/frontend_security_audit.cjs"));
    ...
    test("script executa via node", () => {
      const out = run(`node "${path.join(SCRIPTS, "js/scripts/frontend_security_audit.cjs")}" "${path.resolve(PROJECT, "resources")}"`);
