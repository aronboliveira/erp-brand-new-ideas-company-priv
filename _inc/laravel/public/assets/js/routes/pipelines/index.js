        ...
        if (!el.getAttribute("data-listener-bound-click")) {
            ...
            el.addEventListener("click", function (e) {
                ...
                if (hasBs) {
                    ...
                    if (!modal)
                        return;
                    const modalTitle = modal.querySelector(".modal-title"), modalBody = modal.querySelector(".modal-body p"), cancelBtn = modal.querySelector(".modal-footer .btn-light"), yesBtnEl = modal.querySelector("#confirm-yes-btn");
                    if (modalTitle)
                    ...
                    if (yesBtnEl)
                        yesBtnEl.textContent = "OK";
                    const inst = window.bootstrap.Modal.getOrCreateInstance(modal), yesBtn = modal.querySelector("#confirm-yes-btn");
                    ...
                    if (confirm((title ? title + "\n\n" : "") + body)) {
                        try {
                            if (yes) {
                                // SECURITY: Safe handler dispatch instead of new Function()
                                const handlers = window.__confirmHandlers, fn = handlers?.[yes];
                                if (fn) {
                                    fn();
                                }
                                else {
