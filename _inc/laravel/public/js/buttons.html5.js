...
(function (factory) {
    ...
    const _exportData = function (dt, config) {
        const newLine = _newLine(config), data = dt.buttons.exportData(config.exportOptions), boundary = config.fieldBoundary, separator = config.fieldSeparator, reBoundary = new RegExp(boundary, "g"), escapeChar = config.escapeChar !== undefined ? config.escapeChar : "\\";
        ...
        const header = config.header ? join(data.header) + newLine : "", footer = config.footer && data.footer ? newLine + join(data.footer) : "", body = [];
    ...
    function createCellPos(n) {
        const ordA = "A".charCodeAt(0), ordZ = "Z".charCodeAt(0), len = ordZ - ordA + 1;
        let s = "";
        while (n >= 0) {
            ...
            if ($.isPlainObject(val)) {
                ...
                if (_ieExcel) {
                    ...
                    for (i = worksheet.attributes.length - 1; i >= 0; i--) {
                        const attrName = worksheet.attributes[i].nodeName, attrValue = worksheet.attributes[i].nodeValue;
    ...
    DataTable.ext.buttons.copyHtml5 = {
        ...
        action: function (e, dt, button, config) {
            ...
            const message = $("<span>" +
                dt.i18n("buttons.copyKeys", "Press <i>ctrl</i> or <i>\u2318</i> + <i>C</i> to copy the table data<br>to your system clipboard.<br><br>" +
    ...
    DataTable.ext.buttons.csvHtml5 = {
        ...
        action: function (e, dt, button, config) {
            // Set the text
            let output = _exportData(dt, config).str, charset = config.charset;
            if (config.customize)
                output = config.customize(output, config);
            if (charset !== false) {
                if (!charset)
    ...
    DataTable.ext.buttons.excelHtml5 = {
        ...
        action: function (e, dt, button, config) {
            ...
            const rels = getXml("xl/worksheets/sheet1.xml"), relsGet = rels.getElementsByTagName("sheetData")[0], xlsx = {
                _rels: {
            ...
            const zip = new _jsZip()(), zipConfig = {
                type: "blob",
    ...
    DataTable.ext.buttons.pdfHtml5 = {
        ...
        action: function (e, dt, button, config) {
            const newLine = _newLine(config), data = dt.buttons.exportData(config.exportOptions), rows = [];
            if (config.header)
...
//# sourceMappingURL=buttons.html5.js.map