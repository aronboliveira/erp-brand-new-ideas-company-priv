...
(() => {
    ...
    const showError = (key) => {
        ...
        const msg = window.translations?.[lang]?.[key] ||
            ...
            errFb;
        if (toastContainer.querySelector(`.toast[data-error-key="${key}"]`))
            return;
        ...
        document.querySelectorAll(".copy_link").forEach((el) => {
            ...
            el.addEventListener("click", (e) => {
                ...
                try {
                    ...
                    window.show_toastr("success", window.translations?.en?.copy_link_success ?? "Link copied");
                }
                catch (err) {
                    if (window.location.hostname === "localhost" ||
                        window.location.hostname === "127.0.0.1")
