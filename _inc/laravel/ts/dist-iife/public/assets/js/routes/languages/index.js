...
(() => {
    ...
    if (!el || el.getAttribute("data-listener-attached") === "true")
        return;
    ...
    function handler() {
        if (!el)
            return;
        try {
            const isChecked = el.checked ?? false, mode = isChecked ? "on" : "off", url = el.getAttribute("data-url"), href = el.form?.action ?? el.getAttribute("href");
            ...
            $.ajax({
                ...
                .done((data) => {
                window.show_toastr?.("success", data.message, "success");
            })
                .fail(() => {
    ...
    function showError(message) {
        try {
            ...
            if (!container) {
                const hasBootstrap = Array.from(document.querySelectorAll('link[rel="stylesheet"]')).some(l => /bootstrap/i.test(l.href)) && window.bootstrap.Toast;
                if (hasBootstrap) {
