...
//        $user = \Auth::user();
    ...
    public function saveGooglecalendarSettings(Request $request)
    {
        ...
        if ($request->google_calendar_json_file) {
            ...
            $file_name = $request->google_calendar_json_file->getClientOriginalName();
            $file_path = md5(time()) . '/' . md5(time()) . "." . $request->google_calendar_json_file->getClientOriginalExtension();

            $file = $request->file('google_calendar_json_file');
            $file->move($dir, $file_path);
            $post['google_calendar_json_file'] = $file_path;
