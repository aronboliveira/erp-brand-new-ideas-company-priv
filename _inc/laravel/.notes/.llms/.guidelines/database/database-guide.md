...
- **216+ tabelas** no schema `erp_brand_new_ideas_company_db`
- **216+ migrations** em `database/migrations/`
...
| `operation_ledgers`      | OperationLedger    | Durable state manager for high-impact operations         |
| `operation_steps`        | OperationStep      | Ordered step/state log for critical operations           |
| `outbox_messages`        | OutboxMessage      | Publish-after-commit intent queue                        |
| `inbox_messages`         | InboxMessage       | Receive-side idempotency tracking                        |
| `operational_events`     | OperationalEvent   | Durable operational timeline for medium+ events          |
