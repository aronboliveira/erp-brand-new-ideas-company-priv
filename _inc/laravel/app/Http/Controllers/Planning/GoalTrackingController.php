...
final class GoalTrackingController extends Controller
{
    ...
    public function index(Request $request): RedirectResponse|\Illuminate\View\View
    {
        try {
            ...
            if ($resp = self::guard($request, PermissionsConstants::MNG_GTR, self::REDIRECT_INDEX))
                return $resp;
            $user = $userOrRedirect;
            $goalTrackings = strtolower($user[UsersConstants::COL_TP]) === 'employee'
                ...
                ->where(DatabaseConstants::TABLE_CREATOR, $user?->creatorId())
                ->get();
            return view(ViewsConstants::GL_TRC . '.index', compact('goalTrackings'));
        } catch (\Throwable $e) {
    ...
    public function create(Request $request): RedirectResponse|\Illuminate\View\View
    {
        try {
            if (($userOrRedirect = self::_checkLogin()) instanceof RedirectResponse)
                return $userOrRedirect;
            if ($resp = self::guard($request, 'create goal tracking', self::REDIRECT_INDEX))
                return $resp;
            $user = $userOrRedirect;
            ...
            return view(ViewsConstants::GL_TRC . '.' . __FUNCTION__, compact('branches', 'goalTypes', 'status'));
        } catch (\Throwable $e) {
            return defaultUndefinedException(
                $request,
                $e,
    ...
    public function show(Request $request, GoalTracking $goalTracking): RedirectResponse|\Illuminate\View\View|null
    {
        $action = __METHOD__;
        ...
        try {
            ...
            return view(ViewsConstants::GL_TRC . '.show', compact('goalTracking'));
        } catch (\Throwable $e) {
    ...
    public function edit(Request $request, int $id): RedirectResponse|\Illuminate\View\View
    {
        try {
            if (($userOrRedirect = self::_checkLogin()) instanceof RedirectResponse)
                return $userOrRedirect;
            if ($resp = self::guard($request, 'edit goal tracking', self::REDIRECT_INDEX))
                return $resp;
            $user = $userOrRedirect;
            ...
            return view(ViewsConstants::GL_TRC . '.edit', compact(
                'branches',
                'goalTypes',
                'goalTracking',
                'ratings',
    ...
    public function destroy(Request $request, int $id): RedirectResponse
    {
        try {
            if (($userOrRedirect = self::_checkLogin()) instanceof RedirectResponse)
                return $userOrRedirect;
            if ($resp = self::guard($request, 'delete goal tracking', self::REDIRECT_INDEX))
                return $resp;
            $user = $userOrRedirect;
