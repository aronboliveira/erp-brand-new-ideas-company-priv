...
> **Cross-references:** [`KNOWN_ISSUES.md`](KNOWN_ISSUES.md) (open issues list) · [`CURRENT_WORKING_ISSUES_WORK.md`](CURRENT_WORKING_ISSUES_WORK.md) (try/fail/success journal) · [`NEXT_STEPS.md`](NEXT_STEPS.md) (remaining tasks) · [`RESOLVED_ISSUES.md`](RESOLVED_ISSUES.md) (resolved archive) · [`README.md`](README.md) (notes overview)
...
## Session 11 — Claude Skip Reduction + Static Cleanup Continuation (2026-05-08 → 2026-05-09)

**Source:** recovered Claude session
`d9570c96-1845-40c3-b642-65229ebd51b9` and updated handoff at
`.tmp/claude/20260808/HANDOFF.md`.

**Scope:** PHPUnit skip reduction, PHPStan/ESLint cleanup, UtilityTest
settings-leak stabilization, and handoff reconciliation after Claude hit
usage/token limit.

### Commits

15 commits landed between `5a6c9afac` and `157145304`, including:

- `a0243988d` — reconciled `ProjectsConstants::COL_STG` with the `tasks.stages`
  JSON migration and added the Customer/User bridge.
- `1e73c3e13` — cleared PHPStan and ESLint blockers.
- `2bc9199cd`, `002d2d518`, `157145304` — stabilized UtilityTest factories,
  unique fixtures, and DEFAULT_UUID settings cleanup.

### Final Captured PHPUnit State

```text
Tests: 10575, Assertions: 20348, Errors: 1, Failures: 2,
Deprecations: 38, Skipped: 8, Incomplete: 4.
```

Open items moved to `KNOWN_ISSUES.md`:

- RT-007 — `OrderTest::order_uses_uuid_for_primary_key` duplicate
  `u@uuid.test`.
- RT-008 — Discover upload-failure test gets null max-upload validator error.
- RT-009 — Features `featureStore` test does not reach success flash.
- RT-010 — 8 remaining aliasMock skips.
- RT-011 — 4 incomplete markers.

Closed stale handoff items:

- Old RT-004 (`UtilityTest` 6 failures) — final utils suite is clean.
- Old RT-005 (PHPStan 12 BillProduct namespace errors) — fixed by `1e73c3e13`.
- Old ESLint 34 warnings — fixed by `1e73c3e13`.

---

## Session 12 — Codex continuation: remaining Unit-suite failures closed (2026-05-09)

**Source:** `.tmp/codex/20260509/handsoff.md`.

### Fixed

- RT-007: `OrderTest::order_uses_uuid_for_primary_key` now generates unique
  order/email/receipt values for the full-suite order.
- RT-008: Discover upload-failure test now seeds complete local storage
  settings and reaches the intended mime-validation error.
- RT-009: Features `featureStore` upload test now seeds complete local storage
  settings and ensures the upload directory exists.
