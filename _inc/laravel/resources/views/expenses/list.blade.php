                        ...
                        @if(count($expenses) > 0)
                            @foreach($expenses as $expense)
                                <tr>
                                    ...
                                    <td>
                                        <span class="h6 text-sm font-weight-bold mb-0">{{ $expense->project->name }} <span class="badge badge-xs badge-{{ (\Auth::user()->checkProject($expense->project->id) == 'Owner') ? ProjectsConstants::STT_SCS : ProjectsConstants::STT_WRN  }}">{{ \Auth::user()->checkProject($expense->project->id) }}</span></span>
                                    </td>
                                    <td>
