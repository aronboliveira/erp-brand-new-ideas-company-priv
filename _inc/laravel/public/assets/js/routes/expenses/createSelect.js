...
(() => {
    ...
    const initSelection = () => {
        ...
        radios.forEach(r => {
            if (r.getAttribute("data-listener-active") === "true")
                return;
    ...
    const setupAjax = (type) => {
        ...
        if (!sel || sel.getAttribute("data-listener-active") === "true")
            return;
        sel.setAttribute("data-listener-active", "true");
        const detail = document.getElementById(`${type}_detail`), box = document.getElementById(`${type}-box`);
        if (!sel.getAttribute("data-listener-bound-change")) {
            sel.setAttribute("data-listener-bound-change", "1");
            sel.addEventListener("change", () => {
                if (detail)
                    detail.classList.replace("d-none", "d-block");
                if (box)
