...
$qrHtml = '';
try {$rid = data_get($bill, 'bill_id');
    $enc = $rid ? Crypt::encrypt($rid) : null;
    $routeName = VW::BIL . '.link.copy';
    $url = ($enc && Route::has($routeName)) ? route($routeName, $enc) : '#';
    $qrHtml = (new \Milon\Barcode\DNS2D)->getBarcodeHTML($url, 'QRCODE', 2, 2);} catch (\InvalidArgumentException $e) {Log::warning('QR: ' . get_class($e) . ' | ' . $e->getMessage());
    $qrHtml = '<div></div>';} catch (\Exception $e) {Log::error('QR: ' . get_class($e) . ' | ' . $e->getMessage());
    $qrHtml = '<div></div>';} catch (\Throwable $e) {Log::critical('QR: ' . get_class($e) . ' | ' . $e->getMessage());
    $qrHtml = '<div></div>';}
...
try {$totalRate     = Utility::priceFormat($settings, data_get($bill, 'totalRate', 0)) ?: '0';} catch (\Throwable $e) {}
try {$totalDiscount = Utility::priceFormat($settings, data_get($bill, 'totalDiscount', 0)) ?: '0';} catch (\Throwable $e) {}
try {$totalTaxPrice = Utility::priceFormat($settings, data_get($bill, 'totalTaxPrice', 0)) ?: '0';} catch (\Throwable $e) {}
try {$subTotal      = Utility::priceFormat($settings, method_exists($bill, 'getSubTotal') ? $bill->getSubTotal() : 0) ?: '0';} catch (\Throwable $e) {}
try {$grandTotalRow  = Utility::priceFormat($settings, (method_exists($bill, 'getSubTotal') ? $bill->getSubTotal() : 0) - (method_exists($bill, 'getTotalDiscount') ? $bill->getTotalDiscount() : 0) + (method_exists($bill, 'getTotalTax') ? $bill->getTotalTax() : 0)) ?: '0';} catch (\Throwable $e) {}
try {$paidTotal      = Utility::priceFormat($settings, (method_exists($bill, 'getTotal') ? $bill->getTotal() : 0) - (method_exists($bill, 'getDue') ? $bill->getDue() : 0) - (method_exists($bill, 'billTotalDebitNote') ? $bill->billTotalDebitNote() : 0)) ?: '0';} catch (\Throwable $e) {}
try {$debitNoteTotal = Utility::priceFormat($settings, method_exists($bill, 'billTotalDebitNote') ? $bill->billTotalDebitNote() : 0) ?: '0';} catch (\Throwable $e) {}
try {$dueAmount      = Utility::priceFormat($settings, method_exists($bill, 'getDue') ? $bill->getDue() : 0) ?: '0';} catch (\Throwable $e) {}
?>
<!DOCTYPE html>
                    ...
                    <?php if (isset($bill->itemData) && is_iterable($bill->itemData) && count($bill->itemData) > 0): ?>
                        <?php foreach ($bill->itemData as $item): ?>
                            ...
                            try {$unit = ProductServiceUnit::find(data_get($item, 'unit'));
                                $unitLabel = data_get($unit, 'name', '');} catch (\InvalidArgumentException $e) {Log::warning('Unit find: ' . $e->getMessage());} catch (\Exception $e) {Log::error('Unit find: ' . $e->getMessage());} catch (\Throwable $e) {Log::critical('Unit find: ' . $e->getMessage());}
                            ?>
                            <tr>
                        ...
                        <td colspan="2" class="sub-total">
                            <table class="total-table">
                                ...
                                <?php if (method_exists($bill, 'getTotalDiscount') && $bill->getTotalDiscount()): ?>
                                    <tr>
                                        <td><?= e(__('Discount')) ?>:</td>
                                        <td><?php try {echo e(Utility::priceFormat($settings, $bill->getTotalDiscount()));} catch (\InvalidArgumentException $e) {Log::warning('Total discount: ' . $e->getMessage());
                                                echo '0';} catch (\Exception $e) {Log::error('Total discount: ' . $e->getMessage());
                                                echo '0';} catch (\Throwable $e) {Log::critical('Total discount: ' . $e->getMessage());
                                                echo '0';} ?></td>
                                    </tr>
                                <?php endif; ?>
