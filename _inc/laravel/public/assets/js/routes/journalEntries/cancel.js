...
(() => {
    try {
        const btns = Array.from(document.querySelectorAll("button.cancel-link[data-href][data-guard-msg],input.cancel-link[data-href][data-guard-msg]"));
        if (btns.length === 0)
            return;
        btns.forEach(btn => {
            if (btn.getAttribute("data-click-guarded") === "true")
                return;
