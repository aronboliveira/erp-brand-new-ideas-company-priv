...
/** Show element */
function _show(el) { if (el)
    el.style.display = ""; }
/** Hide element */
function _hide(el) { if (el)
    el.style.display = "none"; }
/** Toggle element */
function _toggle(el) { if (el)
 ...
 *-------------------------------------------------------------
 */
let messenger, auth_id = _meta("url") ? _meta("url").getAttribute("data-user") : "", route = _meta("route") ? _meta("route").getAttribute("content") : "", url = _meta("url") ? _meta("url").getAttribute("content") : "", access_token = _meta("csrf-token") ? _meta("csrf-token").getAttribute("content") : "", typingTimeout, typingNow = 0, temporaryMsgId = 0, defaultAvatarInSettings = null, messengerColor, dark_mode;
const messagesContainer = qs(".messenger-messagingView .m-body"), messengerTitleDefault = (qs(".messenger-headTitle") || {}).textContent || "", messageInput = qs("#message-form .m-send");
/**
 *-------------------------------------------------------------
 * Global Templates
 *-------------------------------------------------------------
...
function loadingSVG(w_h = "25px", className = null) {
    return ('<svg class="loadingSVG ' + (className || "") +
...
const app_modal = function ({ show = true, name, data = 0, buttons = true, header = null, body = null }) {
    ...
    // PULL REQUEST START — Alteração customizada em arquivo vendor (correção de XSS)
    if (header) {
        const h = qs(".app-modal-header", modal);
        if (h)
            h.innerHTML = _sanitizeHtml(header);
    }
    if (body) {
        const b = qs(".app-modal-body", modal);
        if (b)
            b.innerHTML = _sanitizeHtml(body);
    }
    // PULL REQUEST END — Fim da alteração customizada
    const footer = qs(".app-modal-footer", modal);
    if (footer)
        buttons ? _show(footer) : _hide(footer);
    const card = qs('.app-modal-card[data-name="' + name + '"]');
    if (show) {
        _show(modal);
        if (card) {
            card.classList.add("app-show-modal");
            card.setAttribute("data-modal", data);
        }
    }
    else {
        _hide(modal);
        if (card) {
            card.classList.remove("app-show-modal");
            card.setAttribute("data-modal", data);
        }
    }
};
...
function disableOnLoad(action) {
    ...
    if (action) {
        ...
        if (messagesContainer)
            messagesContainer.style.opacity = ".5";
        if (messageInput)
...
function errorMessageCard(id) {
    ...
    const p = qs("p", card);
    if (p)
        p.insertAdjacentHTML("afterbegin", '<span class="fas fa-exclamation-triangle"></span>');
}
...
function IDinfo(id, type) {
    ...
    if (messenger != 0) {
        ...
        _post(url + "/idInfo", { _token: access_token, id: id, type: type })
            ...
            if (infoAvatar)
                infoAvatar.style.backgroundImage = _safeCssUrl(data.user_avatar);
            const headerAvatar = qs(".header-avatar");
            if (headerAvatar)
                headerAvatar.style.backgroundImage = _safeCssUrl(data.user_avatar);
            // PULL REQUEST END — Fim da alteração customizada
            ...
            if (infoName)
                infoName.textContent = data.fetch.name;
            const userName = qs(".m-header-messaging .user-name");
            if (userName)
                userName.textContent = data.fetch.name;
            // PULL REQUEST END — Fim da alteração customizada
            data.favorite > 0
...
function fetchMessages(id, type) {
    if (messenger != 0) {
        _post(url + "/fetchMessages", { _token: access_token, id: id, type: type })
            ...
            const msgs = messagesContainer ? qs(".messages", messagesContainer) : null;
            if (msgs)
                msgs.innerHTML = _sanitizeHtml(data.messages);
            // PULL REQUEST END — Fim da alteração customizada
            scrollBottom(messagesContainer);
            makeSeen(true);
        })
...
function cancelUpdatingAvatar() {
    ...
    if (preview)
        preview.style.backgroundImage = defaultAvatarInSettings;
 ...
 *-------------------------------------------------------------
 */
const pusher = window.__appPusher, channel = pusher?.subscribe("private-chatify");
if (channel && typeof channel.bind === "function") {
    channel.bind("messaging", function (data) {
        if (data.from_id == messenger.split("_")[1] && data.to_id == auth_id) {
            ...
            const msgs = messagesContainer ? qs(".messages", messagesContainer) : null;
            if (msgs)
                msgs.insertAdjacentHTML("beforeend", data.message);
            scrollBottom(messagesContainer);
            makeSeen(true);
    ...
    channel.bind("client-typing", function (data) {
        if (data.from_id == messenger.split("_")[1] && data.to_id == auth_id) {
            ...
            if (indicator)
                data.typing ? _show(indicator) : _hide(indicator);
        }
        scrollBottom(messagesContainer);
    });
    ...
    channel.bind("client-contactItem", function (data) {
        if (data.update_for == auth_id && data.updating)
            updateContatctItem(data.update_to);
    });
}
else
    console.warn('Pusher channel "private-chatify" not found!');
...
if (activeStatusChannel && typeof activeStatusChannel.bind === "function") {
    activeStatusChannel.bind("pusher:member_added", function (member) {
        ...
        if (item) {
            ...
            if (avatar)
                avatar.insertAdjacentHTML("beforebegin", activeStatusCircle());
        }
    });
    ...
    console.warn('Pusher channel "presence-activeStatus" not found!');
/**
 *-------------------------------------------------------------
...
function makeSeen(status) {
    ...
    _post(url + "/makeSeen", { _token: access_token, id: messenger.split("_")[1] })
        ...
        if (counter)
            counter.textContent = data.messengerCount;
    })
        .catch(function () { });
...
function checkInternet(state, selector) {
    ...
    switch (state) {
        case "connected":
            if (net_errs < 1) {
                if (messengerTitle)
                    messengerTitle.textContent = messengerTitleDefault;
                if (selector) {
                    selector.classList.add("successBG-rgba");
                    qsa("span", selector).forEach(function (s) { _hide(s); });
                    _show(selector);
                    _show(qs(".ic-connected", selector));
                }
        ...
        case "connecting":
            if (messengerTitle) {
                const ic = qs(".ic-connecting");
                messengerTitle.textContent = ic ? ic.textContent : "";
            }
            if (selector) {
                selector.classList.remove("successBG-rgba");
                qsa("span", selector).forEach(function (s) { _hide(s); });
                _show(selector);
                _show(qs(".ic-connecting", selector));
            }
            net_errs = 1;
            break;
        default:
            if (messengerTitle) {
                const ic = qs(".ic-noInternet");
                messengerTitle.textContent = ic ? ic.textContent : "";
            }
            if (selector) {
                selector.classList.remove("successBG-rgba");
                qsa("span", selector).forEach(function (s) { _hide(s); });
                _show(selector);
                _show(qs(".ic-noInternet", selector));
            }
            net_errs = 1;
            break;
    }
...
function getContacts() {
    ...
    if (list)
        list.innerHTML = listItemLoading(4);
    _get(url + "/getContacts", { _token: access_token, messenger_id: messenger.split("_")[1] })
        .then(function (data) {
        // PULL REQUEST START — Alteração customizada em arquivo vendor (correção de XSS)
        if (list)
            list.innerHTML = _sanitizeHtml(data.contacts);
        const allMembers = qs(".all_members");
        if (allMembers)
            allMembers.innerHTML = _sanitizeHtml(data.allUsers);
        // PULL REQUEST END — Fim da alteração customizada
        cssMediaQueries();
    })
...
function updateContatctItem(user_id) {
    if (user_id != auth_id) {
        ...
        _post(url + "/updateContacts", { _token: access_token, user_id: user_id, messenger_id: messenger.split("_")[1] })
            ...
            const list = qs(".listOfContacts");
            if (list)
                list.insertAdjacentHTML("afterbegin", data.contactItem);
            const counter = qs(".custom_messanger_counter");
            if (counter)
                counter.textContent = data.messengerCount;
            cssMediaQueries();
...
function star(user_id) {
    if (messenger.split("_")[1] != auth_id) {
        _post(url + "/star", { _token: access_token, user_id: user_id })
            ...
            if (fav)
                data.status > 0 ? fav.classList.add("favorite") : fav.classList.remove("favorite");
        })
...
function getFavoritesList() {
    ...
    if (favs)
        favs.innerHTML = avatarLoading(4);
    _post(url + "/favorites", { _token: access_token })
        .then(function (data) {
        // PULL REQUEST START — Alteração customizada em arquivo vendor (correção de XSS)
        if (favs)
            favs.innerHTML = _sanitizeHtml(data.favorites);
        // PULL REQUEST END — Fim da alteração customizada
        cssMediaQueries();
    })
...
function getSharedPhotos(user_id) {
    _post(url + "/shared", { _token: access_token, user_id: user_id })
        ...
        if (list)
            list.innerHTML = _sanitizeHtml(data.shared);
        // PULL REQUEST END — Fim da alteração customizada
    })
...
function messengerSearch(input) {
    ...
    if (records)
        records.innerHTML = listItemLoading(4);
    _post(url + "/search", { _token: access_token, input: input })
        ...
        if (records) {
            ...
            if (svg)
                svg.remove();
            // PULL REQUEST START — Alteração customizada em arquivo vendor (correção de XSS)
            if (data.addData === "append")
                records.insertAdjacentHTML("beforeend", _sanitizeHtml(data.records));
            else
                records.innerHTML = _sanitizeHtml(data.records);
            // PULL REQUEST END — Fim da alteração customizada
        }
        cssMediaQueries();
    })
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (infoNavA)
        infoNavA.addEventListener("click", function () { _show(qs(".show-infoSide")); });
    const showInfoSide = qs(".show-infoSide");
    if (showInfoSide)
        showInfoSide.addEventListener("click", function () { _hide(this); });
    // Favorites drag
    hScroller(".messenger-favorites");
    // Click list item
    onBody("click", ".messenger-list-item", function () {
        const trAction = qs("tr[data-action]", this);
        if (trAction && trAction.getAttribute("data-action") === "1")
            _hide(qs(".messenger-listView"));
        const pId = qs("p[data-id]", this);
        if (pId)
            messenger = pId.getAttribute("data-id");
        IDinfo(messenger.split("_")[1], messenger.split("_")[0]);
    });
    // Click favorite item
    onBody("click", ".favorite-list-item", function () {
        const div = qs("div", this);
        if (div && div.getAttribute("data-action") === "1")
            _hide(qs(".messenger-listView"));
        const avatar = qs("div.avatar", this);
        if (avatar)
            messenger = "user_" + avatar.getAttribute("data-id");
        IDinfo(messenger.split("_")[1], messenger.split("_")[0]);
    });
    // List view buttons
    const listViewX = qs(".listView-x");
    if (listViewX)
        listViewX.addEventListener("click", function () { _hide(qs(".messenger-listView")); });
    const showListView = qs(".show-listView");
    if (showListView)
        showListView.addEventListener("click", function () { _show(qs(".messenger-listView")); });
    // Favorite button
    const favBtn = qs(".add-to-favorite");
    if (favBtn)
        favBtn.addEventListener("click", function () { star(messenger.split("_")[1]); });
    cssMediaQueries();
    // Message form submit
    ...
    onBody("change", ".upload-attachment", function (e) {
        ...
        reader.addEventListener("loadstart", function () {
            const form = qs("#message-form");
            if (form)
                form.insertAdjacentHTML("beforebegin", loadingSVG());
        });
    ...
    onBody("click", ".chat-image", function () {
        ...
        if (boxSrc)
            boxSrc.setAttribute("src", src);
    });
    const imgClose = qs(".imageModal-close");
    if (imgClose)
        imgClose.addEventListener("click", function () { _hide(qs("#imageModalBox")); });
    // Search
    ...
    if (deleteBtn)
        deleteBtn.addEventListener("click", function () { app_modal({ name: "delete" }); });
    ...
    if (deleteModal) {
        ...
        if (cancelDel)
            cancelDel.addEventListener("click", function () { app_modal({ show: false, name: "delete" }); });
    }
    // Settings
    const settingsBtn = qs(".settings-btn");
    if (settingsBtn)
        settingsBtn.addEventListener("click", function () { app_modal({ name: "settings" }); });
    const updateForm = qs("#updateAvatar");
    if (updateForm)
        updateForm.addEventListener("submit", function (e) { e.preventDefault(); updateSettings(); });
    ...
    onBody("change", ".upload-avatar", function (e) {
        if (defaultAvatarInSettings == null) {
            const preview = qs(".upload-avatar-preview");
            if (preview)
                defaultAvatarInSettings = preview.style.backgroundImage;
        }
        const file = e.target.files[0];
        if (!file)
            return;
        ...
        reader.addEventListener("loadstart", function () {
            const preview = qs(".upload-avatar-preview");
            if (preview)
                preview.insertAdjacentHTML("beforeend", loadingSVG("42px", "upload-avatar-loading"));
        });
        reader.addEventListener("load", function (ev) {
            const preview = qs(".upload-avatar-preview");
            if (preview) {
