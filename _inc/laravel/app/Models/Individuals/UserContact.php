...
class UserContact extends Model
{
    ...
    {
        // name: nullable no DB, mas precisa ser único; se vazio, gera uuid-string
        ...
        // booleans defaults defensivos (null -> false)
    ...
    private function normalizeAndSyncFromUser(): void
    {
        ...
        // normaliza email/phone atuais (se existirem)
        ...
        // 2) email sync (somente se email do contato estiver null)
        ...
        // 3) phone sync (somente se phone do contato estiver null)
        ...
        // 4) address sync (somente se address do contato estiver null)
        ...
        // 6) last_contacted_at >= user.created_at (interpretação coerente de “não ser menor”)
        try {
        ...
        // re-normaliza email/phone pós-sync
        $email = $this->getAttribute('email');
        $phone = $this->getAttribute('phone');
    ...
    private function avoidUniqueCollisionsOrNullify(): void
    {
        ...
        // se ambos viraram null por colisão, falha (regra do schema/negócio)
        $email = $this->getAttribute('email');
        $phone = $this->getAttribute('phone');
    ...
    private function getUserRowCached(string $userId): ?array
    {
        ...
        try {
            ...
            // normaliza chaves esperadas via constantes (quando existirem)
