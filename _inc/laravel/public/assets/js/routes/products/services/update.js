...
(() => {
    ...
    const qtyWrap = document.querySelector(".quantity"), typeRadios = document.querySelectorAll('input[name="type"]'), qtyInput = document.querySelector('input[name="quantity"]');
    const syncQtyVisibility = () => {
        const checked = document.querySelector('input[name="type"]:checked'), val = checked?.value;
        if (!qtyWrap || !qtyInput)
            return;
        if (val === "service") {
    ...
    syncQtyVisibility();
    const file = document.getElementById("pro_image"), img = document.getElementById("image");
    if (file && img) {
