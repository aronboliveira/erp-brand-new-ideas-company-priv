...
from ...Models.info.notification_template_langs import NotificationTemplateLangs

class NotificationSeeder(BaseCommand):
    help = "Seeds the notification templates and their language variants into the database."
    ...
    def handle(self, *args, **options):
        ...
        for slug, label in notifications.items():
            template_obj, created = NotificationTemplates.objects.get_or_create(
                slug=slug,
                defaults={'name': label}
            )

            if not created:
                # If found an existing template, let's refresh it with the new label
                template_obj.name = label
                template_obj.save()

            # If the slug is defined in the defaultTemplate, let's fill or update translations
            if slug in defaultTemplate['notification']:
                template_config = defaultTemplate['notification'][slug]

                for lang, content_str in template_config['lang'].items():
                    nt_lang_obj, sub_created = NotificationTemplateLangs.objects.get_or_create(
                        parent=template_obj,
                        lang=lang,
                        defaults={
                    ...
                    if not sub_created:
                        nt_lang_obj.variables = template_config['variables']
                        nt_lang_obj.content = content_str
                        nt_lang_obj.save()

        self.stdout.write(self.style.SUCCESS("Notification templates seeded successfully."))