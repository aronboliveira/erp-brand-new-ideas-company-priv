...
(function () {
    ...
    const schedulePointerupError = (msg) => {
        ...
        if (!host || host.getAttribute(dataErr) === "true")
            return;
    ...
    const buildDeleteUrl = (id, el) => {
        ...
        if ((!url || url === "#") && (!href || href === "#")) {
            const fallback = "{{ url('zoom-meeting') }}".replace(/\/$/, "") + "/" + id;
            return fallback;
        }
    ...
    const onConfirmDelete = function () {
        const id = String(this.getAttribute("uid") ?? ""), targetEl = this, url = buildDeleteUrl(id, targetEl), urlAttr = targetEl.getAttribute("data-url"), hrefAttr = targetEl.getAttribute("href");
        if ((!urlAttr || urlAttr === "#") &&
            (!hrefAttr || hrefAttr === "#") &&
            (!url || url === "#")) {
    ...
    const bind = () => {
        if (!ensureJq())
            return;
        const host = document.body;
        if (host.getAttribute(dataInit) === "true")
            return;
        host.setAttribute(dataInit, "true");
        $(document).on("click" + ns, ".member_remove", function () {
            onOpenConfirm.call(this);
        });
        $(document).on("click" + ns, ".confirm_yes.m_remove", function () {
