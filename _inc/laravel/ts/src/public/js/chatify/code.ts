...
function disableOnLoad(action = true) {
  if (action) {
    ...
    // show star button
    if (messenger.split("_")[1] != auth_id) $(".add-to-favorite").show();
    // show send card
...
function sendMessage() {
  ...
  const tempID = "temp_" + temporaryMsgId,
    hasFile = $(".upload-attachment").val() ? true : false;
...
$(document).ready(function () {
  ...
  pusher?.connection?.bind("state_change", function (states) {
    checkInternet(states.current, $(".internet-connection"));
