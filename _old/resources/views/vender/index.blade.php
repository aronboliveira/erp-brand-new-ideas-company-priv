                            ...
                            <tbody>
                                @foreach ($vendors as $k => $Vendor)
                                    <tr class="cust_tr" id="vend_detail">
                                        <td class="Id">
                                            @can('show vendor')
                                                <a href="{{ route('vendor.show', \Crypt::encrypt($Vendor['id'])) }}" class="btn btn-outline-primary">
                                                    {{ AUth::user()->vendorNumberFormat($Vendor['vendor_id']) }}
                                                </a>
                                            @else
                                                <a href="#" class="btn btn-outline-primary"> {{ AUth::user()->vendorNumberFormat($Vendor['vendor_id']) }}
                                                </a>
                                            @endcan
                                        </td>
                                        <td>{{ $Vendor['name'] }}</td>
                                        ...
                                        <td>{{ $Vendor['email'] }}</td>
                                        <td>{{ \Auth::user()->priceFormat($Vendor['balance']) }}</td>
                                        <td class="Action">
                                            <span>
                                                    ...
                                                    @else
                                                        ...
                                                        @can('delete vendor')
                                                            <div class="action-btn bg-danger ms-2">
                                                                {!! Form::open(['method' => 'DELETE', 'route' => ['vendor.destroy', $Vendor['id']], 'id' => 'delete-form-' . $Vendor['id']]) !!}
