...
@section(YieldingConstants::ADM_CTT)
    <div class="card">
        <div class="invoice-title">{{ $user?->estimateNumberFormat($estimation->estimation_id) }}</div>
        <div class="invoice-detail pb-2">
                    ...
                    <div class="col-md-3 col-sm-6 col-6">
                        <div class="text-status"><strong>{{__('Status')}} :</strong>
                            @if($estimation->status == 0)
                                <span class="badge badge-pill badge-primary">{{ __(Estimation::$statuses[$estimation->status]) }}</span>
                            @elseif($estimation->status == 1)
                                <span class="badge badge-pill badge-danger">{{ __(Estimation::$statuses[$estimation->status]) }}</span>
                            @elseif($estimation->status == 2)
                                <span class="badge badge-pill badge-warning">{{ __(Estimation::$statuses[$estimation->status]) }}</span>
                            @elseif($estimation->status == 3)
                                <span class="badge badge-pill badge-success">{{ __(Estimation::$statuses[$estimation->status]) }}</span>
                            @elseif($estimation->status == 4)
                                <span class="badge badge-pill badge-info">{{ __(Estimation::$statuses[$estimation->status]) }}</span>
                            @endif
                        </div>
                    </div>
                    <div class="col-md-9 col-sm-9 col-9">
                        <div class="text-status text-end">{{__('Issue Date')}}:<strong>{{ $user?->dateFormat($estimation->issue_date) }}</strong></div>
                    </div>
                </div>
            </div>
            <div class="row">
                            ...
                            <table class="table align-items-center mb-0">
                                ...
                                @foreach($estimation->getProducts as $product)
                                    <tr>
                                        ...
                                        <td class="small-order">{{$user?->priceFormat($product->pivot->price)}}</td>
                                        <td class="small-order">{{$product->pivot->quantity}}</td>
                                        @php
                                            $price = $product->pivot->price * $product->pivot->quantity;
                                        @endphp
                                        <td class="invoice-order text-end">{{$user?->priceFormat($price)}}</td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
            ...
            <div class="row text-end">
                <div class="col-md-3">
                    ...
                    @endphp
                    <div class="text-status"><strong>{{__('Subtotal')}} :</strong> {{$user?->priceFormat($subTotal)}}</div>
                </div>
                <div class="col-md-3">
                    <div class="text-status"><strong>{{__('Discount')}} :</strong> {{$user?->priceFormat($estimation->discount)}}</div>
                </div>
                <div class="col-md-3">
                    @php
                        $tax = $estimation->getTax();
                    @endphp
                    <div class="text-status"><strong>{{$estimation->tax->name}} ({{$estimation->tax->rate}} %) :</strong> {{$user?->priceFormat($tax)}}</div>
                </div>
                <div class="col-md-3">
                    <div class="text-status"><strong>{{__('Total')}} :</strong> {{$user?->priceFormat($subTotal-$estimation->discount+$tax)}}</div>
                </div>
            </div>
        </div>
    </div>
