	...
	public function getDates(): array
	{
		...
		try {
			foreach (static::DATE_COLUMNS as $dateColumn)
				if (Schema::hasColumn($this->getTable(), $dateColumn) && !in_array($dateColumn, $dates, true))
					$dates[] = $dateColumn;
			if (Schema::hasColumn($this->getTable(), 'from') && in_array(strtolower(Schema::getColumnType($this->getTable(), 'from')), ['date', 'timestamp', 'datetime']) && !in_array('from', $dates, true))
				$dates[] = 'from';
			if (Schema::hasColumn($this->getTable(), 'to') && in_array(strtolower(Schema::getColumnType($this->getTable(), 'to')), ['date', 'timestamp', 'datetime']) && !in_array('to', $dates, true))
				$dates[] = 'to';
