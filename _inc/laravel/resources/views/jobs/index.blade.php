...
$user = Auth::user();
        ...
        \Log::error('jobs/index — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
    }
@endphp
...
@section(YieldingConstants::ADM_ACT_BTN)
    <div class="{{ VC::FEND }}">
        @if($canCreate)
            <a href="{{ $createUrl }}" class="{{ VC::BT_SM_PM }} route-guard" data-url="{{ $createUrl }}" data-guard-msg="{{ base64_encode($createGuard) }}" data-bs-toggle="tooltip" title="{{ __('Create') }}" data-title="{{ __('Create New Job') }}">
                <i class="{{ VC::TI_PLS }}"></i>
            </a>
        @endif
    </div>
...
@section(YieldingConstants::ADM_CTT)
    <div class="{{ VC::RW }}">
        <div class="{{ VC::CL4 }} {{ VC::CM6 }}">
            <div class="{{ VC::CD }}"><div class="{{ VC::CD_BD }}">
    ...
    <div class="{{ VC::RW }}">
        <div class="{{ VC::CM12 }}">
            <div class="{{ VC::CD }}"><div class="{{ VC::CD_BD_TB_BD }}">
                <div class="{{ VC::TB_RSP }}">
...
@endphp
                                    <tr>
                                        ...
                                        <td>
                                            @if($isActive)
                                                <span class="status_badge badge {{ VC::BG_P }} p-2 {{ VC::PX3 }} rounded">{{ $statusTxt }}</span>
                                            @else
                                                <span class="status_badge badge bg-danger p-2 {{ VC::PX3 }} rounded">{{ $statusTxt }}</span>
                                            @endif
                                        </td>
                                        <td>{{ $createdOut }}</td>
                                        @if($hasAnyAction)
                                                ...
                                                @if($isActive)
                                                    <div class="{{ VC::ACT_BTN_WRN }}">
                                                        <a href="{{ $reqUrl }}" class="{{ VC::BT_SM_CT }} route-guard copy-link"
                                                           data-url="{{ $reqUrl }}" data-guard-msg="{{ base64_encode($requirementGuard) }}"
                                                ...
                                                @if($canShow)
                                                    <div class="{{ VC::ACT_BTN_INF }}">
                                                        <a href="{{ $showUrl }}" class="{{ VC::BT_SM_CT }} route-guard"
                                                           data-url="{{ $showUrl }}" data-guard-msg="{{ base64_encode($showGuard) }}"
                                                ...
                                                @if($canEdit)
                                                    <div class="{{ VC::ACT_BTN_PRIM }}">
                                                        <a href="{{ $editUrl }}" class="{{ VC::BT_SM_CT }} route-guard"
                                                           data-url="{{ $editUrl }}" data-guard-msg="{{ base64_encode($editGuard) }}"
                                                    ...
                                                    <div class="{{ VC::ACT_BTN_DNG_2 }}">
                                                        {!! Collective\Html\FormFacade::open(['method'=>'DELETE','route'=>[$destroyBase,$jid],'id'=>'delete-form-'.$jid,'data-url'=>$destroyUrl,'data-guard-msg'=>$destroyGuard]) !!}
                                                            <a href="#"
                                                               ...
                                                               data-url="{{ $destroyUrl }}" data-guard-msg="{{ base64_encode($destroyGuard) }}"
                                ...
                                @endforeach
                            @else
                                <tr><td colspan="{{ $hasAnyAction ? 7 : 6 }}" class="{{ VC::TXCT }}">{{ __('No jobs available') }}</td></tr>
                            @endif
                        </tbody>
                    </table>
                </div>
