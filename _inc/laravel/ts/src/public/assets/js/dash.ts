/**
 * @fileoverview TypeScript version of public/assets/js/dash.js
 * @generated from original JavaScript - manual review recommended
 * @module dash
 */
/* eslint-disable @typescript-eslint/explicit-function-return-type, @typescript-eslint/no-unsafe-argument, @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-call, @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unused-vars, @typescript-eslint/prefer-for-of, no-console */
...
document.addEventListener("DOMContentLoaded", function (): void {
  ...
  if (!document.querySelector<HTMLElement>("body")?.classList.contains("dash-horizontal")) {
    addscroller();
  }
  if (document.querySelector<HTMLElement>("body")?.classList.contains("dash-horizontal")) {
    if (
  ...
  const hamburger = document.querySelector<HTMLElement>(".hamburger:not(.is-active)");
  if (hamburger) {
    hamburger.addEventListener("click", function (): void {
      if (
        document.querySelector<HTMLElement>(".hamburger")?.classList.contains("is-active")
      ) {
        document.querySelector<HTMLElement>(".hamburger")?.classList.remove("is-active");
      } else {
        document.querySelector<HTMLElement>(".hamburger")?.classList.add("is-active");
      }
    });
  }
  ...
  const verticalnavtoggle = document.querySelector<HTMLElement>("#vertical-nav-toggle");
  ...
  const mobilecollapsever = document.querySelector<HTMLElement>("#mobile-collapse");
  if (mobilecollapsever) {
    mobilecollapsever.addEventListener("click", function (): void {
      if (
        !document.querySelector<HTMLElement>("body")?.classList.contains("dash-horizontal")
      ) {
        // menuclick();
      }
      const tempsdbr = document.querySelector<HTMLElement>(".dash-sidebar");
      const tem = document.querySelector<HTMLElement>("body");
      if (tem) {
        if (document.querySelector<HTMLElement>("body")?.classList.contains("no-scroll")) {
          rmmenu();
        } else {
          document.querySelector<HTMLElement>("body")?.classList.add("no-scroll");
        }
      }
      if (tempsdbr) {
        if (
  ...
  // Menu collapse click start
  const mobilecollapse = document.querySelector<HTMLElement>(".dash-horizontal #mobile-collapse");
  if (mobilecollapse) {
    mobilecollapse.addEventListener("click", function (): void {
      if (
        ...
        document.querySelector<HTMLElement>(".topbar")?.classList.add("mob-sidebar-active");
        document
          .querySelector(".topbar")
          .insertAdjacentHTML(
            "beforeend",
  ...
  const topbarlinklist = document.querySelector<HTMLElement>(".dash-horizontal .topbar .dash-navbar>li>a");
  if (topbarlinklist) {
  ...
  if (document.querySelector<HTMLElement>("body")?.classList.contains("minimenu")) {
    collapseedge();
  }
  // notification scrollbar start
  if (document.querySelector<HTMLElement>(".drp-notification .noti-body")) {
    const px = new PerfectScrollbar(".drp-notification .noti-body", {
      wheelSpeed: 0.5,
      swipeEasing: 0,
...
function addscroller() {
  ...
  if (document.querySelector<HTMLElement>(".navbar-content")) {
    const px = new PerfectScrollbar(".navbar-content", {
      wheelSpeed: 0.5,
      swipeEasing: 0,
      // eslint-disable-next-line @typescript-eslint/strict-boolean-expressions
      suppressScrollX: !0,
...
function rmmenu() {
  ...
  const tem = document.querySelector<HTMLElement>("body");
  if (tem) {
    document.querySelector<HTMLElement>("body")?.classList.remove("no-scroll");
  }
  if (tempov) {
    document
      .querySelector(".dash-sidebar")
  ...
  if (document.querySelector<HTMLElement>(".topbar")) {
    document.querySelector<HTMLElement>(".topbar")?.classList.remove("mob-sidebar-active");
  }

  document.querySelector<HTMLElement>(".dash-sidebar .dash-menu-overlay")?.remove();
...
function rmactive() {
  ...
  document.querySelector<HTMLElement>(".topbar .dropdown")?.classList.remove("show");
  document.querySelector<HTMLElement>(".topbar .dropdown-menu")?.classList.remove("show");
  document.querySelector<HTMLElement>(".dash-sidebar .dash-menu-overlay")?.remove();
  document.querySelector<HTMLElement>(".topbar .dash-menu-overlay")?.remove();
}
...
function scrolltargetmenu(value) {
  document.addEventListener("DOMContentLoaded", function (): void {
    if (document.querySelector<HTMLElement>(".navbar-content")) {
      ...
      const t = off.top;
      if (t > 300) {
        document.querySelector<HTMLElement>(".navbar-content")?.scrollTop = t - 300;
      }
    }
  });
}
...
const tablayclick = document.querySelector<HTMLElement>(".tab-sidemenu > ul > li");
if (tablayclick) {
...
// nested Layout start
const pctogglesidemenu = document.querySelector<HTMLElement>(".dash-toggle-sidemenu");
if (pctogglesidemenu) {
  pctogglesidemenu.addEventListener("click", function (): void {
    if (
      ...
      document.querySelector<HTMLElement>(".dash-sideoverlay")?.classList.add("active");
      document.querySelector<HTMLElement>(".page-sidebar")?.classList.add("active");
      document.querySelector<HTMLElement>(".dash-toggle-sidemenu")?.classList.add("active");
    } else {
      document.querySelector<HTMLElement>(".dash-sideoverlay")?.classList.remove("active");
      document.querySelector<HTMLElement>(".page-sidebar")?.classList.remove("active");
      document
        .querySelector(".dash-toggle-sidemenu")
        .classList.remove("active");
    }
...
const pcovelayclk = document.querySelector<HTMLElement>(".dash-sideoverlay, .dash-toggle-sidemenu.active");
if (pcovelayclk) {
  pcovelayclk.addEventListener("click", function (): void {
    document.querySelector<HTMLElement>(".dash-sideoverlay")?.classList.remove("active");
    document.querySelector<HTMLElement>(".page-sidebar")?.classList.remove("active");
    document.querySelector<HTMLElement>(".dash-toggle-sidemenu")?.classList.remove("active");
  });
}
// nested Layout end

if (document.querySelector<HTMLElement>("body")?.classList.contains("layout-topbar")) {
...
if (document.querySelector<HTMLElement>("body")?.classList.contains("dash-horizontal")) {
  ...
  if (docW > 1024) {
    const topbarhasmenu = document.querySelector<HTMLElement>(".dash-horizontal .topbar .dash-submenu .dash-hasmenu");
    if (topbarhasmenu) {
      topbarhasmenu.addEventListener(
        ...
        function (): void {
          hpx.destroy();
          document.querySelector<HTMLElement>(".scroll-menu")?.removeAttribute("style");
          document
...
function collapseedge() {
  ...
  const docW = window.innerWidth;
  if (docW > 1024) {
    const minimenuhasmenu = document.querySelector<HTMLElement>(".minimenu .dash-sidebar .dash-submenu .dash-hasmenu");
    if (minimenuhasmenu) {
...
const tc = document.querySelectorAll(".prod-likes .form-check-input");
for (let t = 0; t < tc.length; t++) {
  let prodlike = tc[t];
...
const slideToggle = (target, duration = 0) => {
  if (window.getComputedStyle(target).display === "none") {
    slideDown(target, duration); return;
  } else {
    slideUp(target, duration); return;
  }
};
// =======================================================
// =======================================================
