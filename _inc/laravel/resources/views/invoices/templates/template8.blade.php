...
$settings_data = Utility::settingsById($invoice[DatabaseConstants::COL_TABLE_CREATOR] ?? null);
        ...
        $fmtInvoiceNo = function ($settings, $id) use ($hasInvoiceNumberFormat) { return $hasInvoiceNumberFormat ? Utility::invoiceNumberFormat($settings, $id) : (string) $id; };
        ...
        $fmtPrice = function ($settings, $amount) use ($hasPriceFormat) { return $hasPriceFormat ? Utility::priceFormat($settings, $amount) : number_format((float) $amount, 2); };
        ...
        \Log::error('invoices/templates/template8 — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
    }
@endphp
<!DOCTYPE html>
