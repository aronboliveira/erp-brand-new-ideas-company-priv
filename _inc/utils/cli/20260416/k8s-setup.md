...
## git
git add .gitignore _inc/laravel/{.gitignore,Dockerfile,azure-pipelines.yml,composer.json,k8s/,scripts/k8s-deploy.sh} .github/workflows/ci.yml _inc/utils/containers/
git commit -m "infra: K8s local deploy (minikube/kubectl), GitHub Actions CI/CD, Azure triggers desativados"

## test suites (resultados em _inc/utils/logs/20260416/test-results.md)
...
mkdir -p _inc/utils/containers/{minikube,kubectl,docker}
mkdir -p .backup
