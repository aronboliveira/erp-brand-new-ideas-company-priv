...
document.addEventListener("DOMContentLoaded", function (): void {
  ...
  if (document.querySelector<HTMLElement>(".drp-notification .noti-body")) {
    const _px = new PerfectScrollbar(".drp-notification .noti-body", {
      wheelSpeed: 0.5,
      swipeEasing: false,
...
function addscroller(): void{
  ...
  if (document.querySelector<HTMLElement>(".navbar-content")) {
    const _px = new PerfectScrollbar(".navbar-content", {
      wheelSpeed: 0.5,
      swipeEasing: false,
      suppressScrollX: true,
      wheelPropagation: true,
  ...
  if (
    ...
    for (let i = 0; i < pclinkclick.length; i++) {
      pclinkclick[i].addEventListener("click", function (event: Event) {
        ...
        const parent = targetElement?.parentNode as HTMLElement | null;
        if (parent?.classList.contains("dash-trigger")) {
          parent.classList.remove("dash-trigger");
          slideUp(parent.children[1] as HTMLElement, 200);
        } else {
          const tc = document.querySelectorAll("li.dash-trigger");
          // eslint-disable-next-line @typescript-eslint/prefer-for-of
          for (let t = 0; t < tc.length; t++) {
            const c = tc[t] as HTMLElement;
            c.classList.remove("dash-trigger");
          ...
          if (tmp) {
            slideDown(parent?.children[1] as HTMLElement, 200);
          }
        }
      });
    }
        ...
        if (parent?.classList.contains("dash-trigger")) {
          ...
          if (tc) {
            ...
            for (let t = 0; t < tc.length; t++) {
              ...
              const cParent = c.parentNode as HTMLElement | null;
              if (cParent?.classList.contains("dash-hasmenu")) {
                slideUp(cParent.children[1] as HTMLElement, 200);
              }
            }
          }
          parent?.classList.add("dash-trigger");
          const tmp = parent?.children[1] as HTMLElement | undefined;
          if (tmp) {
...
if (emailmorelink) {
  emailmorelink.addEventListener(
    ...
    function (this: HTMLElement, _e: Event) {
      const children = this.querySelectorAll<HTMLElement>("span");
...
for (let l = 0; l < elem.length; l++) {
  ...
  if (anchor.href == pageUrl && anchor.getAttribute("href") != "") {
    parent?.classList.add("active");
    scrolltargetmenu(parent);
    const grandParent = parent?.parentNode?.parentNode as HTMLElement | null;
    grandParent?.classList.add("active");
    grandParent?.classList.add("dash-trigger");
    const parentParent = parent?.parentNode as HTMLElement | null;
    if (parentParent) parentParent.style.display = "block";

    const greatGrandParent = grandParent?.parentNode
      ?.parentNode as HTMLElement | null;
    greatGrandParent?.classList.add("active");
    greatGrandParent?.classList.add("dash-trigger");
    const ggpParent = grandParent?.parentNode as HTMLElement | null;
    if (ggpParent) ggpParent.style.display = "block";
  ...
  for (let t = 0; t < tc.length; t++) {
    ...
    c.addEventListener("click", function (event: Event) {
      ...
      if (targetElement?.tagName == "I") {
        targetElement =
          (targetElement.parentNode?.parentNode as HTMLElement | null) ?? null;
      }
...
function showmenu(event: Event): void{
  (event.target as HTMLElement).children[1].classList.add("show");
}

function hidemenu(event: Event): void{
  (event.target as HTMLElement).children[1].classList.remove("show");
}
// topbar Layout end
...
if (
  ...
  let hpx: PerfectScrollbar | undefined;
    ...
    if (topbarhasmenu) {
      topbarhasmenu.addEventListener(
        ...
        function (this: HTMLElement): void {
          const elm = this.children[1] as HTMLElement | undefined;
          if (!elm) return;
          const off = elm.getBoundingClientRect();
          const l = off.left;
          const t = off.top;
  ...
  if (docW > 1024) {
    ...
    if (minimenuhasmenu) {
      minimenuhasmenu.addEventListener("mouseenter", function (event: Event) {
        ...
        const elm = (targetElement?.children[1] as HTMLElement) ?? null;
        if (!elm) return;
        const off = elm.getBoundingClientRect();
        const _l = off.left;
        const t = off.top;
...
const tc = document.querySelectorAll<HTMLInputElement>(
  ".prod-likes .form-check-input",
);
// eslint-disable-next-line @typescript-eslint/prefer-for-of
for (let t = 0; t < tc.length; t++) {
  let prodlike: Element = tc[t];
  prodlike.addEventListener("change", function (event: Event) {
    ...
    if (currentTarget?.checked) {
      prodlike = event.target as Element;
