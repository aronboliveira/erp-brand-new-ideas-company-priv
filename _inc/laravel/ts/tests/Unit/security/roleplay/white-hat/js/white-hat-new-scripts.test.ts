...
describe("White Hat — Crypto Verifier (WASM)", () => {
  ...
  const { verifyHashFormat, verifyJwtIntegrity, verifyCsrfTokens } = require(path.join(SCRIPTS, "wasm/scripts/crypto_verifier"));
...
describe("White Hat — CSP Bypass Tester", () => {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const { CSP_BYPASS_VECTORS, parseCSP, analyzeBypasses } = require(path.join(SCRIPTS, "js/scripts/csp_bypass_test"));
