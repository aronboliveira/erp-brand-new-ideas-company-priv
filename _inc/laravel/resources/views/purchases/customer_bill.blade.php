...
<html lang="{{ str_replace('_', '-', is_string(app()->getLocale()) ? app()->getLocale() : DatabaseConstants::DEFAULT_LANG) }}" dir="{{ $siteRtl === 'on' ? 'rtl' : 'ltr' }}">
    ...
    <body class="{{ $color }}">
        ...
        <div class="{{ ViewClassNamesConstants::MCT_CT }}">
            <div class="row justify-content-between align-items-center mb-3">
                <div class="col-md-12 d-flex align-items-center justify-content-between justify-content-md-end">
                    <div class="all-button-box mx-2">
                        <a href="{{ route(ViewsConstants::PRC.'.pdf', \Crypt::encrypt($purchase->id))}}" target="_blank" class="btn btn-primary mt-3">
                            {{__('Download')}}
                        </a>
                    </div>
                </div>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col">
                                            <div class="float-end mt-3">
                                                {!! DNS2D::getBarcodeHTML(route(ViewsConstants::BIL.'.link.copy',\Illuminate\Support\Facades\Crypt::encrypt($purchase->id)), "QRCODE",2,2) !!}
                                            </div>
                                        </div>
                                    </div>
                                    <div class="row mt-3">
                                        <div class="col">
                                            <small>
                                                ...
                                                @if($purchase->status == 0)
                                                    <span class="badge bg-primary">{{ __(\App\Models\Bill::$statuses[$purchase->status]) }}</span>
                                                @elseif($purchase->status == 1)
                                                    <span class="badge bg-warning">{{ __(\App\Models\Bill::$statuses[$purchase->status]) }}</span>
                                                @elseif($purchase->status == 2)
                                                    <span class="badge bg-danger">{{ __(\App\Models\Bill::$statuses[$purchase->status]) }}</span>
                                                @elseif($purchase->status == 3)
                                                    <span class="badge bg-info">{{ __(\App\Models\Bill::$statuses[$purchase->status]) }}</span>
                                                @elseif($purchase->status == 4)
                                                    <span class="badge bg-success">{{ __(\App\Models\Bill::$statuses[$purchase->status]) }}</span>
                                                @endif
                                            </small>
                                        </div>
