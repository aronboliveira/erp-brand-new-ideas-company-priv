...
@push(StacksConstants::ADM_SCR_PG)
    ...
    <script async>
        (function(){
            ...
            const renderBarChart=(selector,options)=>{const target=qs(selector);if(!target){return;}if(typeof window.ApexCharts!=="function"){try{console.error("ApexCharts unavailable");}catch(_){ }scheduleInteractiveError(getMsgFor(target,"plugin_unavailable"));return;}try{target.innerHTML="";const chart=new window.ApexCharts(target,options);chart.render();}catch(_){scheduleInteractiveError(getMsgFor(target,"chart_unavailable"));}};
            ...
            const init=()=>{if(!window.jQuery){try{console.error("jQuery unavailable");}catch(_){ }}initDynamic();initStaticCharts();};
