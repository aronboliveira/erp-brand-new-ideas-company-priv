...
use App\Traits\{HasNullableAuditColumns, HasPaymentRequestColumns, TracksFailures};
use Illuminate\Database\{Migrations\Migration, Schema\Blueprint};
use Illuminate\Support\Facades\{Log, Schema};

class CreateInvoicesTable extends Migration
{
    use HasNullableAuditColumns, HasPaymentRequestColumns, TracksFailures;
    ...
    public function up(): void
    {
        Schema::create(self::TABLE, function (Blueprint $table) {
            ...
            $table->uuid(BC::COL_BL_ID)->index()->nullable(); // ? nullable for tests
            $table->uuid(BC::COL_CST_ID)->index();
            $table->uuid(BC::COL_TAX_ID)->nullable();
            $table->date(BC::COL_ISS_DT)->default(now()->format('Y-m-d'))->index();
