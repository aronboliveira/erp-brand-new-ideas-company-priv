...
class ReportController extends Controller
{
    ...
    public function billSummary(Request $request)
    {
        ...
        if (\Auth::user()->can('bill report')) {
            ...
            return view('report.bill_report', compact('bills', 'vendor', 'status', 'totalBill', 'totalDueBill', 'totalPaidBill', 'billTotal', 'monthList', 'filter'));
        } else {
            return redirect()->back()->with('error', __('Permission Denied.'));
        }
    }
            ...
            foreach ($employees as $id => $employee) {
                ...
                foreach ($dates as $date) {
                    ...
                    if ($dateFormat <= date('Y-m-d')) {
                        $employeeAttendance = EmployeeAttendance::where('employee_id', $id)->where('date', $dateFormat)->first();
    ...
    public function purchaseDailyReport(Request $request)
    {
        if (\Auth::user()->can('manage pos')) {
            ...
            $vendors = Vendor::where('id', '=', $request->vendor)->where('created_by', \Auth::user()->creatorId())->first();
    ...
    public function PayablesReport(Request $request)
    {
        ...
        $payableSummariesDebit = DebitNote::select('vendors.name')
        ...
        $payableDetailsDebit = DebitNote::select('vendors.name')
            ->selectRaw('null as bill')
