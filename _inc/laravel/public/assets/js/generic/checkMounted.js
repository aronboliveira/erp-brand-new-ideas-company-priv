...
(() => {
    const checkMounted = () => {
        const isModal = !document.body ||
            document.querySelector(".modal-body, .modal-header, .modal-content"), targetElement = isModal
            ? document.documentElement ||
                document.querySelector("form, div")
            : document.body;
        if (!targetElement)
            return;
        const content = targetElement.innerHTML || "", m = content.match(/^[\s\n\t\r]*[0-9]+</);
        if (!m)
            return;
        ...
        if (document.getElementById("manual-recovery-overlay"))
            return;
