...
<div class="row">
    ...
    @foreach($performance_types as $performance_type)
        <div class="col-md-12 mt-3"><h6>{{ $performance_type->name }}</h6><hr class="mt-0"></div>
                    ...
                    <fieldset class="rating">
                        @foreach($starOptions as $value => $label)
                            <input class="stars" type="radio"
                                   ...
                                   name="{{ $field }}[{{ $type->id }}]" value="{{ $value }}"
                                   {{ (isset($$field[$type->id]) && $$field[$type->id] == $value) ? 'checked' : '' }} disabled>
                            <label class="full"
