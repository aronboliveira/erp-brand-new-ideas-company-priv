...
class EmployeeExport:
  def __init__(self, employees_data, company_name='ERP Inc'):
      ...
      for emp in self.employees_data:
        try:
          temp.append({
            ...
            "Phone Number": emp.get("phone_number",""),
            "Address": emp.get("address",""),
            "Email ID": emp.get("email",""),
            "Branch": emp.get("branch_id","-"),
            "Department": emp.get("department_id","-"),
            "Designation": emp.get("designation_id","-"),
            "Date of Join": emp.get("date_of_join",""),
            "Account Holder Name": emp.get("account_holder_name",""),
            "Account Number": emp.get("account_number",""),
            "Bank Name": emp.get("bank_name",""),
            "Bank Identifier Code": emp.get("bank_identifier_code",""),
            "Branch Location": emp.get("branch_location",""),
            "Salary": emp.get("salary","")
          })
        except KeyError as e:
          logger.error(f"KeyError in format_data for employee: {str(e)}")
        except Exception as e:
          logger.error(f"Unexpected error formatting employee: {str(e)}")
  ...
  def export_to_excel(self, filename="employee_export.xlsx"):
    try:
      if self.df.empty:
        logger.warning("DataFrame is empty no Excel generated.")
        return None
      wb = Workbook()
      ws = wb.active
      ws.title = "Employees"
      ...
      if cols_count>0:
        ...
        ws["A1"] = f"Employee Export - {self.company_name}"
