...
class ProductStockExport:
    ...
    try:
      ...
      for record in self.stock_data:
        try:
          ...
          logger.error(f"KeyError in format_data stock record: {str(e)}")
        except Exception as e:
          logger.error(f"Unexpected error formatting stock record: {str(e)}")
  ...
  def dataframe(self):
    try:
      if not self.formatted_data:
        logger.warning("No data for DataFrame creation in ProductStockExport.")
        return pd.DataFrame()
  ...
  def export_to_excel(self, filename="product_stock_export.xlsx"):
    try:
      if self.df.empty:
        logger.warning("Empty DataFrame skipping Excel generation.")
        return None
      wb = Workbook()
      ws = wb.active
