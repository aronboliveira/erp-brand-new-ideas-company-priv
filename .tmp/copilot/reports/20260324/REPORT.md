...
### 1.1 Curl Route Scan — 4xx & Timeouts (18 issues)

377 static GET routes tested with authenticated curl session.

#### 4xx Client Errors (13 routes)

| # | Route | HTTP | Root Cause |
|---|-------|------|------------|
| 1 | `/debit_notes/bill` | 422 | Requires `bill_id` query param; endpoint validates input |
| 2 | `/email_templates/create` | 404 | Route may be disabled or renamed in current config |
| 3 | `/install` | 404 | Installer disabled post-installation (expected) |
| 4 | `/installs/database` | 404 | Installer disabled post-installation (expected) |
| 5 | `/installs/environment` | 404 | Installer disabled post-installation (expected) |
| 6 | `/installs/environments/classic` | 404 | Installer disabled post-installation (expected) |
| 7 | `/installs/environments/wizard` | 404 | Installer disabled post-installation (expected) |
| 8 | `/installs/final` | 404 | Installer disabled post-installation (expected) |
| 9 | `/installs/permissions` | 404 | Installer disabled post-installation (expected) |
| 10 | `/installs/requirements` | 404 | Installer disabled post-installation (expected) |
| 11 | `/leaves/export` | 404 | Export route likely needs POST or specific params |
| 12 | `/pos/create` | 404 | POS create uses modal, not a dedicated page |
| 13 | `/purchases/items` | 422 | Requires product/purchase context; validates input |

**Actionable 4xx (excluding installer/expected):** 4 routes
- `/debit_notes/bill` — needs query param, not a bug
- `/email_templates/create` — **investigate**: may be missing controller action
- `/leaves/export` — needs POST, not a GET route bug
- `/purchases/items` — needs context, not a bug
