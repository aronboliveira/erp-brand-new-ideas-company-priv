...
test.describe("Security API severity checks", (): void => {
  test("sensitive finance routes should not be public for anonymous requests", async ({
    request,
  }) => {
    const violations = [];
  ...
  test("framework debug and ignition endpoints should not be reachable", async ({
    request,
  }) => {
    const paths = [
        "/_debugbars/open",
    ...
    for (const path of paths) {
      const method = path.includes("update-config") ? "post" : "get",
        res = await request[method](`${BASE_URL}${path}`, {
  ...
  test("mutating webhook/payment endpoints should reject: (reason?: unknown) => void missing auth and CSRF", async ({
    request,
  }) => {
    const postTargets = [
        "/payment-i-p-n",
