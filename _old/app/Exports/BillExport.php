...
class BillExport implements FromCollection, WithHeadings
{
    ...
    public function collection()
    {
        ...
        foreach($data as $k => $bill)
        {
            ...
            unset( $bill->id,$bill->vendor_id,$bill->created_by, $bill->shipping_display,$bill->discount_apply,$bill->created_at,$bill->updated_at);
            $data[$k]["bill_id"] = \Auth::user()->billNumberFormat($bill->bill_id);
//            $data[$k]["vendor_id"] = \Auth::user()->vendorNumberFormat($bill->vendor_id);
