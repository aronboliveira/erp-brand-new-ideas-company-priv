...
$lang = Utility::fetchUserLang();
        ...
        \Log::error('competencies/index — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
    }
@endphp
...
@section(YieldingConstants::ADM_BDC)
    ...
    <li class="{{ VC::BCI }}">{{ __('Competencies') }}</li>
@endsection
...
@endphp
                                                <div class="{{ VC::ACT_BTN_PRIM }}">
                                                    <a href="#" class="{{ VC::DFL_IL }} {{ VC::ALC }}" id="editCompetencyBtn_{{ $cid ?: 'na' }}" data-url="{{ $editRoute }}" data-guard-msg="{{ base64_encode($editGuardMsg) }}" data-listener-alias="edit-competency" data-ajax-popup="true" data-title="{{ __('Edit Competencies') }}" data-bs-toggle="tooltip" title="{{ __('Edit') }}"><i class="{{ VC::TI_PC_WT }}"></i></a>
                                                </div>
                                            @endcan
