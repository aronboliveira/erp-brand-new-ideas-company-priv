...
class ContractNote extends Model
{
    ...
    public function getNotesPartsAttribute(): array
    {
        try {
            ...
            Log::error(static::class . '::getNotesPartsAttribute — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
            return [];
        }
    }
    ...
    public function getNotesHasAttachmentsAttribute(): bool
    {
        try {
            ...
            Log::error(static::class . '::getNotesHasAttachmentsAttribute — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
            return false;
        }
    }
    ...
    public static function countForContractCached(string $contractId, int $ttlSeconds = 300): int
    {
        try {
            ...
            Log::error(static::class . '::countForContractCached — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
            return 0;
        }
    }
    ...
    private static function normalizeNotes(self $m): void
    {
        try {
            ...
            Log::error(static::class . '::normalizeNotes — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
        }
    }
    ...
    private static function normalizeNotePart(string $value, self $m): ?string
    {
        try {
            ...
            Log::error(static::class . '::normalizeNotePart — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
            return '';
        }
    }
    ...
    private static function resolveUuidToNoteText(string $uuid): ?string
    {
        try {
            ...
            Log::error(static::class . '::resolveUuidToNoteText — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
            return '';
        }
    }
    ...
    private static function extractNoteTextFromRow(object $row): ?string
    {
        try {
            ...
            Log::error(static::class . '::extractNoteTextFromRow — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
            return '';
        }
    }
    ...
    private static function notesLooksLikeList(string $value): bool
    {
        try {
            ...
            Log::error(static::class . '::notesLooksLikeList — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
            return false;
        }
    }
}
