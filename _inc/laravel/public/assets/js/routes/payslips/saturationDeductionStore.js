...
(() => {
    try {
        const forms = Array.from(document.querySelectorAll('form[id^="saturation-deduction-store-form-"][data-url][data-guard-msg]')) ?? [];
        if (forms.length === 0)
            return;
        forms.forEach(fm => {
            try {
                ...
                fm.addEventListener("submit", (e) => {
                    try {
                        ...
                        console.error(`[saturationDeductionStore] Error:`, err);
                    }
                });
            }
            catch (err) {
...
//# sourceMappingURL=saturationDeductionStore.js.map