...
use App\Models\{Bill, CreditNote, Invoice};
		...
		DB::transaction(function () use (
			...
			for ($i = 1; $i <= self::COUNT; $i++) {
				try {
					...
					if (!empty($invoices) && fake()->boolean((int) round(self::INVOICE_RATIO * 100))) {
					...
					if (!$invoiceId && !$billId) {
						Log::warning('CreditNoteSeeder: No valid invoice or bill found for iteration ' . $i);
						continue;
					}
