        ...
        $(document).on("click", "#pay_with_flutterwave", function () {
            ...
            $('#flutterwave-payment-form').ajaxForm(function (res) {
                ...
                if (res.flag == 1) {
                    ...
                    var flutter_callback = "{{ url('/plan/flutterwave') }}";
                    var x = getpaidSetup({
...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item">
        <a href="{{ Route::has(ViewsConstants::PLN.'.index') ? route(ViewsConstants::PLN.'.index') : "#"}}">
            {{__('Plan')}}
        </a>
    </li>
                    ...
                    <div class="sticky-top" style="top:30px">
                            ...
                            <div class="card price-card price-1 wow animate__fadeInUp" data-wow-delay="0.2s" style="
                                ...
                                <div class="card-body">
                                    ...
                                    <h3 class="mb-4 f-w-600  ">
                                        {{(isset($admin_payment_setting['currency_symbol'])) ? $admin_payment_setting['currency_symbol'] : '$'}}{{ $plan[PlansConstants::COL_PC] . ' / ' . __(\App\Models\Plan::$arrDuration[$plan[PlansConstants::COL_DUR]]) }}</small>
                                        </small>
                                    </h3>
                                    @php
                                        $quotaItems = [
                                        ...
                                        @foreach($quotaItems as $item)
                                            <li>
                                                <span class="theme-avatar">
                                                    <i class="{{ ViewClassNamesConstants::TI_CC_PLS }}"></i>
                                                </span>
                                                {{ $plan->{ $item['key'] } === -1 ? __('Unlimited') : $plan->{ $item['key'] } }}
                                                {{ $item['label'] }}
                                            </li>
                                        @endforeach
                                    </ul>
                        ...
                        <div class="card">
                            @php
                                ...
                                $generic = [
                                    'flutterwave','razorpay','mercado'=>'Mercado Pago','paytm','mollie',
                                    'skrill','coingate','paymentwall','toyyibpay'=>'Toyyibpay','payfast',
                                    'iyzipay'=>'Iyzipay','sspay'=>'SSPay','paytab'=>'Paytab','benefit'=>'Benefit',
                                    'cashfree'=>'Cashfree','aamarpay'=>'AamarPay','paytr'=>'PayTR','yookassa'=>'Yookassa',
                                    'midtrans'=>'Midtrans','xendit'=>'Xendit'
                                ];
                                foreach($generic as $key => $label){
                                    if(is_int($key)){
                                        $methodKey  = $label;
                                        $methodLabel = __($label);
                                    } else {
                                        $methodKey  = $key;
                                        $methodLabel = __($label);
                                    }
                            ...
                            <div class="{{ ViewClassNamesConstants::CD_STK }}" style="top:30px">
                                <div class="{{ ViewClassNamesConstants::LG_FLSH }}" id="useradd-sidenav">
                                    @foreach($methods as $method)
                                        @php
                                            $enabled = ($admin_payment_setting[$method['enabled']] ?? null) === 'on';
                                            $hasExtras = true;
                                            foreach($method['extra'] as $extraKey){
                                                if(empty($admin_payment_setting[$extraKey])){
                                                    $hasExtras = false;
                                                    break;
                                                }
                                            }
                                        ...
                                        @if($enabled && $hasExtras)
                                            <a href="#{{ $method['id'] }}"
                                            class="{{ ViewClassNamesConstants::LGI_ACT_NBD }}{{ $method['active'] ? ' active' : '' }}">
                                                {{ $method['label'] }}
                                                <div class="float-end">
                                ...
                                <div class="col-sm-12 my-2 px-2">
                                    <div class="text-end">
                                        @if($plan->id !== DatabaseConstants::DEFAULT_PLAN && $plan->id !== $user[UsersConstants::COL_PL])
                                            ...
                                            @else
                                                <a href="{{ route('request.cancel',$user->id) }}" class="btn btn-danger mb-2 me-3"
                    ...
                    @if ($admin_payment_setting['is_bank_transfer_enabled'] == 'on' && !empty($admin_payment_setting['bank_details']))
                        <div id="bank_payment" class="card">
                            ...
                            <div class="tab-pane {{ ($admin_payment_setting['is_bank_transfer_enabled'] == 'on' &&!empty($admin_payment_setting['bank_details'])) == 'on'? 'active': '' }}" id="bank_payment">
                                <form role="form" action="{{ route(ViewsConstants::PLN.'.pay.with.bank') }}" method="post" class="require-validation" id="bank-payment-form" enctype = "multipart/form-data">
                                    @csrf
                                    <div class="border p-3 rounded bank-payment-div">
                                        <div class="row">
                                            ...
                                            <div class="col-6">
                                                {{ Collective\Html\FormFacade::label('payment_receipt', __('Payment Receipt'), ['class' => 'form-label']) }}
                                        ...
                                        <div class="row">
                                            <div class="col-6 ">
                                                <div class="custom-radio">
                                                    ...
                                                    {{(isset($admin_payment_setting['currency_symbol'])) ? $admin_payment_setting['currency_symbol'] : '$'}}{{ $plan[PlansConstants::COL_PC]}}</small>
                                                    </small>
                                                </div>
                                            </div>
                                            <div class="col-6">
                                                <div class="custom-radio">
                                                    ...
                                                    <span class="final-price">{{(isset($admin_payment_setting['currency_symbol'])) ? $admin_payment_setting['currency_symbol'] : '$' }}{{ $plan[PlansConstants::COL_PC] }}</span>
                                                </div>
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" name="name" id="card-name-on" class="form-control required" placeholder="{{ $user[UsersConstants::COL_NM] }}">
                                                </div>
                                            </div>
                                            <div class="col-md-12">
                        ...
                        <div id="paypal_payment" class="card">
                            ...
                            <div class="tab-pane {{ ($admin_payment_setting['is_stripe_enabled'] != 'on' && $admin_payment_setting['is_paypal_enabled'] == 'on' &&!empty($admin_payment_setting['paypal_client_id']) &&!empty($admin_payment_setting['paypal_secret_key'])) == 'on'? 'active': '' }}" id="paypal_payment">
                                <form class="w3-container w3-display-middle w3-card-4" method="POST" id="payment-form"
                                      action="{{ route(ViewsConstants::PLN.'.pay.with.paypal') }}">
                                    @csrf
                                    <input type="hidden" name="plan_id" value="{{ \Illuminate\Support\Facades\Crypt::encrypt($plan->id) }}">
                            ...
                            <div id="paystack-payment" class="tabs-card">
                                <div class="">
                                    <form class="w3-container w3-display-middle w3-card-4" method="POST" id="paystack-payment-form"
                                          action="{{ route(ViewsConstants::PLN.'.pay.with.paystack') }}">
                                        @csrf
                                        <input type="hidden" name="plan_id" value="{{ \Illuminate\Support\Facades\Crypt::encrypt($plan->id) }}">
                    ...
                    @if (isset($admin_payment_setting['is_flutterwave_enabled']) && $admin_payment_setting['is_flutterwave_enabled'] == 'on')
                        <div id="flutterwave_payment" class="card">
                            ...
                            <div class="tab-pane " id="flutterwave_payment">
                                <form class="w3-container w3-display-middle w3-card-4" method="POST" id="flutterwave-payment-form" action="{{ route(ViewsConstants::PLN.'.pay.with.flutterwave') }}">
                                    @csrf
                                    <input type="hidden" name="plan_id" value="{{ \Illuminate\Support\Facades\Crypt::encrypt($plan->id) }}">
                                    ...
                                    <div class="col-sm-12 my-2 px-2">
                                        <div class="text-end">
                                            <input id="pay_with_flutterwave" type="button" value="{{ __('Pay Now') }}" class="btn btn-primary mb-2 me-3">
                                        </div>
                                    </div>
                                </form>
                            </div>
                    ...
                    @if (isset($admin_payment_setting['is_razorpay_enabled']) && $admin_payment_setting['is_razorpay_enabled'] == 'on')
                        <div id="razorpay_payment" class="card">
                            ...
                            <div class="tab-pane " id="razorpay_payment">
                                <form class="w3-container w3-display-middle w3-card-4" method="POST" id="razorpay-payment-form" action="{{ route(ViewsConstants::PLN.'.pay.with.razorpay') }}">
                                    @csrf
                                    <input type="hidden" name="plan_id" value="{{ \Illuminate\Support\Facades\Crypt::encrypt($plan->id) }}">
                    ...
                    @if (isset($admin_payment_setting['is_mercado_enabled']) && $admin_payment_setting['is_mercado_enabled'] == 'on')
                        <div id="mercado_payment" class="card">
                            ...
                            <div class="tab-pane " id="mercado_payment">
                                <form class="w3-container w3-display-middle w3-card-4" method="POST" id="payment-form" action="{{ route(ViewsConstants::PLN.'.pay.with.mercado') }}">
                                    @csrf
                                    <input type="hidden" name="plan_id" value="{{ \Illuminate\Support\Facades\Crypt::encrypt($plan->id) }}">
                    ...
                    @if (isset($admin_payment_setting['is_paytm_enabled']) && $admin_payment_setting['is_paytm_enabled'] == 'on')
                        <div id="paytm_payment" class="card">
                            ...
                            <div class="tab-pane " id="paytm_payment">
                                <form role="form" action="{{ route(ViewsConstants::PLN.'.pay.with.paytm') }}" method="post" class="require-validation" id="paytm-payment-form">
                                    @csrf
                                    <input type="hidden" name="plan_id" value="{{ \Illuminate\Support\Facades\Crypt::encrypt($plan->id) }}">
                                    <input type="hidden" name="total_price" id="paytm_total_price" value="{{ $plan[PlansConstants::COL_PC] }}" class="form-control">
                    ...
                    @if (isset($admin_payment_setting['is_mollie_enabled']) && $admin_payment_setting['is_mollie_enabled'] == 'on')
                        <div id="mollie_payment" class="card">
                            ...
                            <div class="tab-pane " id="mollie_payment">
                                <form role="form" action="{{ route(ViewsConstants::PLN.'.pay.with.mollie') }}" method="post" class="require-validation" id="mollie-payment-form">
                                    @csrf
                                    <input type="hidden" name="plan_id" value="{{ \Illuminate\Support\Facades\Crypt::encrypt($plan->id) }}">
                                    <input type="hidden" name="total_price" id="mollie_total_price" value="{{ $plan[PlansConstants::COL_PC] }}" class="form-control">
                    ...
                    @if (isset($admin_payment_setting['is_skrill_enabled']) && $admin_payment_setting['is_skrill_enabled'] == 'on')
                        <div id="skrill_payment" class="card">
                            ...
                            <div class="tab-pane " id="skrill_payment">
                                <form role="form" action="{{ route(ViewsConstants::PLN.'.pay.with.skrill') }}" method="post" class="require-validation" id="skrill-payment-form">
                                    @csrf
                                    ...
                                    @endphp
                                    <input type="hidden" name="plan_id" value="{{ \Illuminate\Support\Facades\Crypt::encrypt($plan->id) }}">
                                    <input type="hidden" name="total_price" id="skrill_total_price" value="{{ $plan[PlansConstants::COL_PC] }}" class="form-control">
                    ...
                    @if (isset($admin_payment_setting['is_coingate_enabled']) && $admin_payment_setting['is_coingate_enabled'] == 'on')
                        <div id="coingate_payment" class="card">
                            ...
                            <div class="tab-pane " id="coingate_payment">
                                <form role="form" action="{{ route(ViewsConstants::PLN.'.pay.with.coingate') }}" method="post" class="require-validation" id="coingate-payment-form">
                                    @csrf
                                    <input type="hidden" name="counpon" id="coingate_coupon" value="">
                                    <input type="hidden" name="plan_id" value="{{ \Illuminate\Support\Facades\Crypt::encrypt($plan->id) }}">
                                    <input type="hidden" name="total_price" id="coingate_total_price" value="{{ $plan[PlansConstants::COL_PC] }}" class="form-control">
                                    <div class="border p-3 mb-3 rounded">
                                        <div class="row">
                                            <div class="col-md-12 mt-4">
                                                <div class="d-flex align-items-center">
                    ...
                    @if (isset($admin_payment_setting['is_paymentwall_enabled']) && $admin_payment_setting['is_paymentwall_enabled'] == 'on')
                        <div id="paymentwall_payment" class="card">
                            ...
                            <div class="tab-pane " id="paymentwall_payment">
                                <form role="form" action="{{ route(ViewsConstants::PLN.'.paymentwallpayment') }}" method="post" id="paymentwall-payment-form" class="w3-container w3-display-middle w3-card-4">
                                    @csrf
                    ...
                    @if (isset($admin_payment_setting['is_toyyibpay_enabled']) && $admin_payment_setting['is_toyyibpay_enabled'] == 'on')
                        <div id="toyyibpay_payment" class="card">
                            ...
                            <div class="tab-pane" id="toyyibpay_payment">
                                <form role="form" action="{{ route(ViewsConstants::PLN.'.toyyibpaypayment') }}" method="post"
                    ...
                    @if (isset($admin_payment_setting['is_sspay_enabled']) && $admin_payment_setting['is_sspay_enabled'] == 'on')
                        <div id="sspay_payment" class="card">
                            ...
                            <div class="tab-pane" id="sspay_payment">
                                <form role="form" action="{{ route(ViewsConstants::PLN.'.sspaypayment') }}" method="post"
                    ...
                    @if (isset($admin_payment_setting['is_paytab_enabled']) && $admin_payment_setting['is_paytab_enabled'] == 'on')
                        <div id="paytab_payment" class="card">
                            ...
                            <div class="tab-pane" id="paytab_payment">
                                <form role="form" action="{{ route(ViewsConstants::PLN.'.pay.with.paytab') }}" method="post"
                    ...
                    @if (isset($admin_payment_setting['is_benefit_enabled']) && $admin_payment_setting['is_benefit_enabled'] == 'on')
                        <div id="benefit_payment" class="card">
                            ...
                            <div class="tab-pane" id="benefit_payment">
                                <form role="form" action="{{ route(ViewsConstants::PLN.'.pay.with.benefit') }}" method="post"
                    ...
                    @if (isset($admin_payment_setting['is_cashfree_enabled']) && $admin_payment_setting['is_cashfree_enabled'] == 'on')
                        <div id="cashfree_payment" class="card">
                            ...
                            <div class="tab-pane" id="cashfree_payment">
                                <form role="form" action="{{ route(ViewsConstants::PLN.'.pay.with.cashfree') }}" method="post"
                    ...
                    @if (isset($admin_payment_setting['is_aamarpay_enabled']) && $admin_payment_setting['is_aamarpay_enabled'] == 'on')
                        <div id="aamarpay_payment" class="card">
                            ...
                            <div class="tab-pane" id="aamarpay_payment">
                                <form role="form" action="{{ route(ViewsConstants::PLN.'.pay.with.aamarpay') }}" method="post"
                    ...
                    @if (isset($admin_payment_setting['is_paytr_enabled']) && $admin_payment_setting['is_paytr_enabled'] == 'on')
                        <div id="paytr_payment" class="card">
                            ...
                            <div class="tab-pane" id="paytr_payment">
                                <form role="form" action="{{ route(ViewsConstants::PLN.'.pay.with.paytr',$plan->id) }}" method="post"
                    ...
                    @if (isset($admin_payment_setting['is_yookassa_enabled']) && $admin_payment_setting['is_yookassa_enabled'] == 'on')
                        <div id="yookassa_payment" class="card">
                            ...
                            <div class="tab-pane" id="yookassa_payment">
                                <form role="form" action="{{ route(ViewsConstants::PLN.'.pay.with.yookassa',$plan->id) }}" method="post"
                    ...
                    @if (isset($admin_payment_setting['is_midtrans_enabled']) && $admin_payment_setting['is_midtrans_enabled'] == 'on')
                        <div id="midtrans_payment" class="card">
                            ...
                            <div class="tab-pane" id="midtrans_payment">
                                <form role="form" action="{{ route(ViewsConstants::PLN.'.pay.with.midtrans',$plan->id) }}" method="post"
                    ...
                    @endif
                     ...
                     <div id="xendit_payment" class="card">
                         ...
                         <div class="tab-pane" id="xendit_payment">
                             <form role="form" action="{{ route(ViewsConstants::PLN.'.pay.with.xendit',$plan->id) }}" method="post"
