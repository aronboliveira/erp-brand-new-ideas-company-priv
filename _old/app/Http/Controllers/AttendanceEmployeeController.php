    ...
    public function index(Request $request)
    {
        ...
        if(\Auth::user()->can('manage attendance'))
        {
            ...
            if(\Auth::user()->type != 'client' && \Auth::user()->type != 'company')
            {
                ...
                $EmployeeAttendance = EmployeeAttendance::where('employee_id', $emp);
                ...
                if($request->type == 'monthly' && !empty($request->month))
                {
                    ...
                    $EmployeeAttendance->whereBetween(
                        'date', [
                                  $start_date,
                                  $end_date,
                              ]
                ...
                {
                    $EmployeeAttendance->where('date', $request->date);
                }
                else
                {
                    $month      = date('m');
            ...
            else
            {
                ...
                $EmployeeAttendance = EmployeeAttendance::whereIn('employee_id', $employee);
...
//                dd($EmployeeAttendance->toSql(), $EmployeeAttendance->getBindings());
                $EmployeeAttendance = $EmployeeAttendance->get();
            ...
            return view('attendance.index', compact('EmployeeAttendance', 'branch', 'department'));
        }
        else
        {
    ...
    public function store(Request $request)
    {
        if(\Auth::user()->can('create attendance'))
        {
            ...
            $attendance = EmployeeAttendance::where('employee_id', '=', $request->employee_id)->where('date', '=', $request->date)->where('clock_out', '=', '00:00:00')->get()->toArray();
            if($attendance)
            {
                return redirect()->route('EmployeeAttendance.index')->with('error', __('Employee Attendance Already Created.'));
            }
            else
            {
                $date = date("Y-m-d");
                ...
                return redirect()->route('EmployeeAttendance.index')->with('success', __('Employee attendance successfully created.'));
            }
        }
        else
        {
    ...
    public function edit($id)
    {
        ...
        {
            $EmployeeAttendance = EmployeeAttendance::where('id', $id)->first();
            $employees          = Employee::where('created_by', '=', \Auth::user()->creatorId())->get()->pluck('name', 'id');

            return view('attendance.edit', compact('EmployeeAttendance', 'employees'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
...
//        dd($request->all());
        ...
        {
            $employeeId      = EmployeeAttendance::where('employee_id', $request->employee_id)->first();
            $check = EmployeeAttendance::where('employee_id', '=', $request->employee_id)->where('date', $request->date)->first();
            // dd($check->date);
            ...
            if ($check->date == date('Y-m-d')) {
                ...
                return redirect()->route('EmployeeAttendance.index')->with('success', __('Employee attendance successfully updated.'));
            } else {
                return redirect()->route('EmployeeAttendance.index')->with('error', __('you can only update current day attendance.'));
            }
        }
        ...
        $todayAttendance = EmployeeAttendance::where('employee_id', '=', $employeeId)->where('date', date('Y-m-d'))->first();
        //        dd($todayAttendance);
...
//        {
            ...
            if(Auth::user()->type == 'Employee')
            {
                ...
                EmployeeAttendance::where('id',$id)->update($EmployeeAttendance);
                //                $EmployeeAttendance->save();
            ...
            else
            {
                ...
                $EmployeeAttendance->clock_out     = $clockout_time;
                $EmployeeAttendance->late          = $late;
                $EmployeeAttendance->early_leaving = $earlyLeaving;
                $EmployeeAttendance->overtime      = $overtime;
                $EmployeeAttendance->total_rest    = '00:00:00';
...
//        }
    ...
    public function destroy($id)
    {
        if(\Auth::user()->can('delete attendance'))
        {
            ...
            return redirect()->route('EmployeeAttendance.index')->with('success', __('Attendance successfully deleted.'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
...
//        {
            ...
            $attendance = EmployeeAttendance::orderBy('id', 'desc')->where('employee_id', '=', $employeeId)->where('clock_out', '=', '00:00:00')->first();

            if($attendance != null)
            {
                $attendance            = EmployeeAttendance::find($attendance->id);
            ...
            $checkDb = EmployeeAttendance::where('employee_id', '=', \Auth::user()->id)->get()->toArray();
                ...
                if(!empty($employees)){
                    foreach($employees as $employee)
                    {
                        ...
                        if($request->$present == 'on')
                        {
                            ...
                            $attendance = EmployeeAttendance::where('employee_id', '=', $employee)->where('date', '=', $request->date)->first();
            ...
            foreach ($attendance as $key => $value) {
                if ($key != 0) {
                    ...
                    if (!empty($employeeData)) {
                        ...
                        $check = EmployeeAttendance::where('employee_id', $employeeId)->where('date', $value[1])->first();
                        if ($check) {
                            $check->update([
                                'late' => $late,
                                'early_leaving' => ($earlyLeaving > 0) ? $earlyLeaving : '00:00:00',
                            ...
                            $time_sheet = EmployeeAttendance::create([
