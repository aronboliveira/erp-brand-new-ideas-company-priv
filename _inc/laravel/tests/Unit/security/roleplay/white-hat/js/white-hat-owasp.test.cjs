...
const BOOLEAN_BLIND = ["1' AND 1=1--", "1' AND 1=2--", "1' AND SUBSTRING(@@version,1,1)='8'--"];
...
const SQLI_PATTERN = /(\bUNION\b|\bSELECT\b|\bSLEEP\b|\bEXTRACTVALUE\b|\bUPDATEXML\b|\bWAITFOR\b|--|;|')/i;
...
describe("White Hat — OWASP SQLi Payload Classification", () => {
  ...
  describe("Encoding analysis", () => {
    test.each(ALL)("encodeURIComponent transforma payload: %s", payload => {
