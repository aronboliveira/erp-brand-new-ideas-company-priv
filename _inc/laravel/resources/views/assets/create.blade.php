...
@endphp
{{ Collective\Html\FormFacade::open(['url'=>ViewsConstants::ACC_AST,'method'=>'post']) }}
    <div class="modal-body">
        ...
        <div class="{{ $row }}">
            <div class="{{ $col12 }} {{ $formGroup }}">
                ...
                {{ Collective\Html\FormFacade::select('employee_id[]',$employee,null,['class'=>"$formControl select2",'id'=>'choices-multiple1','multiple']) }}
            </div>
            <div class="{{ $colMd6 }} {{ $formGroup }}">
                {{ Collective\Html\FormFacade::label('name',__('Name'),['class'=>$formLabel]) }}
                {{ Collective\Html\FormFacade::text('name','',['class'=>$formControl,'required']) }}
            </div>
            <div class="{{ $colMd6 }} {{ $formGroup }}">
                {{ Collective\Html\FormFacade::label('amount',__('Amount'),['class'=>$formLabel]) }}
                {{ Collective\Html\FormFacade::number('amount','',['class'=>$formControl,'required','step'=>'0.01']) }}
            </div>
            <div class="{{ $colMd6 }} {{ $formGroup }}">
                {{ Collective\Html\FormFacade::label('purchase_date',__('Purchase Date'),['class'=>$formLabel]) }}
                {{ Collective\Html\FormFacade::date('purchase_date','',['class'=>$formControl]) }}
            </div>
            <div class="{{ $colMd6 }} {{ $formGroup }}">
                {{ Collective\Html\FormFacade::label('supported_date',__('Supported Date'),['class'=>$formLabel]) }}
                {{ Collective\Html\FormFacade::date('supported_date','',['class'=>$formControl]) }}
            </div>
            <div class="{{ $col12 }} {{ $formGroup }}">
                {{ Collective\Html\FormFacade::label('description',__('Description'),['class'=>$formLabel]) }}
                {{ Collective\Html\FormFacade::textarea('description','',['class'=>$formControl,'rows'=>3]) }}
            </div>
        </div>
    </div>
    <div class="modal-footer">
        <button type="button" class="{{ ViewClassNamesConstants::BT_LG }}" data-bs-dismiss="modal">{{ __('Cancel') }}</button>
        <button type="submit" class="{{ ViewClassNamesConstants::BT_PRM }}">{{ __('Create') }}</button>
    </div>
