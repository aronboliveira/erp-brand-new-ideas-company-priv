...
(() => {
    try {
        const fm = document.querySelector('form[id^="email-template-update-form-"]');
        if (!fm)
            return;
            ...
            fm.addEventListener("submit", (e) => {
                try {
                    ...
                    const msg = fm.getAttribute("data-guard-msg") ??
                        "Update email template route is unavailable. Please contact technical support or your domain administrator.", hasBootstrap = !!(document.querySelector('link[href*="bootstrap"]') &&
                        window.bootstrap);
