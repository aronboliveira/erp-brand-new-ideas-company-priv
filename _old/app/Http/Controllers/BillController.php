...
//            }
    ...
    public function create($vendorId)
    {
        ...
        if(\Auth::user()->can('create bill'))
        {
            ...
            return view('bill.create', compact('vendors', 'bill_number', 'product_services', 'category', 'customFields', 'vendorId','chartAccounts'));
        }
        else
        {
...
//                dd($items);
                ...
                return view('bill.edit', compact('vendors', 'product_services', 'bill', 'bill_number', 'category',
            ...
            if ($bill->created_by == \Auth::user()->creatorId()) {
                ...
                for ($i = 0; $i < count($products); $i++)
                {
                    ...
                    if ($billProduct == null)
                    {
                        ...
                        Utility::updateUserBalance('vendor', $request->vendor_id, $updatePrice, 'debit');
    ...
    public function productDestroy(Request $request)
    {
        ...
        if(\Auth::user()->can('delete bill product'))
        {
            ...
            Utility::updateUserBalance('vendor', $bill->vendor_id, $request->amount, 'credit');
...
//            Utility::userBalance('vendor', $bill->vendor_id, $payment->amount, 'credit');
    ...
    {
        if(\Auth::user()->can('manage vendor bill'))
        {

            $status = Bill::$statues;

            $query = Bill::where('vendor_id', '=', \Auth::user()->vendor_id)->where('status', '!=', '0')->where('created_by', \Auth::user()->creatorId());
    ...
    public function vendorBillShow($id)
    {
        if(\Auth::user()->can('show bill'))
        {
    ...
    public function vendor(Request $request)
    {
        ...
        return view('bill.vendor_detail', compact('vendor'));
    }


    public function vendorBillSend($bill_id)
    {
        return view('vendor.bill_send', compact('bill_id'));
    }
