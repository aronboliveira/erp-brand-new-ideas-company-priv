...
class CrmCompensationService
{
    ...
    public function markRequired(OutboxMessage $message, string $reason, array $context = []): ?OperationLedger
    {
        ...
        $this->operations->recordStep($ledger, 'compensation.required:' . $message->id, 'CRM compensation required', [
            'step_type' => 'cleanup',
            'sequence' => 900,
        ...
        $this->events->record('crm.compensation.required', 'CRM operation needs compensation', [
            'reason' => $reason,
            'message_key' => $message->message_key,
            'event_type' => $message->event_type,
