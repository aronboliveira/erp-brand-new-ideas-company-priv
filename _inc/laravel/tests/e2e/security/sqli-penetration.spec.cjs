// @ts-check
/**
 * @file sqli-penetration.spec.cjs
...
async function assertNoSqlLeak(page, context) {
  ...
  for (const pattern of SQL_LEAK_PATTERNS) {
    expect(
      ...
      `Vazamento SQL (${pattern}) detectado em ${context}`
    ).toBe(false);
  }
}
...
test.describe("SQLi Penetração — Login", () => {
  for (const payload of SQLI_PAYLOADS.slice(0, 6)) {
    test(`Login rejeita payload: ${payload.substring(0, 30)}`, async ({
      page,
    }) => {
      ...
      const emailInput = page
        .locator('#email-input, input[name="email"]')
        .first();
      const pwInput = page
        .locator('#pw-input, input[name="password"]')
        .first();

      if (
        (await emailInput.isVisible({ timeout: 5000 }).catch(() => false)) &&
        (await pwInput.isVisible({ timeout: 3000 }).catch(() => false))
      ) {
        ...
        const submitBtn = page
          .locator(
            '#saveBtn, button[type="submit"], input[type="submit"]'
          )
          .first();
        if (
...
test.describe("SQLi Penetração — Busca GET", () => {
  ...
  for (const { url, param } of SEARCH_ROUTES) {
    for (const payload of SQLI_PAYLOADS.slice(0, 4)) {
      test(`GET ${url}?${param}= rejeita SQLi: ${payload.substring(0, 25)}`, async ({
        page,
      }) => {
        ...
        await assertNoSqlLeak(
          page,
          `GET ${url}?${param}=${payload.substring(0, 25)}`
        );
      });
    }
  }
...
test.describe("SQLi Penetração — POST Formulários", () => {
  ...
  for (const target of FORM_TARGETS) {
    test(`POST ${target.createUrl} rejeita SQLi nos campos`, async ({
      page,
    }) => {
      ...
      const submitBtn = page
        .locator(
          'button[type="submit"], input[type="submit"], #saveBtn'
        )
        .first();
...
test.describe("SQLi Penetração — API Endpoints", () => {
  ...
  for (const payload of SQLI_PAYLOADS.slice(0, 5)) {
    test(`API POST /login com SQLi retorna status seguro: ${payload.substring(0, 25)}`, async ({
      request,
    }) => {
      // Obter CSRF token
      ...
      expect(
        SAFE_STATUSES,
        `Status HTTP inseguro (${resp.status()}) para payload: ${payload}`
      ).toContain(resp.status());

      const body = await resp.text();
      for (const p of SQL_LEAK_PATTERNS) {
        expect(
          p.test(body.substring(0, 5000)),
          `Vazamento SQL (${p}) na resposta de /login`
        ).toBe(false);
      }
    });
  }
  ...
  for (const payload of SEARCH_PAYLOADS.slice(0, 4)) {
    test(`API GET /invoices?search= com SQLi: ${payload.substring(0, 25)}`, async ({
      request,
    }) => {
      const resp = await request.get(
        `${BASE_URL}/invoices?search=${encodeURIComponent(payload)}`,
        { maxRedirects: 5 }
      );

      expect(
        resp.status(),
        `500 em GET /invoices?search=${payload}`
      ).not.toBe(500);
    });
  }
});
...
test.describe("SQLi Penetração — Path Params", () => {
  ...
  for (const route of ROUTES) {
    for (const malId of MALICIOUS_IDS.slice(0, 3)) {
      test(`${route}/${malId.substring(0, 20)} retorna ≠ 500`, async ({
        page,
      }) => {
        const response = await page.goto(
          `${BASE_URL}${route}/${encodeURIComponent(malId)}`,
...
test.describe("SQLi Penetração — Time-Based Blind", () => {
  test("Login com SLEEP payload não demora > 4s", async ({ page }) => {
    ...
    expect(
      elapsed,
      `Possível blind SQLi! Login demorou ${elapsed}ms`
    ).toBeLessThan(15000);
