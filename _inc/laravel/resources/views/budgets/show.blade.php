...
$user = Auth::user();
        ...
        \Log::error('budgets/show — ' . get_class($e) . ': ' . $e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
    }
@endphp
...
@endphp
        ...
        @endpush
        <li class="{{ VC::BCI }}">{{ $budget->name ?? __('No budget name available') }}</li>
    @endsection
    @push(StacksConstants::ADM_SCR_PG)
    ...
    @section(YieldingConstants::ADM_CTT)
        <div class="{{ VC::C12 }} {{ VC::MT4 }}">
            <div class="{{ VC::CD_POS }}">
                <h6 class="{{ VC::RPT_TX_DEF }} {{ VC::TXCT }}">{{__('Year :')}} {{ $budget->from }}</h6>
            </div>
        </div>
        <div class="row">
            <div class="{{ VC::C12 }}">
                <div class="card">
                    <div class="{{ VC::CD_BD }}" style="overflow-x: auto">
                        {{--  Monthly Budget--}}
...
@endphp
                                    <tr>
                                        ...
                                        @if($hasMonths)
                                            @foreach($monthList as $month)
                                                <th colspan="3" scope="colgroup" class="{{ VC::TXCT_BR1 }}">{{ isset($month) && $month !== '' ? $month : __('No month label available') }}</th>
                                            @endforeach
                                        @else
                                            <th colspan="3" scope="colgroup" class="{{ VC::TXCT_BR1 }}">{{ __('No months available') }}</th>
                                        @endif
                                    </tr>
                                    <tr>
                                        @if($hasMonths)
                                ...
                                <tr>
                                    <th colspan="37" class="{{ VC::TX_DK_LBL }}"><span>{{ __('Income :') }}</span></th>
                                </tr>
...
@endphp
                                ...
                                @else
                                    <tr>
                                        <td class="{{ VC::TXCT_DK }}" colspan="37">{{ __('No income products or months available') }}</td>
                                    </tr>
                                @endif
...
@endphp
                                ...
                                <tr class="total">
                                    <td class="{{ VC::TX_DK }}"><span></span><strong>{{ __('Total :') }}</strong></td>
                                    @if($hasMonths)
                                        @foreach($monthList as $month)
                                            @php
                                                try {
...
@endphp
                                            <td class="{{ VC::TX_DK }} {{ $month }}_total_income"><strong>{{ $bFmt }}</strong></td>
                                            <td class="{{ VC::TX_DK }}">
                                                <strong>{{ $aFmt }}</strong>
                                                <p>{{ $pctA != 0 ? '(' . $pctA . '%)' : '' }}</p>
                                            </td>
                                        ...
                                        @endforeach
                                    @else
                                        <td class="{{ VC::TX_DK }}" colspan="3"><strong>{{ __('No totals available (missing months)') }}</strong></td>
                                    @endif
                                </tr>

                                <tr>
                                    <th colspan="37" class="{{ VC::TX_DK_LBL }}"><span>{{ __('Expense :') }}</span></th>
                                </tr>
...
@endphp
                                ...
                                @else
                                    <tr>
                                        <td class="{{ VC::TXCT_DK }}" colspan="37">{{ __('No expense products or months available') }}</td>
                                    </tr>
                                @endif
...
@endphp
                                            <td class="{{ VC::TX_DK }} {{ $month }}_total_expense"><strong>{{ $bFmt }}</strong></td>
                                            <td class="{{ VC::TX_DK }}">
                                                <strong>{{ $aFmt }}</strong>
                                                <p>{{ $pctA != 0 ? '(' . $pctA . '%)' : '' }}</p>
                                            </td>
                                ...
                                <tfoot>
                                    <tr class="total">
                                        <td class="{{ VC::TX_DK }}"><span></span><strong>{{ __('NET PROFIT :') }}</strong></td>
                                        @php
                                            $overbudgetprofit ??= [];
                                            try {
...
@endphp
                                                <td class="{{ VC::TX_DK }}"><strong>{{ $bFmt }}</strong></td>
                                                <td class="{{ VC::TX_DK }}">
                                                    <strong>{{ $aFmt }}</strong>
                                                    <p>{{ $pctA != 0 ? '(' . $pctA . '%)' : '' }}</p>
                                                </td>
                                            ...
                                            @endforeach
                                        @else
                                            <td class="{{ VC::TX_DK }}" colspan="3"><strong>{{ __('No profit data available (missing months)') }}</strong></td>
                                        @endif
                                    </tr>
                                </tfoot>
                            </table>
...
@endphp
                                ...
                                <tr class="total">
                                    <td class="{{ VC::TX_DK }}"><strong>{{ __('Total :') }}</strong></td>
...
@endphp
                                        ...
                                        @endforeach
                                    @else
                                        <td class="{{ VC::TX_DK }}" colspan="3"><strong>{{ __('No income totals available') }}</strong></td>
                                    @endif
                                </tr>
...
@endphp
                                        ...
                                        @endforeach
                                    @else
                                        <td class="{{ VC::TX_DK }}" colspan="3"><strong>{{ __('No expense totals available') }}</strong></td>
                                    @endif
                                </tr>
...
@endphp
                                                <td class="{{ VC::TX_DK }}"><strong>{{ $bFmt }}</strong></td>
                                                <td class="{{ VC::TX_DK }}"><strong>{{ $aFmt }}</strong></td>
                                                <td class="{{ VC::TX_DK }}">
                                                    <strong>{{ $oFmt }}</strong>
                                                    <p class="{{ $oClass }}">{{ $pctO != 0 ? '(' . $pctO . '%)' : '' }}</p>
                                                </td>
                                            @endforeach
                                        @else
                                            <td class="{{ VC::TX_DK }}" colspan="3"><strong>{{ __('No profit data available') }}</strong></td>
                                        @endif
                                    </tr>
                                </tfoot>
                            </table>
...
@endphp
    ...
    @endsection
@else
    <div class="{{ VC::ALT_WRN }}">{{__('No budget data available')}}</div>
@endif
