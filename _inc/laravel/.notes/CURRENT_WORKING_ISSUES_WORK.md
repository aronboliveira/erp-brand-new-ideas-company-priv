...
> Issue listings and resolution summaries live in sibling files:
>
> - **Open issues:** [`KNOWN_ISSUES.md`](KNOWN_ISSUES.md)
> - **Resolved issues:** [`RESOLVED_ISSUES.md`](RESOLVED_ISSUES.md)
> - **Summarized sessions:** [`CURRENT_WORKING_ISSUES.md`](CURRENT_WORKING_ISSUES.md)
>
> This file documents the _process_ — the trial-and-error path to each fix.
> Last updated: 2026-05-09
> **Cross-references:** [`NEXT_STEPS.md`](NEXT_STEPS.md) (remaining work) · [`README.md`](README.md) (notes overview) · [`KNOWN_ISSUES.md`](KNOWN_ISSUES.md) (open issues) · [`RESOLVED_ISSUES.md`](RESOLVED_ISSUES.md) (resolved archive)
...
## ✅ Session 11 — Claude handoff recovery and current-state correction (2026-05-09)

### Tried

- Read the stale `.tmp/claude/20260808/HANDOFF.md` / `.xml` pair.
- Searched the local Claude project store for session
  `d9570c96-1845-40c3-b642-65229ebd51b9`.
- Inspected the final JSONL entries and the captured PHPUnit logs in
  `/tmp/v3.log` and Claude task outputs.

### Found

- The archived handoff stopped at `a0243988d`, but the live session
  continued to `157145304`.
- The old handoff's PHPStan, ESLint, and UtilityTest failure sections
  were stale because later commits fixed them.
- Claude stopped because of usage/token limit while waiting on final
  PHPUnit verification.

### ✅ Success

- Updated `.tmp/claude/20260808/HANDOFF.md` and `HANDOFF.xml` to the
  true token-limit state.
- Updated `KNOWN_ISSUES.md`, `NEXT_STEPS.md`, `RESOLVED_ISSUES.md`,
  and `CURRENT_WORKING_ISSUES.md` to make RT-007..RT-011 the current
  open set.
- Merged legacy `./notes` content into root `./.notes` and turned
  `./notes` files into redirect/stale shims.

---

## ✅ Session 12 — Codex Unit-suite continuation (2026-05-09)

### Tried

- Patched only tests under `_inc/laravel/tests`.
- Kept migrations and `_inc/.seeders/` untouched.
- Used `vendor/bin/phpunit`, not `php artisan test`.

### Fixed

- RT-007: randomized unique fields in the Order UUID primary-key test.
- RT-008: seeded local storage settings for the Discover upload-failure path.
- RT-009: seeded local storage settings and ensured the landing-page upload
  directory for the Features upload success path.
...
- Handoff saved at `.tmp/codex/20260509/handsoff.md`.
